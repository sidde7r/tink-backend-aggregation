...
/**
 * Tests that {@link AmazonHttpClient#executeHelper()} method passes the correct context information
 * into the configured RetryPolicy.
 */
public class AmazonHttpClientRetryPolicyTest extends RetryPolicyTestBase {
    ...
    /**
     * Tests AmazonHttpClient's behavior upon simulated service exceptions when the request payload
     * is repeatable.
     */
    @Test
    public void testServiceExceptionHandling() {
        ...
        injectMockHttpClient(
                testedClient,
                new ReturnServiceErrorHttpClient(random500StatusCode, randomErrorCode));
        ...
        // Verifies that the correct information was passed into the RetryCondition and
        // BackoffStrategy
        verifyExpectedContextData(
                retryCondition, originalRequest, expectedServiceException, EXPECTED_RETRY_COUNT);
        verifyExpectedContextData(
                backoffStrategy, originalRequest, expectedServiceException, EXPECTED_RETRY_COUNT);
    ...
    /**
     * Tests AmazonHttpClient's behavior upon simulated IOException during executing the http
     * request when the request payload is repeatable.
     */
    @Test
    public void testIOExceptioinHandling() {
        ...
        // BackoffStrategy
        verifyExpectedContextData(
                retryCondition, originalRequest, expectedClientException, EXPECTED_RETRY_COUNT);
        verifyExpectedContextData(
                backoffStrategy, originalRequest, expectedClientException, EXPECTED_RETRY_COUNT);
    ...
    public void testServiceExceptionHandlingWithNonRepeatableRequestContent() {
        ...
        Request<?> testedNonRepeatableRequest =
                getSampleRequestWithNonRepeatableContent(originalRequest);
        ...
        verifyExpectedContextData(retryCondition, null, null, EXPECTED_RETRY_COUNT);
    ...
    /**
     * Tests AmazonHttpClient's behavior upon simulated IOException when the request payload is not
     * repeatable.
     */
    @Test
    ...
    /**
     * Tests AmazonHttpClient's behavior upon simulated RuntimeException (which should be handled as
     * an unexpected failure and not retried).
     */
    @Test
