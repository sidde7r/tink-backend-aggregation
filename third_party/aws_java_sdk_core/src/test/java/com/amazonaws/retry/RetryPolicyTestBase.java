...
public class RetryPolicyTestBase {
    ...
    protected static final AmazonWebServiceRequest originalRequest =
            new TestAmazonWebServiceRequest();
    protected static final HttpResponseHandler<AmazonServiceException> errorResponseHandler =
            new TestHttpResponseHandler();

    /**
     * The retry condition and back-off strategy implementations that record all the context data
     * passed into shouldRetry and calculateSleepTime methods.
     */
    ...
    public static void injectMockHttpClient(
            AmazonHttpClient amazonHttpClient, ConnectionManagerAwareHttpClient mockHttpClient) {
        try {
    ...
    public static Request<?> getSampleRequestWithRepeatableContent(
        ...
        DefaultRequest<?> request =
                new DefaultRequest(amazonWebServiceRequest, "non-existent-service");
    ...
    public static Request<?> getSampleRequestWithNonRepeatableContent(
        ...
        request.setContent(
                new NonRepeatableInputStream("Some content that could only be read once."));
        return request;
    }
    ...
    private static class ContextDataCollection {
        ...
        public List<AmazonWebServiceRequest> failedRequests =
                new LinkedList<AmazonWebServiceRequest>();
        ...
        if (expectedRetries > 0) {
            if (failedRequest != null) {
                ...
                for (AmazonWebServiceRequest seenRequest : contextDataCollection.failedRequests) {
                    Assert.assertTrue(
                            "seeRequest=" + seenRequest + ", failedRequest=" + failedRequest,
    ...
    /**
     * An error response handler implementation that simply - keeps the status code - sets the error
     * code by the status text (which comes from the reason phrase in the low-level response)
     */
    public static class TestHttpResponseHandler
    ...
    /**
     * A mock HttpClient implementation that does nothing but throws the specified IOException or
     * RuntimeException upon any call on execute(...) method.
     */
    public static class ThrowingExceptionHttpClient extends MockHttpClient {
        ...
        public HttpResponse execute(HttpUriRequest request)
            ...
            if (t instanceof IOException) {
                ...
                Assert.fail(
                        "The expected throwable should be either an IOException or RuntimeException.");
                return null;
            }
        }
    }
    ...
    /**
     * A mock HttpClient implementation that does nothing but directly returns a BasicHttpResponse
     * object with the specified status code upon any call on execute(...) method.
     */
