...
/**
 * Tests the behavior when both {@link ClientConfiguration#setMaxErrorRetry(int)} and {@link
...
public class ClientConfigurationMaxErrorRetryTest extends RetryPolicyTestBase {
    ...
    /**
     * -- Explicitly set maxErrorRetry in ClientConfiguration level; -- Default/custom RetryPolicy's
     * that don't override such setting.
     */
    @Test
    ...
    /**
     * -- Explicitly set maxErrorRetry in ClientConfiguration level; -- Custom RetryPolicy's that
     * want to override such setting.
     */
    @Test
    public void testRetryPolicyLevelMaxErrorRetry() {
        ...
        clientConfiguration.setRetryPolicy(
                new RetryPolicy(null, null, RETRY_POLICY_LEVEL_MAX_ERROR_RETRY, false));
        ...
        clientConfiguration.setRetryPolicy(
                new RetryPolicy(
                        ...
                        null,
                        RETRY_POLICY_LEVEL_MAX_ERROR_RETRY,
                        true));
    ...
    private void testActualRetries(int expectedRetryAttempts) {
        ...
        injectMockHttpClient(
                testedClient, new ReturnServiceErrorHttpClient(500, "fake 500 service error"));
