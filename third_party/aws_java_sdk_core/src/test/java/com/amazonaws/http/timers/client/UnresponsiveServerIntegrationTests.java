...
public class UnresponsiveServerIntegrationTests extends UnresponsiveMockServerTestBase {
    ...
    private static final int LONGER_SOCKET_TIMEOUT =
            CLIENT_EXECUTION_TIMEOUT * PRECISION_MULTIPLIER;
    private static final int SHORTER_SOCKET_TIMEOUT =
            CLIENT_EXECUTION_TIMEOUT / PRECISION_MULTIPLIER;
    private static final int LONGER_REQUEST_TIMEOUT =
            CLIENT_EXECUTION_TIMEOUT * PRECISION_MULTIPLIER;
    private static final int SHORTER_REQUEST_TIMEOUT =
            CLIENT_EXECUTION_TIMEOUT / PRECISION_MULTIPLIER;
    ...
    public void clientExecutionTimeoutDisabled_SocketTimeoutExceptionIsThrown_NoThreadsCreated() {
        httpClient =
                new AmazonHttpClient(
                        new ClientConfiguration().withSocketTimeout(1 * 1000).withMaxErrorRetry(0));
     ...
     * interrupts the current thread for another reason we don't want to squash the {@link
     * InterruptedException}. We should set the thread's interrupted status and throw the exception
     * back out (we can't throw the actual {@link InterruptedException} because it's checked)
     */
    @Test(timeout = TEST_TIMEOUT)
    ...
    public void
        ...
        httpClient =
                new AmazonHttpClient(
                        new ClientConfiguration()
                                .withClientExecutionTimeout(CLIENT_EXECUTION_TIMEOUT)
                                .withRequestTimeout(LONGER_REQUEST_TIMEOUT)
                                .withMaxErrorRetry(0));
    ...
    public void
        ...
        httpClient =
                new AmazonHttpClient(
                        new ClientConfiguration()
                                .withClientExecutionTimeout(CLIENT_EXECUTION_TIMEOUT)
                                .withRequestTimeout(SHORTER_REQUEST_TIMEOUT)
                                .withMaxErrorRetry(0));
