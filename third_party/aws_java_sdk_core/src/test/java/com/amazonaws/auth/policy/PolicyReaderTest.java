...
public class PolicyReaderTest {
    ...
    public void testPrincipals() {
        ...
        policy.withStatements(
                new Statement(Effect.Allow)
                        .withResources(new Resource("resource"))
                        .withPrincipals(
                                new Principal(Services.AmazonEC2),
        ...
        assertEquals(
                ...
                statements.get(0).getPrincipals().get(0).getId());
        assertEquals("Service", statements.get(0).getPrincipals().get(1).getProvider());
        assertEquals(
                Services.AmazonElasticTranscoder.getServiceId(),
                statements.get(0).getPrincipals().get(1).getId());
        ...
        assertThat(
                statements.get(0).getPrincipals(),
    ...
    public void testMultipleConditionKeysForConditionType() throws Exception {
        ...
        policy.withStatements(
                new Statement(Effect.Allow)
                        ...
                        .withConditions(
                                new StringCondition(
                                        StringComparisonType.StringNotLike, "key1", "foo"),
                                new StringCondition(
                                        StringComparisonType.StringNotLike, "key1", "bar")));
    ...
    /**
     * Test policy parsing when the "Effect" is not mentioned in a Statement. The Effect must be
     * default to "Deny" when it is not mentioned.
     */
    @Test
    ...
    public void testMultipleStatements() throws Exception {
        ...
        assertEquals(
                ...
                statements.get(0).getConditions().get(0).getConditionKey());
        assertEquals(1, statements.get(0).getConditions().get(0).getValues().size());
        assertEquals(
                "192.168.143.0/24", statements.get(0).getConditions().get(0).getValues().get(0));
        ...
        assertEquals(
                "192.168.143.188/32", statements.get(0).getConditions().get(1).getValues().get(0));

        assertEquals(
                ConditionFactory.SOURCE_IP_CONDITION_KEY,
                statements.get(1).getConditions().get(0).getConditionKey());
    ...
    public void testNoStatementArray() {
        ...
        assertEquals(1, statements.get(0).getActions().size());
        assertEquals(
                "acm:DescribeCertificate", statements.get(0).getActions().get(0).getActionName());
    ...
    public void testFederatedUserBySAMLProvider() {
        ...
        assertEquals(Effect.Allow, statements.get(0).getEffect());
        assertEquals(
                "sts:AssumeRoleWithSAML", statements.get(0).getActions().get(0).getActionName());
        assertEquals(1, statements.get(0).getConditions().size());
        ...
        assertEquals(
                ...
                statements.get(0).getConditions().get(0).getValues().get(0));
        assertEquals(1, statements.get(0).getPrincipals().size());
        assertEquals(
                "arn:aws:iam::862954416975:saml-provider/myprovider",
                statements.get(0).getPrincipals().get(0).getId());
    ...
    public void testCloudHSMServicePrincipal() {
        ...
        assertEquals(1, statements.get(0).getPrincipals().size());
        assertEquals(
                Services.AWSCloudHSM.getServiceId(),
                statements.get(0).getPrincipals().get(0).getId());
        assertEquals("Service", statements.get(0).getPrincipals().get(0).getProvider());
    }
     ...
     *
     * @see TT:0030871921
     *     <p>When a service is mentioned in the principal, we always try to figure out the service
     *     from <code>com.amazonaws.auth.policy.Principal.Services</code> enum. For new services
     *     introduced, if the enum is not updated, then the parsing fails.
     */
    @Test
