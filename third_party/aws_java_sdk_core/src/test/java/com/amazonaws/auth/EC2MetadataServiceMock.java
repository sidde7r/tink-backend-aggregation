...
/**
 * Mock server for imitating the Amazon EC2 Instance Metadata Service. Tests can use this class to
 * start up a server on a localhost port, and control what response the server will send when a
 * connection is made.
 */
public class EC2MetadataServiceMock {
    ...
    /**
     * Sets the name of the file that should be sent back as the response from this mock server. The
     * files are loaded from the com/amazonaws/auth directory of the tst folder, and no file
     * extension should be specified.
     *
     * @param responseFileName The name of the file that should be sent back as the response from
     *     this mock server.
     */
    ...
    /**
     * Accepts a newline delimited list of security credential names that the mock metadata service
     * should advertise as available.
     *
    ...
    private ServerSocket startServerSocket() {
        try {
            ...
            System.out.println(
                    "Started mock metadata service at: "
    ...
    /**
     * Thread subclass that listens for connections on an opened server socket and response with a
     * predefined response file.
     */
                ...
                try {
                    ...
                    if (resourcePath.equals(EC2MetadataUtils.SECURITY_CREDENTIALS_RESOURCE)) {
                        ...
                        String responseFilePath =
                                "/com/amazonaws/auth/" + responseFileName + ".json";
                        System.out.println("Serving: " + responseFilePath);

                        InputStream responseFileInputStream =
                                this.getClass().getResourceAsStream(responseFilePath);

                        List<String> dataFromFile = IOUtils.readLines(responseFileInputStream);

                        StringBuilder credentialsString = new StringBuilder();

                        for (String line : dataFromFile) credentialsString.append(line);

                        httpResponse = formHttpResponse(credentialsString.toString());
                        outputStream.write(httpResponse.getBytes());
