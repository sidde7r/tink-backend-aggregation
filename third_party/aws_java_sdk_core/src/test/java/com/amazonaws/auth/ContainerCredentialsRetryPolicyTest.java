...
public class ContainerCredentialsRetryPolicyTest {
    ...
    public void shouldRetry_ReturnsTrue_For5xxStatusCode() {
        assertTrue(
                retryPolicy.shouldRetry(
                        1,
                        CredentialsEndpointRetryParameters.builder().withStatusCode(501).build()));
    }
    ...
    public void shouldRetry_ReturnsFalse_ForNon5xxStatusCode() {
        assertFalse(
                retryPolicy.shouldRetry(
                        1,
                        CredentialsEndpointRetryParameters.builder().withStatusCode(404).build()));
        assertFalse(
                retryPolicy.shouldRetry(
                        1,
                        CredentialsEndpointRetryParameters.builder().withStatusCode(300).build()));
        assertFalse(
                retryPolicy.shouldRetry(
                        1,
                        CredentialsEndpointRetryParameters.builder().withStatusCode(202).build()));
    }
