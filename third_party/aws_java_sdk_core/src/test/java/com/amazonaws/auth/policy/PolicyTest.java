...
public class PolicyTest {
    ...
    public void testPrincipals() {
        ...
        assertEquals(users.asText(), "accountId1");
        assertEquals(
                webProviders.get(0).asText(), WebIdentityProviders.Amazon.getWebIdentityProvider());
        assertEquals(webProviders.get(1).asText(), "*");
    }

    /**
     * Policies with multiple conditions that use the same comparison type must be merged together
     * in the JSON format, otherwise there will be two keys with the same name and one will override
     * the other.
     */
    @Test
    public void testMultipleConditionKeysForConditionType() throws Exception {
        ...
        JsonNode stringLikeCondition =
                conditions.get(StringComparisonType.StringNotLike.toString());
    ...
    /**
     * Tests that a policy correctly assigns unique statement IDs to any added statements without
     * IDs yet.
     */
    @Test
    ...
    /**
     * Tests that a Statement including a Resource specified as a NotResource element is serialized
     * to JSON with a "NotResource" attribute
     */
    @Test
    public void testNotResourceElement() {
        Policy policy =
                new Policy("S3PolicyId1")
                        .withStatements(
                                new Statement(Effect.Allow)
                                        .withId("0")
                                        .withPrincipals(Principal.AllUsers)
                                        .withActions(new TestAction("action1"))
                                        .withResources(
                                                new Resource("any-resource").withIsNotType(true)));
    ...
    /** Asserts that each statement in the specified policy has a unique ID assigned to it. */
