...
public class ProfilesConfigFileWriter {
    ...
    /**
     * Write all the credential profiles to a file. Note that this method will clobber the existing
     * content in the destination file if it's in the overwrite mode. Use {@link
     * #modifyOrInsertProfiles(File, Profile...)} instead, if you want to perform in-place
     * modification on your existing credentials file.
     *
     * @param destination The destination file where the credentials will be written to.
     * @param overwrite If true, this method If false, this method will throw exception if the file
     *     already exists.
    ...
    public static void dumpToFile(File destination, boolean overwrite, Profile... profiles) {
        ...
        try {
            writer =
                    new OutputStreamWriter(
                            new FileOutputStream(destination, false), StringUtils.UTF8);
        ...
        try {
            ...
            ProfilesConfigFileWriterHelper writerHelper =
                    new ProfilesConfigFileWriterHelper(writer, modifications);
     ...
     * Only the properties of the affected profiles will be modified; all the unaffected profiles
     * and comment lines will remain the same. This method does not support renaming a profile.
     *
    ...
    /**
     * Modify one profile in the existing credentials file by in-place modification. This method
     * will rename the existing profile if the specified Profile has a different name.
     *
     * @param destination The destination file to modify
    ...
    public static void modifyOneProfile(File destination, String profileName, Profile newProfile) {
        final Map<String, Profile> modifications =
                Collections.singletonMap(profileName, newProfile);
    ...
    /**
     * A package-private method that supports all kinds of profile modification, including renaming
     * or deleting one or more profiles.
     *
    ...
    /**
     * Implementation of AbstractProfilesConfigFileScanner, which reads the content from an existing
     * credentials file (if any) and then modifies some of the profile properties in place.
     */
    private static class ProfilesConfigFileWriterHelper extends AbstractProfilesConfigFileScanner {
        ...
        private final Map<String, Set<String>> existingProfileProperties =
         ...
         * @param modifications A map of all the new profiles, keyed by the profile name. If a
         *     profile name is associated with a null value, it's profile content will be removed.
         */
        ...
        /**
         * Read the existing content of a credentials file, and then make in-place modification
         * according to the new profiles specified in this class.
         */
        ...
        protected void onProfileStartingLine(String profileName, String line) {
            ...
            if (deletedProfiles.contains(profileName)) return;
        ...
        protected void onProfileProperty(
            ...
            if (newProfiles.containsKey(profileName)) {
                String newValue = newProfiles.get(profileName).getPropertyValue(propertyKey);
                if (newValue != null) {
