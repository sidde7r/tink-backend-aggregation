...
public class AWS4Signer extends AbstractAWSSigner
    ...
    private static final FIFOCache<SignerKey> signerCache =
            new FIFOCache<SignerKey>(SIGNER_CACHE_MAX_SIZE);
    private static final List<String> listOfHeadersToIgnoreInLowerCase =
    ...
    /**
     * Endpoint prefix to compute the region name for signing when the {@link #regionName} is null.
     */
    ...
    /**
     * Whether double url-encode the resource path when constructing the canonical request. By
     * default, we enable double url-encoding.
     *
     * <p>TODO: Different sigv4 services seem to be inconsistent on this. So for services that want
     * to suppress this, they should use new AWS4Signer(false).
     */
     ...
     *
     * @param doubleUrlEncoding Whether double url-encode the resource path when constructing the
     *     canonical request.
     */
    ...
    /**
     * Sets the service name that this signer should use when calculating request signatures. This
     * can almost always be determined directly from the request's end point, so you shouldn't need
     * this method, but it's provided for the edge case where the information is not in the
     * endpoint.
     *
     * @param serviceName The service name to use when calculating signatures in this signer.
     */
    @Override
    ...
    /**
     * Sets the region name that this signer should use when calculating request signatures. This
     * can almost always be determined directly from the request's end point, so you shouldn't need
     * this method, but it's provided for the edge case where the information is not in the
     * endpoint.
     *
     * @param regionName The region name to use when calculating signatures in this signer.
     */
    @Override
    ...
    /**
     * Sets the endpoint prefix which is used to compute the region that is used for signing the
     * request.
     *
     * <p>This value is passed to {@link AWS4SignerRequestParams} class which has the logic to
     * compute region.
     *
    ...
    /**
     * Sets the date that overrides the signing date in the request. This method is internal and
    ...
    /**
     * Returns a copy of date that overrides the signing date in the request. Return null by
     * default.
     */
    ...
    public void sign(SignableRequest<?> request, AWSCredentials credentials) {
        ...
        final AWS4SignerRequestParams signerParams =
    ...
    public void presignRequest(
        ...
        final AWS4SignerRequestParams signerRequestParams =
                new AWS4SignerRequestParams(
                        request,
                        overriddenDate,
                        regionName,
     ...
     * .amazon.com/general/latest/gr/sigv4-create-canonical-request.html to generate the canonical
     * request.
     */
    protected String createCanonicalRequest(SignableRequest<?> request, String contentSha256) {
        ...
        final String path =
                SdkHttpUtils.appendUri(request.getEndpoint().getPath(), request.getResourcePath());

        final StringBuilder canonicalRequestBuilder =
                new StringBuilder(request.getHttpMethod().toString());
     ...
     */
    protected String createStringToSign(
            String canonicalRequest, AWS4SignerRequestParams signerParams) {

        final StringBuilder stringToSignBuilder =
                new StringBuilder(signerParams.getSigningAlgorithm());
        stringToSignBuilder
                .append(LINE_SEPARATOR)
        ...
        if (log.isDebugEnabled()) log.debug("AWS4 String to Sign: '\"" + stringToSign + "\"");

        return stringToSign;
    }

    /**
     * Step 3 of the AWS Signature version 4 calculation. It involves deriving the signing key and
     * computing the signature. Refer to http://docs.aws.amazon
     * .com/general/latest/gr/sigv4-calculate-signature.html
     */
    private final byte[] deriveSigningKey(
        ...
        if (log.isDebugEnabled()) {
            log.debug(
                    "Generating a new signing key as the signing key not available in the cache for the date "
    ...
    private final String computeSigningCacheKeyName(
            AWSCredentials credentials, AWS4SignerRequestParams signerRequestParams) {
    ...
    protected final byte[] computeSignature(
            String stringToSign, byte[] signingKey, AWS4SignerRequestParams signerRequestParams) {
        return sign(
                stringToSign.getBytes(Charset.forName("UTF-8")),
                signingKey,
    ...
    private String buildAuthorizationHeader(
        ...
        final String signingCredentials =
                credentials.getAWSAccessKeyId() + "/" + signerParams.getScope();
    ...
    /**
     * Calculate the hash of the request's payload. Subclass could override this method to provide
     * different values for "x-amz-content-sha256" header or do any other necessary set-ups on the
     * request headers. (e.g. aws-chunked uses a pre-defined header value, and needs to change some
     * headers relating to content-encoding and content-length.)
     */
     ...
     * chunk-encoding). The default implementation doesn't need to do anything.
     */
    ...
    /**
     * Calculate the hash of the request's payload. In case of pre-sign, the existing code would
     * generate the hash of an empty byte array and returns it. This method can be overridden by sub
     * classes to provide different values (e.g) For S3 pre-signing, the content hash calculation is
    ...
    /**
     * Generates an expiration date for the presigned url. If user has specified an expiration date,
     * check if it is in the given limit.
     */
    ...
    protected byte[] newSigningKey(
            AWSCredentials credentials, String dateStamp, String regionName, String serviceName) {
        byte[] kSecret =
                ("AWS4" + credentials.getAWSSecretKey()).getBytes(Charset.forName("UTF-8"));
