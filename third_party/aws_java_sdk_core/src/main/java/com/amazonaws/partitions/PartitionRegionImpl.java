...
public class PartitionRegionImpl implements RegionImpl {
    ...
    /**
     * Returns the endpoint for the given service. If the region is not present under service
     * configuration; but matches the region regex, then this system tries to guess the endpoint and
     * returns it.
     */
    @Override
            ...
            if (service.getEndpoints().containsKey(region)) {
                ...
                Endpoint merged =
                        Endpoint.merge(
                                ...
                                Endpoint.merge(
                                        service.getDefaults(), service.getEndpoints().get(region)));
                        ...
                        Endpoint.merge(
                                ...
                                Endpoint.merge(
                                        ...
                                        service.getEndpoints()
                                                .get(service.getPartitionEndpoint())));
    ...
    /**
     * Returns true if the service is explicitly enabled for a region or a service is partition wide
     * enabled. Returns false otherwise.
     *
     * <p>For new regions that match the partition's region regex, if the service is enabled
     * partition wide then this method will return true.
     */
    @Override
    ...
    /**
     * This method returns true only if the metadata for the service contains the given region in
    ...
    /**
     * Returns true if the service is explicitly enabled for https in this region or a service is
     * partition wide enabled. Returns false otherwise.
     *
     * <p>For new regions that match the partition's region regex, if the service is enabled
     * partition wide then this method will return true.
     */
    ...
    /**
     * Returns true if the service is explicitly enabled for http in this region or a service is
     * partition wide enabled. Returns false otherwise.
     *
     * <p>For new regions that match the partition's region regex, if the service is enabled
     * partition wide then this method will return true.
