...
public class Endpoint {
    ...
    public static Endpoint merge(Endpoint defaults, Endpoint override) {
        ...
        merged.setCredentialScope(
                override.getCredentialScope() != null
                        ? override.getCredentialScope()
                        : defaults.getCredentialScope());

        merged.setHostName(
                override.getHostName() != null ? override.getHostName() : defaults.getHostName());

        merged.setSslCommonName(
                override.getSslCommonName() != null
                        ? override.getSslCommonName()
                        : defaults.getSslCommonName());

        merged.setProtocols(
                override.getProtocols() != null
                        ? override.getProtocols()
                        : defaults.getProtocols());

        merged.setSignatureVersions(
                override.getSignatureVersions() != null
    ...
    /**
     * A convienient methods that returns true if the endpoint support HTTPS scheme. Returns false
     * otherwise.
     */
    ...
    /**
     * A convienient methods that returns true if the endpoint support HTTP scheme. Returns false
     * otherwise.
     */
