 ...
 * You can use the data to build more generic AMIs that can be modified by configuration files
 * supplied at launch time. For example, if you run web servers for various small businesses, they
 * can all use the same AMI and retrieve their content from the Amazon S3 bucket you specify at
 * launch. To add a new customer at any time, simply create a bucket for the customer, add their
 * content, and launch your AMI.<br>
 *
 * <p>You can disable the use of the EC2 Instance meta data service by either setting the {@link
 * SDKGlobalConfiguration#AWS_EC2_METADATA_DISABLED_ENV_VAR} or {@link
...
public class EC2MetadataUtils {
    ...
    /**
     * Get the local hostname of the instance. In cases where multiple network interfaces are
    ...
    /**
     * Get information about the last time the instance profile was updated, including the
     * instance's LastUpdated date, InstanceProfileArn, and InstanceProfileId.
     */
    public static IAMInfo getIAMInstanceProfileInfo() {
        ...
        try {
            ...
            log.warn(
                    "Unable to parse IAM Instance profile info (" + json + "): " + e.getMessage(),
                    e);
            return null;
        }
    }
    ...
    /**
     * The instance info is only guaranteed to be a JSON document per http://docs
     * .aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-metadata.html
     *
     * <p>This method is only a best attempt to capture the instance info as a typed object.
     *
     * <p>Get an InstanceInfo object with dynamic information about this instance.
     */
    ...
    static InstanceInfo doGetInstanceInfo(String json) {
        if (null != json) {
            try {
                ...
                log.warn(
                        "Unable to parse dynamic EC2 instance info ("
                                + json
                                + ") : "
                                + e.getMessage(),
                        e);
     ...
     * The method avoids interpreting other parts of the instance info JSON document to minimize
     * potential failure.
     *
    ...
    public static Map<String, IAMSecurityCredential> getIAMSecurityCredentials() {
        ...
        if (credentials != null) {
            for (String credential : credentials) {
                String json =
                        getData(EC2_METADATA_ROOT + "/iam/security-credentials/" + credential);
                try {
                    ...
                    log.warn(
                            "Unable to process the credential ("
                                    + credential
                                    + "). "
                                    + e.getMessage(),
                            e);
    ...
    public static String getData(String path, int tries) {
        ...
        if (null != items && items.size() > 0) return items.get(0);
        return null;
    }
    ...
    private static List<String> getItems(String path, int tries, boolean slurp) {
        if (tries == 0) throw new SdkClientException("Unable to contact EC2 metadata service.");

        List<String> items;
        try {
            String hostAddress = getHostAddressForEC2MetadataService();
            String response =
                    EC2CredentialsUtils.getInstance().readResource(new URI(hostAddress + path));
            if (slurp) items = Collections.singletonList(response);
            else items = Arrays.asList(response.split("\n"));
            return items;
        } catch (AmazonClientException ace) {
            log.warn(
                    "Unable to retrieve the requested metadata (" + path + "). " + ace.getMessage(),
                    ace);
            return null;
        } catch (Exception e) {
            ...
            int pause =
                    (int)
                            (Math.pow(2, DEFAULT_QUERY_RETRIES - tries)
                                    * MINIMUM_RETRY_WAIT_TIME_MILLISECONDS);
            try {
                Thread.sleep(
                        pause < MINIMUM_RETRY_WAIT_TIME_MILLISECONDS
                                ? MINIMUM_RETRY_WAIT_TIME_MILLISECONDS
                                : pause);
    ...
    private static String fetchData(String path, boolean force) {
        try {
            if (force || !cache.containsKey(path)) cache.put(path, getData(path));
    ...
    /**
     * The temporary security credentials (AccessKeyId, SecretAccessKey, SessionToken, and
     * Expiration) associated with the IAM role.
     */
    ...
    /**
     * This POJO is a best attempt to capture the instance info which is only guaranteed to be a
     * JSON document per http://docs.aws.amazon.com/AWSEC2/latest
     * /UserGuide/ec2-instance-metadata.html
     *
     * <p>Instance info includes dynamic information about the current instance such as region,
    ...
    public static class InstanceInfo {
        ...
        public InstanceInfo(
                ...
                @JsonProperty(value = "devpayProductCodes", required = false)
            ...
            this.devpayProductCodes =
                    devpayProductCodes == null ? null : devpayProductCodes.clone();
        }
    ...
    public static class NetworkInterface {
         ...
         * In multiple-interface environments, an interface can be attached by a third party, such
         * as Elastic Load Balancing. Traffic on an interface is always billed to the interface
         * owner.
         */
        ...
        /**
         * Get the private IPv4 address(es) that are associated with the public-ip address and
        ...
        public List<String> getIPv4Association(String publicIp) {
            return EC2MetadataUtils.getItems(
                    EC2_METADATA_ROOT + path + "ipv4-associations/" + publicIp);
        }

        private String getData(String key) {
            if (data.containsKey(key)) return data.get(key);

            // Since the keys are variable, cache a list of which ones are available to prevent
            // unnecessary trips to the service.
