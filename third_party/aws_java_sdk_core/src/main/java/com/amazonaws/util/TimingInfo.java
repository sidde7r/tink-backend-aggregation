 ...
 *
 * <p>In contrast to {@link TimingInfoFullSupport}, which is intended to be a full support of the
 * timing info, this class only provides a minimal support of start and end time (ie with no-ops for
 * sub-event measurements) for backward compatiblity reasons.
 *
 * <p>This class is instantiated instead of {@link TimingInfoFullSupport} when request metric
...
public class TimingInfo {
    ...
    /**
     * The wall clock time (as the number of milliseconds since Epoch) of when the timing
     * measurement starts; or null if unknown. This field is not meant to be used for timing
     * measurement. For more info, see:
     * https://blogs.oracle.com/dholmes/entry/inside_the_hotspot_vm_clocks
     */
    private final Long startEpochTimeMilli;
    /**
     * Start time in nanosecond used for timing measurement. Note the value in this field may have
     * nothing to do with the wall clock time. The wall clock time of when the timing measurement
     * starts can optionally be captured in {@link #startEpochTimeMilli}. For more info, see:
     * https://blogs.oracle.com/dholmes/entry/inside_the_hotspot_vm_clocks
     *
     * <p>Note System.nanoTime() can return negative values.
     */
    private final long startTimeNano;
    /**
     * End time in nanosecond used for timing measurement or null if unknown. Note the value in this
     * field is only meant to be used for timing measurement, and is not directly related to the
     * wall clock time. For more info, see:
     * https://blogs.oracle.com/dholmes/entry/inside_the_hotspot_vm_clocks
     *
     * <p>Note System.nanoTime() can return negative values.
     */
    ...
    /**
     * Captures the current wall clock time (since epoch in millisecond) and the current time (in
     * nanosecond) used for timing measurement. For more info, see:
     * https://blogs.oracle.com/dholmes/entry/inside_the_hotspot_vm_clocks
     */
    ...
    public static TimingInfo startTimingFullSupport() {
        return new TimingInfoFullSupport(
    ...
    /**
     * Returns a {@link TimingInfoFullSupport} based on the given start and end time in nanosecond,
    ...
    /**
     * Returns a {@link TimingInfoFullSupport} based on the given start time since epoch in
     * millisecond, and the given start and end time in nanosecond.
     *
    ...
    /**
     * Returns an instance of {@link TimingInfo} that is not modifiable, given the start and end
     * nano times.
     */
     ...
     */
    public static TimingInfo unmodifiableTimingInfo(
            long startEpochTimeMilli, long startTimeNano, Long endTimeNano) {
    ...
    public final Double getTimeTakenMillisIfKnown() {
        return isEndTimeKnown() ? durationMilliOf(startTimeNano, endTimeNano) : null;
    }

    /**
     * Returns the duration in milliseconds as double, preserving the decimal precision as
     * necessary, for the given start and end time in nanoseconds.
     */
    public static double durationMilliOf(long startTimeNano, long endTimeNano) {
        ...
        return micros / 1000.0; // convert microseconds to milliseconds in double rather than long,
