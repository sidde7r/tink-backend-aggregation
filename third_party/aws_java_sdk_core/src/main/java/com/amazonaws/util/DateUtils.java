...
public class DateUtils {
    ...
    protected static final DateTimeFormatter compressedIso8601DateFormat =
            DateTimeFormat.forPattern("yyyyMMdd'T'HHmmss'Z'").withZone(GMT);
     ...
     *
     * <p>See https://github.com/aws/aws-sdk-java/issues/233
     */
    ...
    /**
     * Returns the original runtime exception iff the joda-time being used at runtime behaves as
     * expected.
     *
     * @throws IllegalStateException if the joda-time being used at runtime doens't appear to be of
     *     the right version.
     */
    private static <E extends RuntimeException> E handleException(E ex) {
        if (JodaTime.hasExpectedBehavior()) return ex;
    ...
    /**
     * Parses the specified date string as a compressedIso8601DateFormat ("yyyyMMdd'T'HHmmss'Z'")
    ...
    public static Date parseServiceSpecificDate(String dateString) {
        ...
        try {
            ...
            return new Date(
                    dateValue.scaleByPowerOfTen(AWS_DATE_MILLI_SECOND_PRECISION).longValue());
    ...
    /**
     * Returns the number of days since epoch with respect to the given number of milliseconds since
     * epoch.
     */
