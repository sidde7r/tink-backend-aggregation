 ...
 * A Base 64 codec API.
 *
 * <p>See http://www.ietf.org/rfc/rfc4648.txt
 *
 * @author Hanson Char
 */
public enum Base64 {
        ...
        if (isJaxbAvailable) {
            ...
            try {
                ...
                if (inconsistentJaxbImpls.containsKey(className)) {
                    LOG.warn(
                            "A JAXB implementation known to produce base64 encodings that are "
    ...
    public static String encodeAsString(byte... bytes) {
        ...
        if (isJaxbAvailable) {
            try {
                ...
                // Note the converter should eventually be initialized and printBase64Binary should
                // start working again
                LOG.debug(
                        "Recovering from JAXB bug: https://netbeans.org/bugzilla/show_bug.cgi?id=224923",
                        ex);
            }
        }
