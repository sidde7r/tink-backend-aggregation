...
public enum Throwables {
    ;
    /**
     * Returns the root cause of the given throwable, or null if the given throwable is null. If the
     * root cause is over 1000 level deep, the original throwable will be returned defensively as
     * this is heuristically considered a circular reference, however unlikely.
     */
    public static Throwable getRootCause(Throwable orig) {
        ...
        LogFactory.getLog(Throwables.class)
                .debug(
                        "Possible circular reference detected on "
                                + orig.getClass()
                                + ": ["
                                + orig
                                + "]");
    ...
    /**
     * Same as {@link #failure(Throwable)}, but the given errmsg will be used if it was wrapped as
     * either an {@link AmazonClientException} or {@link AbortedException}.
     */
