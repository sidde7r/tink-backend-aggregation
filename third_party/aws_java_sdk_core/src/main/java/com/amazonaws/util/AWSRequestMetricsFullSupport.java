...
/**
 * In contrast to {@link AWSRequestMetrics}, which is intended to be a minimal support of AWS SDK
 * request metrics, this class is the full support of AWS SDK request metrics including features
 * such as properties and sub-events.
 *
 * <p>This class is instantiated instead of {@link AWSRequestMetrics} when request metric collection
 * is required during a particular service request/response cycle.
 */
@NotThreadSafe
public class AWSRequestMetricsFullSupport extends AWSRequestMetrics {
     ...
     * after endEvent is called. For every startEvent there should be a corresponding endEvent. If
     * you start the same event without ending it, this will overwrite the old event. i.e. There is
     * no support for recursive events yet. Having said that, if you start and end an event in that
     * sequence multiple times, all events are logged in timingInfo in that order.
     *
     * <p>This feature is enabled if the system property "com.amazonaws.sdk.enableRuntimeProfiling"
     * is set, or if a {@link RequestMetricCollector} is in use either at the request, web service
    ...
    public void startEvent(String eventName) {
        ...
        eventsBeingProfiled.put(
                eventName,
                TimingInfo.startTimingFullSupport(System.currentTimeMillis(), System.nanoTime()));
    }
    ...
    /**
     * End an event which was previously started. Once ended, log how much time the event took. It
     * is illegal to end an Event that was not started. It is good practice to endEvent in a finally
    ...
    /**
     * Add 1 to an existing count for a given event. If the count for that event does not exist,
     * then it creates one and initializes it to 1.
     *
     * <p>This feature is enabled if the system property "com.amazonaws.sdk.enableRuntimeProfiling"
     * is set, or if a {@link RequestMetricCollector} is in use either at the request, web service
     * client, or AWS SDK level.
    ...
    public void log() {
        if (latencyLogger.isDebugEnabled()) {
            ...
            for (Entry<String, List<TimingInfo>> entry :
                    timingInfo.getSubMeasurementsByName().entrySet()) {
                keyValueFormat(entry.getKey(), entry.getValue(), builder);
            }
