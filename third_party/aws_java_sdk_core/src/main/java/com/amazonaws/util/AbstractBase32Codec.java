...
abstract class AbstractBase32Codec implements Codec {
    ...
    public final byte[] encode(byte[] src) {
        ...
        if (remainder == 0) {
            ...
            for (int s = 0, d = 0; s < src.length; s += 5, d += 8) encode5bytes(src, s, dest, d);
            return dest;
        }
        ...
        for (; s < src.length - remainder; s += 5, d += 8) encode5bytes(src, s, dest, d);
    ...
    private final void encode5bytes(byte[] src, int s, byte[] dest, int d) {
        ...
        dest[d++] =
                (byte) alphabets[(p & MASK_3BITS) << 2 | (p = src[s++]) >>> 6 & MASK_2BITS]; // 3 2
    ...
    private final void decode5bytes(byte[] src, int s, byte[] dest, int d) {
        int p = 0;
        // operator precedence in descending order: >>> or <<, &, |
        dest[d++] = (byte) (pos(src[s++]) << 3 | (p = pos(src[s++])) >>> 2 & MASK_3BITS); // 5 3
        dest[d++] =
                (byte)
                        ((p & MASK_2BITS) << 6
                                | pos(src[s++]) << 1
                                | (p = pos(src[s++])) >>> 4 & 1); //   2 5 1
        dest[d++] =
                (byte)
                        ((p & MASK_4BITS) << 4
                                | (p = pos(src[s++])) >>> 1 & MASK_4BITS); //       4 4
        dest[d++] =
                (byte)
                        ((p & 1) << 7
                                | pos(src[s++]) << 2
                                | (p = pos(src[s++])) >>> 3 & MASK_2BITS); //         1 5 2
        dest[d] = (byte) ((p & MASK_3BITS) << 5 | pos(src[s])); //             3 5
        return;
    }
    ...
    public final byte[] decode(byte[] src, final int length) {
        ...
        for (; d < dest.length - fq % 5; s += 8, d += 5) decode5bytes(src, s, dest, d);

        if (fq < 5) decode1to4bytes(fq, src, s, dest, d);
        return dest;
    }
