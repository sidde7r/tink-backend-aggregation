...
public class BinaryUtils {
     ...
     *
     * <p>The internal states of the given byte buffer will be restored when this method completes
     * execution.
     *
     * <p>When handling <code>ByteBuffer</code> from user's input, it's typical to call the {@link
     * #copyBytesFrom(ByteBuffer)} instead of {@link #copyAllBytesFrom(ByteBuffer)} so as to account
     * for the position of the input <code>ByteBuffer</code>. The opposite is typically true,
     * however, when handling <code>ByteBuffer</code> from withint the unmarshallers of the
     * low-level clients.
     */
    ...
    public static byte[] copyBytesFrom(ByteBuffer bb) {
        ...
        if (bb.hasArray()) {
            return Arrays.copyOfRange(
                    bb.array(), bb.arrayOffset() + bb.position(), bb.arrayOffset() + bb.limit());
        }
