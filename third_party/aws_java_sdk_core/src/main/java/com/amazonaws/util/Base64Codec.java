...
class Base64Codec implements Codec {
    ...
    private static class LazyHolder {
        ...
        private static byte[] decodeTable() {
            ...
            for (int i = 0; i <= 'z'; i++) {
                if (i >= 'A' && i <= 'Z') dest[i] = (byte) (i - 'A');
                else if (i >= '0' && i <= '9') dest[i] = (byte) (i - OFFSET_OF_0);
                else if (i == '+') dest[i] = (byte) (i - OFFSET_OF_PLUS);
                else if (i == '/') dest[i] = (byte) (i - OFFSET_OF_SLASH);
                else if (i >= 'a' && i <= 'z') dest[i] = (byte) (i - OFFSET_OF_a);
                else dest[i] = -1;
            }
            return dest;
        }
    ...
    public byte[] encode(byte[] src) {
        ...
        if (remainder == 0) {
            ...
            for (int s = 0, d = 0; s < src.length; s += 3, d += 4) encode3bytes(src, s, dest, d);
            return dest;
        }
        ...
        for (; s < src.length - remainder; s += 3, d += 4) encode3bytes(src, s, dest, d);
    ...
    void decode4bytes(byte[] src, int s, byte[] dest, int d) {
        int p = 0;
        // operator precedence in descending order: >>> or <<, &, |
        dest[d++] = (byte) (pos(src[s++]) << 2 | (p = pos(src[s++])) >>> 4 & MASK_2BITS); // 6 2
        dest[d++] =
    ...
    public byte[] decode(byte[] src, final int length) {
        ...
        for (; d < dest.length - fq % 3; s += 4, d += 3) decode4bytes(src, s, dest, d);

        if (fq < 3) decode1to3bytes(fq, src, s, dest, d);
        return dest;
    }
