...
/**
 * Can be used to configure the default log factory for the AWSJavaClientCore and
 * AWSJavaClientSigners. Default to JUL, unless AWSJavaClientRuntime is present which will default
 * it to Jakarta Commons Logging.
 */
@ThreadSafe
public abstract class InternalLogFactory {
    ...
    /**
     * Returns an SDK logger that logs using the currently configured default log factory, given the
     * class.
     */
    ...
    /**
     * Used to explicitly configure the log factory. The log factory can only be configured at most
     * once. All subsequent configurations will have no effect.
     *
     * <p>Note explicitly configuring the log factory will have positive performance impact on all
     * subsequent logging, since the specific logger can be directly referenced instead of being
     * searched every time.
     *
     * @param factory the log factory to be used internally by the SDK
     * @return true if the log factory is successfully configured; false otherwise (ie the log
     *     factory is not allowed to be configured more than once for performance reasons.)
     */
    public static synchronized boolean configureFactory(InternalLogFactory factory) {
        ...
        if (factoryConfigured) return false;
