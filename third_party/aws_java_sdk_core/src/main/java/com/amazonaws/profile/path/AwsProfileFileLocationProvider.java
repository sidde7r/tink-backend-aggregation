...
public interface AwsProfileFileLocationProvider {
    ...
    /**
     * Location provider for the shared AWS credentials file. Checks the environment variable
     * override first, then checks the default location (~/.aws/credentials), and finally falls back
     * to the legacy config file (~/.aws/config) that we still support loading credentials from.
     */
    AwsProfileFileLocationProvider DEFAULT_CREDENTIALS_LOCATION_PROVIDER =
            new AwsProfileFileLocationProviderChain(
                    new CredentialsEnvVarOverrideLocationProvider(),
                    new CredentialsDefaultLocationProvider(),
                    new CredentialsLegacyConfigLocationProvider());

    /**
     * Location provider for the shared AWS Config file. Checks environment variable override first
     * then falls back to the default location (~/.aws/config) if not present.
     */
    AwsProfileFileLocationProvider DEFAULT_CONFIG_LOCATION_PROVIDER =
            new AwsProfileFileLocationProviderChain(
                    new ConfigEnvVarOverrideLocationProvider(),
                    new SharedConfigDefaultLocationProvider());
