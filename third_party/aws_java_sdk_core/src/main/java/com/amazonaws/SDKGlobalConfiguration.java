...
public class SDKGlobalConfiguration {
    ...
    /**
     * System property used when starting up the JVM to enable the default metrics collected by the
     * AWS SDK, which uploads the derived statistics to Amazon CloudWatch.
     *
     * <pre>
     * Example:
    ...
    /**
     * System property for the AWS region to use when creating clients. See {@link
    ...
    /**
     * Path to an override file for the region metadata loaded by the SDK that maps service/region
     * pairs to endpoints and vice versa.
     */
    public static final String REGIONS_FILE_OVERRIDE_SYSTEM_PROPERTY =
            "com.amazonaws.regions.RegionUtils.fileOverride";

    /**
     * By default, the SDK will attempt to download an up-to-date set of region metadata from Amazon
     * CloudFront when first required. This allows you to look up information about new regions and
     * services without having to download a new version of the SDK every time.
     *
     * <p>If the SDK cannot download region metadata from Amazon CloudFront, it will fall back to
     * loading the region metadata bundled with the SDK when it was released, which may be out of
     * date.
     *
     * <p>Setting this system property to anything other than {@code null} will <i>disable</i> this
     * remote fetching, and will <i>only</i> load region metadata that was bundled with the SDK.
     * This gives you control over when new region metadata will be picked up by your application,
     * and isolates you from potential issues if your application depends on being able to load
     * metadata about a region which is not included in your version of the SDK and therefore may
     * not be available in the event of network outages.
     */
    public static final String DISABLE_REMOTE_REGIONS_FILE_SYSTEM_PROPERTY =
            "com.amazonaws.regions.RegionUtils.disableRemote";

    /**
     * By default, the AmazonS3Client will continue to use the legacy S3Signer to authenticate
     * requests it makes to S3 in regions that support the older protocol. Setting this property to
     * anything other than null will cause the client to upgrade to Signature Version 4 whenever it
     * has been configured with an explicit region (which is a required parameter for Signature
     * Version 4). The client will continue to use the older signature protocol when not configured
     * with a region to avoid breaking existing applications.
     *
     * <p>Signature Version 4 is more secure than the legacy S3Signer, but requires calculating a
     * SHA-256 hash of the entire request body which can be expensive for large upload requests.
     */
    @Deprecated
    ...
    /**
     * Like {@link #ENABLE_S3_SIGV4_SYSTEM_PROPERTY}, but causes the client to always use Signature
     * Version 4, assuming a region of &quot;us-east-1&quot; if no explicit region has been
     * configured. This guarantees that the more secure authentication protocol will be used, but
     * will cause authentication failures in code that accesses buckets in regions other than US
     * Standard without explicitly configuring a region.
     */
    @Deprecated
     ...
     *
     * <p>If the client is not configured with an explicit region, bucket operations (eg. {@code
     * listObjects}) will attempt to determine the bucket's region the first time that bucket is
     * seen. This may result in a cross-region call.
     *
     * <p>Setting this system property to anything other than {@code null} or {@code false} will
     * <i>disable</i> this implicit fixed-region invocation, and any attempt to use a regionless
     * client will fail with an IllegalStateException. This allows SDK users to force all S3 clients
     * to be created with a region defined, so that calls to unintended regions are avoided.
     *
     * <p>Note: Even with this option enabled, SDK users can still create clients not attached to
     * fixed regions using specific opt-in methods on the S3 client builder. For instance, the
     * {@code enableGlobalBucketAccess} option explicitly allows creation of a client that will
     * automatically identify the location of a bucket via a call to a single, fixed region.
     */
    ...
    /**
     * Overrides the client default {@link ClientConfiguration} to use configuration with values
     * tailored towards clients operating in the same AWS region as the service endpoint they call.
     * Timeouts in in-region optimized configurations are generally set much lower than the client
    ...
    /**
     * @deprecated with {@link AmazonWebServiceRequest#getRequestClientOptions()} and {@link
     *     RequestClientOptions#setReadLimit(int)}.
     *     <p>The default size of the buffer when uploading data from a stream. A buffer of this
     *     size will be created and filled with the first bytes from a stream being uploaded so that
     *     any transmit errors that occur in that section of the data can be automatically retried
     *     without the caller's intervention.
     *     <p>If not set, the default value of 128 KB will be used.
     */
    @Deprecated
     ...
     *     <p>Internal system property to enable timing info collection.
     */
    @Deprecated
    ...
    /**
     * Environment variable to disable CBOR protocol. This forces the request to be sent over the
     * wire as a AWS JSON.
     */
    public static final String AWS_CBOR_DISABLE_ENV_VAR = "AWS_CBOR_DISABLE";

    /**
     * System property to disable CBOR protocol. This forces the request to be sent over the wire as
     * a AWS JSON
     */
    public static final String AWS_CBOR_DISABLE_SYSTEM_PROPERTY = "com.amazonaws.sdk.disableCbor";

    /**
     * Environment variable to disable Ion binary protocol. This forces the request to be sent over
     * the wire as Ion text.
     */
    public static final String AWS_ION_BINARY_DISABLE_ENV_VAR = "AWS_ION_BINARY_DISABLE";

    /**
     * System property to disable Ion binary protocol. This forces the request to be sent over the
     * wire as Ion text
     */
    ...
    /**
     * Environment variable to disable loading credentials or regions from EC2 Metadata instance
     * service.
     */
