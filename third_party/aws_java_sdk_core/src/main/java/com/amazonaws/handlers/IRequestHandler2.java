...
public interface IRequestHandler2 {
     ...
     *
     * <p>Warning: Modifications to this message will leak out to the user, who might reuse the
     * request object without realizing that it was modified as part of sending it the first time.
     * For this reason, we recommend that you only modify the request in the {@link
     * #beforeRequest(Request)} method. If you require data that is not available in that method,
     * you should create a {@link AmazonWebServiceRequest#clone()} of the request (which performs a
     * deep copy of the handler context map), add any context data you require in the {@link
     * #beforeRequest(Request)} method using {@link
     * AmazonWebServiceRequest#addHandlerContext(HandlerContextKey, Object)}, and return your cloned
     * message from this method. You can then extract that data for use in the request using {@link
     * Request#getHandlerContext(HandlerContextKey)}.
     *
     ...
     *
     * @param context container for callback-related data; includes the request
     */
     ...
     *
     * @param request The low level request being processed.
     * @param httpResponse The Raw HTTP response before being unmarshalled
     ...
     * invoked whether or not the attempt resulted in a successful response or an error. This
     * callback is invoked within a finally block and so any exceptions it generates will "replace"
     * the current exception, if one is outstanding.
     *
     * @param context container for the request as well as all possible results of the attempt
     */
    ...
    /**
     * Runs any additional processing logic on a request after it has failed. This callback is
     * invoked from a catch block. If it generates an exception, the original AmazonClientException
     * will be lost.
     *
     * @param request The request that generated an error.
     * @param response the response or null if the failure occurred before the response is made
     *     available
