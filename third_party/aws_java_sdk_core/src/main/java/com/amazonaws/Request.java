...
/**
 * Represents a request being sent to an Amazon Web Service, including the parameters being sent as
 * part of the request, the endpoint to which the request should be sent, etc.
 *
 * <p>This class is only intended for use inside the AWS client libraries and request handlers.
 * Users of the AWS SDK for Java should not implement this interface.
 *
...
public interface Request<T> extends SignableRequest<T>, HandlerContextAware {
     ...
     *
     * <p>Note that List values within the parameters Map must use an implementation that supports
     * null values.
     */
    ...
    /**
     * Sets the service endpoint (ex: "https://ec2.amazonaws.com") to which this request should be
     * sent.
     *
    ...
    /**
     * @return The name of the Amazon service this request is for. This is used as the service name
     *     set in request metrics and service exceptions. This is NOT the same as the service name
     *     returned by {@link AmazonWebServiceClient#getServiceName()} which is used for signing
     *     purpose.
    ...
    /**
     * Sets the optional value for time offset for this request. This will be used by the signer to
     * adjust for potential clock skew. Value is in seconds, positive values imply the current clock
     * is "fast", negative values imply clock is slow.
     *
    ...
    /**
     * Bind the request metrics to the request. Note metrics can be captured before the request is
     * created.
     *
