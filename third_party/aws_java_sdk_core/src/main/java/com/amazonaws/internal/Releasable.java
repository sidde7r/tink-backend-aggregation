 ...
 * same code block that created it, then it is necessary that the release method must not be called
 ...
 * the stream, the creator of the stream would need to explicitly disable the accidental closing via
 * <code>ResettableInputStream#disableClose()</code>, so that the release method becomes the only
 * way to truly close the opened file.
 */
public interface Releasable {
    /**
     * Releases the allocated resource. This method should not be called except by the caller who
     * allocated the resource at the very top of the call stack. This allows, typically, a {@link
     * Closeable} resource to be not unintentionally released owing to the calling of the {@link
