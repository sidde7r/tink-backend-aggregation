 ...
 *
 * <p>In particular, a {@link ResettableInputStream} allows the close operation to be disabled via
 ...
 * closed.
 *
 * <p>The creator of this input stream should therefore always call {@link #release()} in a finally
...
public class ResettableInputStream extends ReleasableInputStream {
     ...
     *     <p>Note the creation of a {@link ResettableInputStream} would entail physically opening a
     *     file. If the opened file is meant to be closed only (in a finally block) by the very same
     *     code block that created it, then it is necessary that the release method must not be
     *     called while the execution is made in other stack frames.
     *     <p>In such case, as other stack frames may inadvertently or indirectly call the close
     *     method of the stream, the creator of the stream would need to explicitly disable the
     *     accidental closing via {@link ResettableInputStream#disableClose()}, so that the release
     *     method becomes the only way to truly close the opened file.
     */
     ...
     * <p>Note the creation of a {@link ResettableInputStream} would entail physically opening a
     * file. If the opened file is meant to be closed only (in a finally block) by the very same
     * code block that created it, then it is necessary that the release method must not be called
     * while the execution is made in other stack frames.
     *
     ...
     * closing via {@link ResettableInputStream#disableClose()}, so that the release method becomes
     * the only way to truly close the opened file.
     *
     * @param _ ignored
     */
    @Override
    public void mark(int ignored) {
        ...
        if (log.isTraceEnabled()) log.trace("File input stream marked at position " + markPos);
    }
     ...
     * converting any IOException into SdkClientException.
     *
     * <p>Note the creation of a {@link ResettableInputStream} would entail physically opening a
     * file. If the opened file is meant to be closed only (in a finally block) by the very same
     * code block that created it, then it is necessary that the release method must not be called
    ...
    /**
     * Convenient factory method to construct a new resettable input stream for the given file,
     * converting any IOException into SdkClientException with the given error message.
     *
     * <p>Note the creation of a {@link ResettableInputStream} would entail physically opening a
     * file. If the opened file is meant to be closed only (in a finally block) by the very same
     * code block that created it, then it is necessary that the release method must not be called
     ...
     * stream, converting any IOException into SdkClientException.
     *
     * <p>Note the creation of a {@link ResettableInputStream} would entail physically opening a
     * file. If the opened file is meant to be closed only (in a finally block) by the very same
     * code block that created it, then it is necessary that the release method must not be called
    ...
    /**
     * Convenient factory method to construct a new resettable input stream for the given file input
     * stream, converting any IOException into SdkClientException with the given error message.
     *
     * <p>Note the creation of a {@link ResettableInputStream} would entail physically opening a
     * file. If the opened file is meant to be closed only (in a finally block) by the very same
     * code block that created it, then it is necessary that the release method must not be called
