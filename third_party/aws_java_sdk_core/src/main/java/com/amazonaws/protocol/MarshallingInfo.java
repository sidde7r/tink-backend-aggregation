...
/**
 * Metadata about the marshalling requirements of a given member. Includes things like where to put
 * the data (payload, query, etc), the location name it's bound to (JSON field name, query param
 * name, etc) and additional directives about how to transform it (i.e. is it binary data, should it
 * be auto generated, etc).
 */
@SdkProtectedApi
public class MarshallingInfo<T> {
    ...
    /**
     * @return Marshall location name. This may represent different things depending on where the
     *     data is meant to be marshalled to. For data bound to the payload, this may be the JSON
     *     field name, for data bound to the query params this will be the parameter name, for data
     *     bound to the path this will be the value of the path placeholder to replace in the
    ...
    /**
     * @return True if the member is explicitly bound to the payload. Applicable only for RESTFUL
     *     services.
     */
    ...
    /**
     * @return True if the payload member is binary data (i.e. a {@link java.nio.ByteBuffer} or
     *     {@link java.io.InputStream} and should be sent as the request content. This is not true
     *     when binary data is bound to a member of the JSON payload since it will be base64 encoded
     *     as a string.
     */
