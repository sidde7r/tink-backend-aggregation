...
public class SimpleTypeJsonMarshallers {
    ...
    public static final JsonMarshaller<Void> NULL =
            new JsonMarshaller<Void>() {
                ...
                public void marshall(Void val, JsonMarshallerContext context, String paramName) {
                    ...
                    // we just don't write the field. If param name is null then we are either in a
                    // container
                    // or the thing being marshalled is the payload itself in which case we want to
                    // preserve
                    // the JSON null.
                    if (paramName == null) {
    ...
    public static final JsonMarshaller<Integer> INTEGER =
            new BaseJsonMarshaller<Integer>() {
                @Override
                public void marshall(
                        Integer val,
                        StructuredJsonGenerator jsonGenerator,
    ...
    public static final JsonMarshaller<BigDecimal> BIG_DECIMAL =
            new BaseJsonMarshaller<BigDecimal>() {
                @Override
                public void marshall(
                        BigDecimal val,
                        StructuredJsonGenerator jsonGenerator,
    ...
    public static final JsonMarshaller<Boolean> BOOLEAN =
            new BaseJsonMarshaller<Boolean>() {
                @Override
                public void marshall(
                        Boolean val,
                        StructuredJsonGenerator jsonGenerator,
    ...
    public static final JsonMarshaller<ByteBuffer> BYTE_BUFFER =
            new BaseJsonMarshaller<ByteBuffer>() {
                @Override
                public void marshall(
                        ByteBuffer val,
                        StructuredJsonGenerator jsonGenerator,
    ...
    public static final JsonMarshaller<StructuredPojo> STRUCTURED =
            new BaseJsonMarshaller<StructuredPojo>() {
                @Override
                public void marshall(
                        StructuredPojo val,
                        StructuredJsonGenerator jsonGenerator,
    ...
    public static final JsonMarshaller<Map> MAP =
            new BaseJsonMarshaller<Map>() {
                ...
                protected boolean shouldEmit(Map map) {
                    return !(map.isEmpty()
                            && map instanceof SdkInternalMap
                            && ((SdkInternalMap) map).isAutoConstruct());
                }
            };

    /**
     * Adapt a {@link StructuredJsonMarshaller} to a {@link JsonMarshaller}. {@link JsonMarshaller}
     * has a lot of internal stuff so we don't want to expose all that across module boundaries.
     *
    ...
    public static <T> JsonMarshaller<T> adapt(final StructuredJsonMarshaller<T> toAdapt) {
        return new BaseJsonMarshaller<T>() {
            @Override
            public void marshall(
                    T val, StructuredJsonGenerator jsonGenerator, JsonMarshallerContext context) {
    ...
    /**
     * Base marshaller that emits the field name if present. The field name may be null in cases
     * like marshalling something inside a list or if the object is the explicit payload member.
     *
    ...
    private abstract static class BaseJsonMarshaller<T> implements JsonMarshaller<T> {
        ...
        /**
         * Hook to prevent emitting the field name. Used for maps and lists since we treat empty
         * auto constructed containers as null.
         */
