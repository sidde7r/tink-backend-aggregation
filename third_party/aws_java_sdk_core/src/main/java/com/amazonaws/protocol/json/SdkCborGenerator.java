...
class SdkCborGenerator extends SdkJsonGenerator {
     ...
     * (https://tools.ietf.org/html/rfc7049#section-2.4.1) we will need to write a tag and write the
     * epoch.
     */
    @Override
    public StructuredJsonGenerator writeValue(Date date) {
        ...
        if (!(getGenerator() instanceof CBORGenerator)) {
            throw new IllegalStateException(
                    "SdkCborGenerator is not created with a CBORGenerator.");
        }
        ...
        try {
            ...
            throw new RuntimeException(
                    "Jackson jackson-core/jackson-dataformat-cbor incompatible library version detected.\nYou have two possible resolutions:\n"
                            ...
                            + "\t\t2) Disable CBOR wire-protocol by passing the -D"
                            + AWS_CBOR_DISABLE_SYSTEM_PROPERTY
                            + " property or setting the "
                            + AWS_CBOR_DISABLE_ENV_VAR
                            + " environment variable (warning this may affect performance)",
                    e);
        }
    }
}
