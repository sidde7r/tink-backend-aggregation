...
/**
 * A progress event. Typically this is used to notify a chunk of bytes has been transferred. Also
 * used to notify other types of progress events such as a transfer starting, or failing.
 *
 * <p>The legacy Amazon S3 progress event com.amazonaws.services.s3.model.ProgressEvent has been
...
public class ProgressEvent {
    ...
    public ProgressEvent(ProgressEventType eventType, long bytes) {
        ...
        if (bytes < 0) throw new IllegalArgumentException("bytes reported must be non-negative");
        this.eventType = eventType;
        this.bytes = bytes;
    }

    /**
     * Returns the number of bytes associated with the event. The number of bytes are not
     * necessarily the same as the number of bytes transferred, and it's meaning depends on the
     * specific event type. For example, the bytes of a {@link
     * ProgressEventType#REQUEST_CONTENT_LENGTH_EVENT} refers to the expected number of bytes to be
     * sent to AWS, not the actual number of bytes that have been transferred.
     */
    ...
    /**
     * Convenient method to returns the number of bytes transferred in this event, or the number of
     * bytes reset (or discarded) if negative. In particular, bytes of a content-length event is
     * excluded.
     */
     ...
     *
     * @return The unique event code that identifies what type of specific type of event this object
     *     represents.
    ...
    static {
        ...
        legacyEventCodes.put(
                ProgressEventType.TRANSFER_PART_STARTED_EVENT, PART_STARTED_EVENT_CODE);
        legacyEventCodes.put(
                ProgressEventType.TRANSFER_PART_COMPLETED_EVENT, PART_COMPLETED_EVENT_CODE);
