...
/**
 * An implementation of ProgressListener interface that delegates progressChanged callback to
 * multiple listeners. It also takes an optional ProgressEventFilter to filter incoming events
 * before passing them to the listeners.
 *
 * <p>This class could be used for both Amazon S3 and Amazon Glacier clients. The legacy Amazon S3
 * progress listener chain com.amazonaws.services.s3.transfer.internal.ProgressListenerChain has
 * been deprecated in favor of this new class.
 */
public class ProgressListenerChain implements ProgressListener, DeliveryMode {
    ...
    /**
     * True if synchronous callback to every listener in this chain is safe with no risk of
     * incurring undue latency.
     *
    ...
    public ProgressListenerChain(
            ProgressEventFilter progressEventFilter, ProgressListener... listeners) {
        if (listeners == null) {
    ...
    public synchronized void addProgressListener(ProgressListener listener) {
        ...
        if (syncCallSafe) syncCallSafe = DeliveryMode.Check.isSyncCallSafe(listener);
