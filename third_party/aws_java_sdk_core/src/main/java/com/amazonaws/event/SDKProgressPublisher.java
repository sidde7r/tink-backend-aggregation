...
/**
 * This class is responsible for executing the callback method of ProgressListener; listener
 * callbacks are executed sequentially in a separate single thread.
 */
public class SDKProgressPublisher {
     ...
     *
     * @return the future of a submitted task; or null if the delivery is synchronous with no future
     *     task involved. Note a listener should never block, and therefore returning null is the
     *     typical case.
     */
    public static Future<?> publishProgress(
            final ProgressListener listener, final ProgressEventType type) {
    ...
    private static Future<?> deliverEvent(
            final ProgressListener listener, final ProgressEvent event) {
    ...
    public static Future<?> publishRequestContentLength(
            final ProgressListener listener, final long bytes) {
    ...
    private static Future<?> publishByteCountEvent(
            final ProgressListener listener, final ProgressEventType type, final long bytes) {
        if (listener == ProgressListener.NOOP || listener == null || bytes <= 0) return null;
    ...
    public static Future<?> publishRequestReset(
            final ProgressListener listener, final long bytesReset) {
    ...
    public static Future<?> publishResponseBytesDiscarded(
            final ProgressListener listener, final long bytesDiscarded) {
        return publishResetEvent(
                listener, ProgressEventType.RESPONSE_BYTE_DISCARD_EVENT, bytesDiscarded);
    }

    /**
     * @param listener must not be null or else the publication will be skipped
     * @param bytesReset the publication will be skipped unless the number of bytes reset is
     *     positive
     */
    private static Future<?> publishResetEvent(
        ...
        if (bytesReset <= 0) return null;
        if (listener == ProgressListener.NOOP || listener == null) return null;
    ...
    /**
     * For internal testing and backward compatibility only. This method blocks until all the
     * submitted callbacks are executed. Listeners should never block so this method should never be
     * used.
     */
    @Deprecated
    ...
    private static final class LazyHolder {
        ...
        /** Creates a new single threaded executor service for performing the callbacks. */
     ...
     *
     * <p>However, the recommended best practice is to always make use of progress listeners that
     * are short-lived (ie do not block) and are subclasses of either {@link SyncProgressListener}
     * or <code>S3SyncProgressListener</code>. That way, the progress publisher (legacy) thread will
     * never be activated in the first place.
     *
     * @param now true if shutdown now; false otherwise.
     */
    public static void shutdown(boolean now) {
        if (now) LazyHolder.executor.shutdownNow();
        else LazyHolder.executor.shutdown();
    }
}
