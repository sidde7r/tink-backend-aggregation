...
/**
 * Extension of SdkClientException that represents an error response returned by an Amazon web
 * service. Receiving an exception of this type indicates that the caller's request was correctly
 ...
 *
 * <p>AmazonServiceException provides callers several pieces of information that can be used to
 * obtain more information about the error and why it occurred. In particular, the errorType field
 * can be used to determine if the caller's request was invalid, or the service encountered an error
 * on the server side while processing it.
 */
public class AmazonServiceException extends SdkClientException {
     ...
     *
     * <p>For example, if a client is using an invalid AWS access key, the returned exception will
     ...
     * in the 4xx range.
     *
     * <p>The Service ErrorType indicates that although the request the caller sent was valid, the
     ...
     *
     * <p>Finally, if there isn't enough information to determine who's fault the error response is,
     * an Unknown ErrorType will be set.
     */
    ...
    /**
     * Constructs a new AmazonServiceException with the specified message and exception indicating
     * the root cause.
     *
     * @param errorMessage An error message describing what went wrong.
     ...
     *
     * <p>NOTE: errorMessage by default is set to the same as the message value passed to the
     * constructor of AmazonServiceException.
     *
    ...
    public void setRawResponseContent(String rawResponseContent) {
        this.rawResponse =
                rawResponseContent == null ? null : rawResponseContent.getBytes(StringUtils.UTF8);
    }
