...
public class RegionUtils {
    ...
    /**
     * Returns the current set of region metadata for this process, initializing it if it has not
     * yet been explicitly initialized before.
     *
    ...
    /**
     * Initializes the region metadata by loading from the default hierarchy of region metadata
     * locations.
     */
     ...
     *
     * @param serviceAbbreviation The service endpoint prefix which can be retrieved from the
    ...
    /**
     * Returns the region with the given regionName and proper partition if found in region
     * metadata. Otherwise, returns a {@link Region} object with given regionName and aws partition.
     */
    ...
    public static RegionMetadata loadMetadataFromURI(
            final URI uri, final ClientConfiguration config) throws IOException {
    ...
    public static RegionMetadata loadMetadataFromResource(
            final ClassLoader classLoader, final String name) throws IOException {
     ...
     */
    @Deprecated
    public static RegionMetadata loadMetadataFromInputStream(final InputStream stream)
            throws IOException {
    ...
    /**
     * This method no longer attempts to retrieve region metadata from CloudFront, as that file is
     * no longer being maintained and the version bundled with the SDK is likely to be more
     * up-to-date.
     *
     * <p>It's deprecated to signal that it no longer has any possibility of retrieving a newer set
     * of metadata than was previously loaded. If you are simply wanting to reinitialize from the
     * bundled region metadata, call {@code initialize}. If you want to maintain your own remote
     * copy of the region metadata and periodically refresh it at runtime, call {@code
     * initializeFromURI}.
     *
    ...
    public static synchronized void initializeFromURI(
            final URI uri, final ClientConfiguration config) {
        try {
    ...
    /**
     * Initializes the region metadata singleton from an XML file stored as a resource of the
     * classloader used to load the RegionUtils class.
     *
     * @param name the path of the resource, relative to the RegionUtils class
     * @throws SdkClientException on error
     */
    ...
    public static synchronized void initializeFromResource(
        ...
        try {
            ...
            throw new SdkClientException(
                    "Error parsing region metadata from resource " + name, exception);
        }
    }
