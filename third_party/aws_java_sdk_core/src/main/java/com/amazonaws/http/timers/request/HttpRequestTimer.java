...
public class HttpRequestTimer {
     ...
     *
     * @param apacheRequest HTTP request this timer will abort if triggered.
     * @param requestTimeoutMillis A positive value here enables the timer, a non-positive value
     *     disables it and returns a dummy tracker task
     * @return Implementation of {@link HttpRequestAbortTaskTrackerImpl} to query the state of the
     *     task and cancel it if appropriate
     */
    public HttpRequestAbortTaskTracker startTimer(
            final HttpRequestBase apacheRequest, final int requestTimeoutMillis) {
        ...
        ScheduledFuture<?> timerTaskFuture =
                executor.schedule(timerTask, requestTimeoutMillis, TimeUnit.MILLISECONDS);
     ...
     * com.amazonaws.http.AmazonHttpClient} is shutdown
     */
