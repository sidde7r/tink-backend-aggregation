...
public class CRC32ChecksumResponseInterceptor implements HttpResponseInterceptor {
    ...
    public void process(HttpResponse response, final HttpContext context)
        ...
        // Only Json protocol has this header, we only wrap CRC32ChecksumCalculatingInputStream in
        // json protocol clients.
        Header[] headers = response.getHeaders("x-amz-crc32");
        if (entity == null || headers == null || headers.length == 0) {
            return;
        }
        HttpEntity crc32ResponseEntity =
                new HttpEntityWrapper(entity) {

                    private final InputStream content =
                            new CRC32ChecksumCalculatingInputStream(wrappedEntity.getContent());
                    ...
                    /**
                     * It's important to override writeTo. Some versions of Apache HTTP client use
                     * writeTo for {@link tink.org.apache.http.entity.BufferedHttpEntity} and the
                     * default implementation just delegates to the wrapped entity which completely
                     * bypasses our CRC32 calculating input stream. The {@link
                     * tink.org.apache.http.entity.BufferedHttpEntity} is used for the request
                     * timeout and client execution timeout features.
                     *
        ...
        context.setAttribute(
                CRC32ChecksumCalculatingInputStream.class.getName(),
