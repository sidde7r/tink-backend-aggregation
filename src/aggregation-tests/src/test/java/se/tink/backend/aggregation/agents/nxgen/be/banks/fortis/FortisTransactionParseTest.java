...
public class FortisTransactionParseTest {
    ...
    public void bulkErrorMessage() {
        EbankingUsersResponse ebankingUsersResponse = SerializationUtils.deserializeFromString(FortisTestData.BULK_ERROR_MESSAGE, EbankingUsersResponse.class);

        Assert.assertEquals("Global error", "E", ebankingUsersResponse.getBusinessMessageBulk().getGlobalIndicator());
    }
    ...
    public void upcomingTransactions() {
        ...
        UpcomingTransactionsResponse upcomingTransaction = SerializationUtils.deserializeFromString(upcoming, UpcomingTransactionsResponse.class);

        Collection<? extends UpcomingTransaction> tinkTransactions = upcomingTransaction.getTinkTransactions();

        Assert.assertEquals("Upcoming transactions", 1, tinkTransactions.size());
        Assert.assertFalse(upcomingTransaction.canFetchMore().orElseThrow(IllegalArgumentException::new));
    }
