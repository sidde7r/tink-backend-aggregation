...
public class DuplicatePaymentExceptionHandlerTest extends ExceptionHandlerBaseTest {

    @Before
    public void setUp() {
        handler = new DuplicatePaymentExceptionHandler();
        super.setUp();
        given(catalog.getString((String) Mockito.any())).willReturn("Catalog string");
    }

    @Test
    public void shouldMarkMetricCanceledAndCancelOperation() {
        // given
        DuplicatePaymentException exception = new DuplicatePaymentException();
        ...
        Assertions.assertThat(signableOperation.getInternalStatus())
                .isEqualTo(InternalStatus.DUPLICATE_PAYMENT.toString());
