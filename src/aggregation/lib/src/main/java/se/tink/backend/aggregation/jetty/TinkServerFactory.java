package se.tink.backend.aggregation.jetty;
 ...
 * <<<<<<<<<<<< TINK NB START >>>>>>>>>>>>>>
 *
 * This class is a COPY of Dropwizard's DefaultServerFactory. The extension we are doing is exposing
 * a StatisticsHandler that we can use in order to retrieve metrics from the Jetty Server
 *
 * <<<<<<<<<<<< TINK NB STOP >>>>>>>>>>>>>>>
 * </pre>
 *
 * <p>The default implementation of {@link ServerFactory}, which allows for multiple sets of
 ...
 * </table>
 *
 * <p>For more configuration parameters, see {@link AbstractServerFactory}.
 *
 * @see ServerFactory
 * @see AbstractServerFactory
 */
@JsonTypeName("default")
public class TinkServerFactory extends AbstractServerFactory {
    @Valid @NotNull
    ...
    public Server build(Environment environment) {
        ...
        final RoutingHandler routingHandler =
                buildRoutingHandler(
                        environment.metrics(), server, applicationHandler, adminHandler);

        statisticsHandler.setHandler(addRequestLog(server, routingHandler, environment.getName()));

        server.setHandler(statisticsHandler);
        return server;
    }

    public void registerJettyMetrics(CollectorRegistry registry) {
        JettyStatisticsCollector jettyStatisticsCollector =
                new JettyStatisticsCollector(statisticsHandler);
        jettyStatisticsCollector.register(registry);
    }
    ...
    private List<Connector> buildAdminConnectors(MetricRegistry metricRegistry, Server server) {
        // threadpool is shared between all the connectors, so it should be managed by the server
        // instead of the
