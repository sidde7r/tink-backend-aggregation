...
public class CreditSafeAgentTest extends AbstractAgentTest<CreditSafeAgent> {
    ...
    public void testRealPersonNumberWithBankId() throws Exception {
        ...
        //credentials.setSensitivePayload(FraudUtils.ID_CONTROL_AUTH_KEY, StringUtils.hashAsStringSHA1(personNumber, FraudUtils.ID_CONTROL_AUTH_SALT));
        AgentTestContext context = testAgentNonTestEnv(credentials);

        ImmutableListMultimap<FraudDetailsContentType, FraudDetailsContent> detailsByType = getFraudDetaildByType(context);
        
        System.out.println(mapper.writerWithDefaultPrettyPrinter().writeValueAsString(detailsByType.values()));
    }
    ...
    public void testDataWithNonPayments() throws Exception {
        ...
        // I changed from 9 -> 10 because of failing test. CreditSafe seem to deliver one more Non Payment (i.e. no bug)
        // Maybe this assert is bad to have as it will change from time to time... /JE
        Assert.assertEquals("Wrong number of non payment details.", 10, detailsByType.get(FraudDetailsContentType.NON_PAYMENT).size(), 0);
        Assert.assertEquals("Wrong number of credits details.", 1, detailsByType.get(FraudDetailsContentType.CREDITS).size(), 0);
        Assert.assertEquals("Wrong number of credits details.", 1, detailsByType.get(FraudDetailsContentType.IDENTITY).size(), 0);
        Assert.assertEquals("Wrong number of credits details.", 1, detailsByType.get(FraudDetailsContentType.ADDRESS).size(), 0);
    }
    ...
    public void testDataImportWithScoring() throws Exception {
        ...
        Assert.assertEquals("Wrong number of scoring details.", 1, detailsByType.get(FraudDetailsContentType.SCORING).size(), 0);
    }
     ...
     * @throws Exception
     */
    private ImmutableListMultimap<FraudDetailsContentType, FraudDetailsContent> getFraudDetaildByType(AgentTestContext context) {

        ImmutableListMultimap<FraudDetailsContentType, FraudDetailsContent> detailsByType = Multimaps
                .index(context.getDetailsContents(),
