...
public class ValidateInternalTransferBodyTest {
    ...
    public void testShortMessageFormatting() {
        ...
        ValidateInternalTransferBody body = new ValidateInternalTransferBody(transfer, "fromAccount", "toAccount");
    ...
    public void testAverageMessageFormatting() {
        ...
        String message = "Tink test, this message is longer than the shortest one but still not of max length";

        Transfer transfer = createTransfer(amount, message);
        ValidateInternalTransferBody body = new ValidateInternalTransferBody(transfer, "fromAccount", "toAccount");

        Assertions.assertThat(body.get("amount").get(0)).isEqualTo("+000000000000003142");
        Assertions.assertThat(body.get("commLine1").get(0)).isEqualTo("Tink test, this message is longer t");
        Assertions.assertThat(body.get("commLine2").get(0)).isEqualTo("han the shortest one but still not ");
    ...
    public void testLongerThanMaxMessageLengthFormatting() {
        ...
        String message = "Tink test, this message is longer than the allowed message length of 140 chars, making sure that the message is divided and truncated appropriately";
        ...
        Assertions.assertThat(body.get("commLine1").get(0)).isEqualTo("Tink test, this message is longer t");
        Assertions.assertThat(body.get("commLine2").get(0)).isEqualTo("han the allowed message length of 1");
        Assertions.assertThat(body.get("commLine3").get(0)).isEqualTo("40 chars, making sure that the mess");
        Assertions.assertThat(body.get("commLine4").get(0)).isEqualTo("age is divided and truncated approp");
        Assertions.assertThat(body.get("memoDate").get(0)).isEqualTo("20180417");
    }
