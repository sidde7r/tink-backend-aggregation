...
/**
 * Parsing of one PaymentEntity in the response list is different from parsing multiple since Nordea sends back
 * an array of entities if multiple in JSON format, if only one they send back a JSON object instead of an array on the
 * key.
 *
 * Since we use these entities (at the time of writing this) to check that we don't sign other than our own transfers in
 * the Nordea outbox we want to make sure we parse all info needed.
 */
public class UnconfirmedPaymentListResponseTest {
    @Test
    public void convertsSinglePayment() throws IOException {
        ObjectMapper objectMapper = new ObjectMapper();
        UnconfirmedPaymentListResponse unconfirmedPaymentListResponse = objectMapper.readValue(RESPONSE_SINGLE_PAYMENT, UnconfirmedPaymentListResponse.class);
        ...
        // Make sure we parse all stuff that's essential for mapping our request/response payment to a PaymentEntity
    ...
    public void convertsMultiplePayments() throws IOException {
        ObjectMapper objectMapper = new ObjectMapper();
        UnconfirmedPaymentListResponse unconfirmedPaymentListResponse = objectMapper.readValue(RESPONSE_MULTIPLE_PAYMENTS, UnconfirmedPaymentListResponse.class);
        ...
        // Don't need to check every value, just know that we are parsing stuff since it's same entity type as first
