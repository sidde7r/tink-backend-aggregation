...
public abstract class Borrower {
    ...
    private static final AggregationLogger log = new AggregationLogger(Borrower.class);
    ...
    public static Borrower createBorrowerFromApplication(GenericApplicationFieldGroup applicantGroup)
        ...
        Borrower borrower;
        if (new DateTime().isBefore(new DateTime(2017, 2, 21, 0, 0, 0))) {
            // TODO: Remove if statement after deploying new API the 21st and merge to only one model with new fields
            borrower = new BorrowerWithOldFields();
        } else {
            borrower = new BorrowerWithNewFields();
        }
        ...
        if (borrower instanceof BorrowerWithOldFields) {
            // TODO: Remove if statement after deploying new API the 21st and only use below else
            BorrowerWithOldFields borrowerWithOldFields = ((BorrowerWithOldFields) borrower);
            borrowerWithOldFields.setHasBeenEmployedForAtLeastTwelveMonths(true);
        }
