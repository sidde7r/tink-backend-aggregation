...
public abstract class DeviceSecurityContextIdValidator extends HandelsbankenValidator<BaseResponse> {
    ...
    public DeviceSecurityContextIdValidator(
            Credentials credentials, BaseResponse response) {
        super(response);
    ...
    protected void validate(Supplier<RuntimeException> fallback) throws SessionException {
        ...
        if (!Strings.isNullOrEmpty(code)) {
            ...
            if (HandelsbankenConstants.AutoAuthentication.Validation.DEVICE_SECURITY_CONTEXT_ID_INVALID.equals(code)) {
                ...
                if (credentialUpdatedDate == null || !credentialUpdatedDate.after(dateOfLoginChange.toDate())) {
                    throw HandelsbankenConstants.AutoAuthentication.UserError.DEVICE_SECURITY_CONTEXT_ID_INVALID
                            .exception();
                }
                throw fallback.get();
            }
