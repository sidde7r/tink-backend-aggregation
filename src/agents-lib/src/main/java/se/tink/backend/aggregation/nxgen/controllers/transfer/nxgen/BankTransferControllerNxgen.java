...
public class BankTransferControllerNxgen implements BankTransferExecutor {
    ...
    public Optional<String> executeTransfer(Transfer transfer) throws TransferExecutionException {
        ...
        Optional<TransferSource> source = accounts.stream()
                .filter(account -> account.getAccountIdentifier().equals(transfer.getSource())).findFirst();
        ...
        if (transferDestination == null) {
            ...
            if (transferDestination == null) {
                ...
                if (!name.isPresent()) {
                    throw TransferExecutionException.builder(SignableOperationStatuses.FAILED)
                            .setEndUserMessage(TransferExecutionException.EndUserMessage.NEW_RECIPIENT_NAME_ABSENT)
                            .build();
                }

                transferDestination = executor.addBeneficiary(name.get(), transfer.getDestination());
            }
        }
