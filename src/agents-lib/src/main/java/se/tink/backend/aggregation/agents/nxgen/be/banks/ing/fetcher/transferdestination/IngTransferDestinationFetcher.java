...
public class IngTransferDestinationFetcher implements TransferDestinationFetcher {
    ...
    private TransferDestinationsResponse getTransferDestinations(Collection<Account> accounts,
            LoginResponseEntity loginResponse) {
        TransferDestinationsResponse transferDestinations = new TransferDestinationsResponse();

        AccountListEntity accountListEntity = apiClient.fetchAccounts(loginResponse)
        ...
        transferDestinations.addDestinations(getToOwnAccountsDestinations(accounts, accountListEntity));
        transferDestinations.addDestinations(getToAllAccountsDestinations(accounts, accountListEntity, loginResponse));

        return transferDestinations;
    }

    private Map<Account,List<TransferDestinationPattern>> getToOwnAccountsDestinations(Collection<Account> accounts,
            AccountListEntity accountListEntity) {

        List<GeneralAccountEntity> sourceAccounts = getSourceAccounts(IngHelper.getAccountRulesPredicate(
                IngConstants.AccountTypes.TRANSFER_TO_OWN_RULE), accountListEntity);
        List<GeneralAccountEntity> destinationAccounts = getOwnDestinationAccounts(accountListEntity);
    ...
    private Map<Account,List<TransferDestinationPattern>> getToAllAccountsDestinations(Collection<Account> accounts,
            AccountListEntity accountListEntity, LoginResponseEntity loginResponse) {
        List<GeneralAccountEntity> sourceAccounts = getSourceAccounts(IngHelper.getAccountRulesPredicate(
                IngConstants.AccountTypes.TRANSFER_TO_ALL_RULE), accountListEntity);
    ...
    private List<GeneralAccountEntity> getSourceAccounts(Predicate<AccountEntity> accountEntityPredicate,
    ...
    private List<GeneralAccountEntity> getAllDestinationAccounts(LoginResponseEntity loginResponse) {
        return apiClient.getBeneficiaries(loginResponse)
                .map(trustedBeneficiariesResponse ->
                        trustedBeneficiariesResponse.getBeneficiaries().getBeneficiaryList())
                .map(this::filterValidDestinationAccounts)
                .orElse(Collections.emptyList());
    }

    private List<GeneralAccountEntity> filterValidDestinationAccounts(List<BeneficiaryEntity> beneficiaryList) {
        return beneficiaryList.stream()
                .filter(beneficiaryEntity -> beneficiaryEntity.generalGetAccountIdentifier() != null)
                .collect(Collectors.toList());
    }
