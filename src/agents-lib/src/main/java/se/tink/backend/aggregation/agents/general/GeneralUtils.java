...
public class GeneralUtils {
    ...
    public static <T extends GeneralAccountEntity> Optional<T> find(final AccountIdentifier identifier, List<T> entities) {
        return entities.stream()
                .filter(gae -> equal(
                        gae.generalGetAccountIdentifier().getIdentifier(DEFAULT_FORMATTER),
                        identifier.getIdentifier(DEFAULT_FORMATTER)))
                .findFirst();
    }

    public static <T extends GeneralAccountEntity> boolean isAccountExisting(AccountIdentifier identifier, List<T> entities) {
