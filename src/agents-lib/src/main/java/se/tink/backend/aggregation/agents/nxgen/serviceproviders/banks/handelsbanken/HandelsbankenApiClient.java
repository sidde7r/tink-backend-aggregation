...
public abstract class HandelsbankenApiClient {
    ...
    private static final AggregationLogger LOGGER = new AggregationLogger(HandelsbankenApiClient.class);

    private final TinkHttpClient client;
    private final HandelsbankenConfiguration<?> handelsbankenConfiguration;
    ...
    public ActivateProfileResponse activateProfile(CreateProfileResponse createProfile,
        ...
        return createPostRequest(createProfile.toActivateProfile()).post(ActivateProfileResponse.class, activateProfileRequest);
    }
    ...
    public HandshakeResponse handshake(EntryPointResponse entrypoint, HandshakeRequest handshakeRequest) {
        return createPostRequest(entrypoint.toPinnedLogin()).post(HandshakeResponse.class, handshakeRequest);
    }

    public ServerProfileResponse serverProfile(HandshakeResponse handshake,
            ServerProfileRequest serverProfileRequest) {
        return createPostRequest(handshake.toGetServerProfile()).post(ServerProfileResponse.class, serverProfileRequest);
    }

    public ChallengeResponse challenge(ServerProfileResponse serverProfile, ChallengeRequest challengeRequest) {
        return createPostRequest(serverProfile.toGetChallenge()) .post(ChallengeResponse.class, challengeRequest);
    }

    public ValidateSignatureResponse validateSignature(ChallengeResponse challenge,
            ValidateSignatureRequest validateSignature) {
        return createPostRequest(challenge.toValidateSignature()).post(ValidateSignatureResponse.class,
                validateSignature);
    }

    public ApplicationEntryPointResponse applicationEntryPoint(AuthorizeResponse authorize) {
        return createRequest(authorize.toApplicationEntryPoint()).get(ApplicationEntryPointResponse.class);
    }
    ...
    public KeepAliveResponse keepAlive(ApplicationEntryPointResponse applicationEntryPoint) {
        try {
            ...
            LOGGER.warn(HandelsbankenConstants.URLS.KeepAlive.LOG_TAG + " - Keeping session alive failed");
    ...
    public TransactionsResponse transactions(HandelsbankenAccount handelsbankenAccount) {
        return createRequest(handelsbankenAccount.toTransactions()).get(handelsbankenConfiguration
    ...
    public HandelsbankenLoansResponse loans(ApplicationEntryPointResponse applicationEntryPoint) {
        return createRequest(applicationEntryPoint.toLoans()).get(handelsbankenConfiguration.getLoansResponse());
    }
    ...
    protected RequestBuilder createRequest(URL url) {
        return client.request(url)
                ...
                .accept(MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML) // unexpectedly Handelsbanken may
                // return application/xml
                .header(HandelsbankenConstants.Headers.X_SHB_DEVICE_NAME, HandelsbankenConstants.Headers.DEVICE_NAME)
                .header(HandelsbankenConstants.Headers.X_SHB_DEVICE_MODEL, HandelsbankenConstants.Headers.DEVICE_MODEL)
                .header(HandelsbankenConstants.Headers.X_SHB_DEVICE_CLASS, HandelsbankenConstants.Headers.DEVICE_CLASS)
                .header(HandelsbankenConstants.Headers.X_SHB_APP_VERSION, handelsbankenConfiguration.getAppVersion());
    }
}
