...
public class IngApiClient {
    ...
    /**
     * This is the final part of the login sequence. The response sets a cookie that is needed for further calls.
     *
    ...
    public List<Product> getApiRestProducts() {
        return Arrays.asList(client.request(IngConstants.Url.GENOMA_API_REST_PRODUCTS)
                .type(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .get(Product[].class));
    }
    ...
    public boolean getApiRestCommunicationLogoutAllSessions() {
        try {
            this.getApiRestCommunication(IngConstants.Default.SESSION_NAME_ALL, IngConstants.Default.ACTION_NAME_LOGOUT,
    ...
    public LoginPinPad postLoginRestSession(LoginID requestBody) {
        return client.request(IngConstants.Url.GENOMA_LOGIN_REST_SESSION)
                .type(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON).post(LoginPinPad.class, requestBody);
    }
}
