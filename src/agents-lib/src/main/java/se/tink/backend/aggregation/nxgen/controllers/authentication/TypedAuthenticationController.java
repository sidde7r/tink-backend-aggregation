...
public class TypedAuthenticationController implements Authenticator {
    ...
    public TypedAuthenticationController(TypedAuthenticator... authenticators) {
        this.authenticatorsByCredentialsType = Arrays.stream(authenticators)
                .filter(a -> Preconditions.checkNotNull(a).getType() != null)
                .collect(Collectors.toMap(TypedAuthenticator::getType, c -> c));

        Preconditions.checkState(!authenticatorsByCredentialsType.isEmpty(),
    ...
    public void authenticate(Credentials credentials) throws AuthenticationException, AuthorizationException {
        NotImplementedException.throwIf(!authenticatorsByCredentialsType.containsKey(
                Preconditions.checkNotNull(credentials.getType())), String.format(
                        "Authentication with CredentialsType %s have not yet been implemented", credentials.getType()));
