...
public class DemoDataUtils {
    ...
    public static List<Account> readAccounts(File file, Credentials credentials) throws IOException {
        ...
        for (int i = 1; i < lines.size(); i++) {
            ...
            // Generally we always validate bankid string here to make sure it isn't reformatted by a bank. However,
    ...
    public static List<Transaction> readTransactions(DemoCredentials demoCredentials, File file,
        ...
        if (account.getType() != null && Objects.equal(account.getType(), AccountTypes.LOAN)
                || account.getType() != null && Objects.equal(account.getType(), AccountTypes.INVESTMENT)) {
            return transactions;
        }
        ...
        for (int i = 1; i < lines.size(); i++) {
            ...
            if (demoCredentials != null || file.getParentFile().getName().endsWith(FILENAME_ENDS_FOR_UNCHANGED_DATE)) {
            ...
            if (transDate.after(today)) {
                ...
                if (transaction.getType() != TransactionTypes.PAYMENT
                        || DateUtils.daysBetween(today, transDate) >= PENDING_FUTURE_TRANSACTION_CUTOFF) {
                    continue;
                } else {
                    transaction.setPending(true);
                    if (transaction.getDescription().equals("Hyra Stockholm")) {

                        Transfer fakePayment = createFakeTransfer("HSB Stockholm Ek", "1157002203", 1954, "3300308",
            ...
            for (int j = 0; j < 10; j++) {
                try {
                    ...
                    transaction.setPayload(TransactionPayloadTypes.valueOf(payloadKey), data.get(6 + (j * 2)));
    ...
    public static Transfer createFakeTransfer(String name, String ocr, double amount, String giroNumber,
            ...
            int dueInDays, TransferType transferType) {
