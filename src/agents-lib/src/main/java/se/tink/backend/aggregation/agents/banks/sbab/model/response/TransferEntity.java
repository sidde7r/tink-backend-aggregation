...
public class TransferEntity {
    ...
    private static final SBABDestinationAccountIdentifierFormatter ACCOUNT_IDENTIFIER_FORMATTER = new SBABDestinationAccountIdentifierFormatter();
    ...
    public void setNegativeAmount(String negativeAmount) {
        ...
        if (Objects.equal(this.negativeAmount, 0)) {
            throw TransferExecutionException.builder(SignableOperationStatuses.FAILED)
                    .setMessage("The transfer amount was 0, probably due to parsing error. New format? Amount string: "
                            + negativeAmount).build();
        }
    }
    ...
    public boolean equals(Object compareObject) {
        ...
        return Objects.equal(other.getNegativeAmount(), getNegativeAmount()) &&
                ...
                Objects.equal(other.getSourceMessage() == null ?
                                null :
                                other.getSourceMessage().toLowerCase(),
                        getSourceMessage() == null ? null : getSourceMessage().toLowerCase()) &&
                Objects.equal(other.getDestinationMessage() == null ?
                        null :
                        other.getDestinationMessage().toLowerCase(), getDestinationMessage() == null ?
                        null :
    ...
    private static boolean isWithinSBAB(Transfer transfer) {
        ...
        if (Objects.equal(destination.getType(), AccountIdentifier.Type.SE)) {
            Optional<ClearingNumber.Details> clearingNumber = ClearingNumber.get(
                    destination.to(SwedishIdentifier.class).getClearingNumber());
            isWithinSBAB = clearingNumber.isPresent() &&
                    Objects.equal(clearingNumber.get().getBank(), ClearingNumber.Bank.SBAB);
        }

        return isWithinSBAB;
    }

    public static TransferEntity create(Transfer transfer, TransferMessageFormatter.Messages messages) {
        ...
        transferEntity
                .setDestinationAccountNumber(transfer.getDestination().getIdentifier(ACCOUNT_IDENTIFIER_FORMATTER));
