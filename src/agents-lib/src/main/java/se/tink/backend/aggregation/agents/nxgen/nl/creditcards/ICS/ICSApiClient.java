...
public class ICSApiClient {
    ...
    public ICSApiClient(TinkHttpClient client, SessionStorage sessionStorage, PersistentStorage persistentStorage) {
        this.client = client;
    ...
    private ICSConfiguration getICSConfiguration() {
        return persistentStorage.get(ICSConstants.Storage.ICS_CONFIGURATION, ICSConfiguration.class).get();
    }
    ...
    public URL getAuthorizeUrl(String state) {
        ...
        ClientCredentialTokenResponse clientCredentialTokenResponse = getTokenWithClientCredential();

        // 2. setup account & get accountRequestId
        AccountSetupResponse accountSetupResponse = accountSetup(clientCredentialTokenResponse.toTinkToken());
        ...
        return getAuthRequest(ICSConstants.URL.OAUTH_AUTHORIZE)
                ...
                .queryParam(ICSConstants.Query.ACCOUNT_REQUEST_ID, accountSetupResponse.getData().getAccountRequestId())
                .queryParam(ICSConstants.Query.STATE, state)
                .queryParam(ICSConstants.Query.REDIRECT_URI, getICSConfiguration().getRedirectUri())
                .queryParam(ICSConstants.Query.RESPONSE_TYPE, ICSConstants.Query.RESPONSE_TYPE_CODE).getUrl();
    }
    ...
    public AccountSetupResponse accountSetup(OAuth2Token token) {
        ...
        AccountSetupRequest request = new AccountSetupRequest()
                .setup(ICSConstants.Permissions.ALL_READ_PERMISSIONS, fromDate, toDate, expirationDate);

        String lastLoggedTime = getLastLoggedTime(new Date());

        return getAPIRequest(ICSConstants.URL.ACCOUNT_SETUP, token,
                getICSConfiguration().getClientId(), getICSConfiguration().getClientSecret(), getFinancialId(),
                getCustomerIpAdress(), getInteractionId())
    ...
    private ClientCredentialTokenResponse getTokenWithClientCredential() {
        return getAPIRequest(ICSConstants.URL.OAUTH_TOKEN)
                ...
                .queryParam(ICSConstants.Query.GRANT_TYPE, ICSConstants.Query.GRANT_TYPE_CLIENT_CREDENTIALS)
                .queryParam(ICSConstants.Query.SCOPE, ICSConstants.Query.SCOPE_ACCOUNTS)
    ...
    private boolean receivedAllReadPermissions(AccountSetupResponse response) {
        return response.getData().getPermissions().equals(ICSConstants.Permissions.ALL_READ_PERMISSIONS);
    }
    ...
    private OAuth2Token getToken() {
        return this.persistentStorage.get(ICSConstants.Storage.TOKEN, OAuth2Token.class)
    ...
    public OAuth2Token fetchToken(String authCode) {
        ...
        return getAPIRequest(ICSConstants.URL.OAUTH_TOKEN)
                ...
                .queryParam(ICSConstants.Query.STATE, state)
                .get(TokenResponse.class).toTinkToken();

    }

    public OAuth2Token refreshToken(String refreshToken) {
        return getAPIRequest(ICSConstants.URL.OAUTH_TOKEN)
                .queryParam(ICSConstants.Query.GRANT_TYPE, ICSConstants.Query.GRANT_TYPE_REFRESH_TOKEN)
                .queryParam(ICSConstants.Query.CLIENT_ID, getICSConfiguration().getClientId())
                .queryParam(ICSConstants.Query.CLIENT_SECRET, getICSConfiguration().getClientSecret())
    ...
    private RequestBuilder getAPIRequest(String url, OAuth2Token token, String clientId, String clientSecret,
            String xFinancialID, String xCustomerIPAddress, String xInteractionId) {
    ...
    public CreditAccountsResponse getAllAccounts() {
        return getAPIRequest(ICSConstants.URL.ACCOUNT, getToken(), getICSConfiguration().getClientId(), getICSConfiguration().getClientSecret(), getFinancialId(),
                getCustomerIpAdress(), getInteractionId())
                .get(CreditAccountsResponse.class);
    }

    public CreditBalanceResponse getAccountBalance(String accountId) {
        return getAPIRequest(String.format(ICSConstants.URL.BALANCES, accountId), getToken(), getICSConfiguration().getClientId(), getICSConfiguration().getClientSecret(),
                getFinancialId(),
                getCustomerIpAdress(), getInteractionId())
    ...
    public CreditTransactionsResponse getTransactions(String accountId) {
        return getAPIRequest(String.format(ICSConstants.URL.TRANSACTIONS, accountId), getToken(), getICSConfiguration().getClientId(),
