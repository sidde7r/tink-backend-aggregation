...
public class DanskeBankApiClient {
    ...
    public DanskeBankApiClient(Client client, String userAgent, BankIdResourceHelper bankIdResourceHelper,
            String providerCountry, String sessionLanguage) {
        this.client = client;
    ...
    public LoginResponse loginConfirmChallenge(LoginResponse loginResponse, String challengeResponse) {
    ...
    public InitBankIdResponse bankIdInitAuth(InitBankIdRequest initBankIdRequest) {
        ...
        ClientResponse initBankIdClientResponse = postBankIdService(BankIdServiceType.INITAUTH, initBankIdAuthMessage);

        MessageContainer initBankIdAuthResponseMessage = initBankIdClientResponse.getEntity(MessageContainer.class);
        return initBankIdAuthResponseMessage.decrypt(bankIdResourceHelper, InitBankIdResponse.class);
    }

    public BankIdResponse bankIdVerify(BankIdServiceType bankIdServiceType, String orderReference)
            throws LoginException {
        MessageContainer verifyAuthMessage = createEncryptedVerifyRequest(bankIdServiceType, orderReference);

        ClientResponse verifyClientResponse = postBankIdService(bankIdServiceType, verifyAuthMessage);

        return getDecryptedBankIdResponse(bankIdServiceType, verifyClientResponse);
    }

    /**
     * Used after BankID auth is complete, to tie the two Danske systems to each other by the NSSID cookie. Without this
     * call the user will never be logged in and authed to access nodes in the `MB` url subset.
     */
    ...
    boolean isCreditCardAccount(AccountEntity accountEntity) {
        ...
        if (cardsResponse.getOutput() == null || cardsResponse.getOutput().getCreditCardAccounts() == null) {
            return false;
        }
    ...
    void fetchCustomerSettings() {
        ...
        createRequestWithCustomParameters(DanskeBankUrl.CUSTOMER_SETTINGS, queryParameters).get(ClientResponse.class);
    }
     ...
     *
     * FYI: After logging number of pages fetched over ~1 day the following stats were outcome: - Median: 1 page fetched
     * - Mean: 1.24… pages fetched - Max: 4 pages
     *
     * @param previousResponse For paging, so that we continue from the last transaction and continue backwards
     * @return One page of transactions. To fetch more do another get and send in the previous response returned.
     */
    public AccountResponse getTransactions(AccountEntity accountEntity, DanskeBankV2Agent.TransactionType type,
        ...
        String transactionsUrl = DanskeBankUrl.serviceAccountsIdTransaction(accountEntity.getAccountId(), type);
    ...
    public TransferResponse createTransfer(TransferRequest transferRequest)
        ...
        String transferRequestSerialized = MAPPER.writeValueAsString(transferRequest).replace("\\\\", "\\");

        return post(DanskeBankUrl.TRANSFER, TransferResponse.class, transferRequestSerialized);
    }

    public BillResponse createPayment(BillRequest billRequest)
            throws IOException {
        // Hack to ensure Jersey/Jackson doesn't escape stuff.
    ...
    public void confirmEInvoice(ChallengeResponseRequest challengeResponseRequest, String transactionId)
            throws UnsupportedEncodingException {
        Preconditions.checkNotNull(Strings.emptyToNull(transactionId));
        sendChallengeResponse(DanskeBankUrl.eInvoiceApproveChallenge(transactionId), challengeResponseRequest);
    }
    ...
    public EInvoiceDetailsResponse getEInvoiceDetails(String transactionId) throws UnsupportedEncodingException {
        String detailsUrl = DanskeBankUrl.eInvoiceDetails(transactionId);

        return get(detailsUrl, EInvoiceDetailsResponse.class);
    }

    public EInvoiceApproveResponse approveEInvoice(EInvoiceApproveRequest request, String transactionId)
            throws IOException {
        // Hack to ensure Jersey/Jackson doesn't escape stuff.
    ...
    private <T extends AbstractResponse> T get(String url, MultivaluedMap<String, String> queryParameters,
    ...
    private <T extends AbstractResponse> T post(String url, Class<T> responseType, Object requestEntity) {
    ...
    private <T extends AbstractResponse> T postWithoutMagicKeyUpdate(String url, Class<T> responseType,
            Object requestEntity) {
        return createRequest(url).post(responseType, requestEntity);
    }

    private ClientResponse postBankIdService(BankIdServiceType serviceType, MessageContainer messageContainer) {
        ClientResponse response = createRequest(serviceType).post(ClientResponse.class, messageContainer);
    ...
    private WebResource.Builder createRequest(BankIdServiceType bankIdServiceType) {
        String authorizationHeader = bankIdResourceHelper.generateAuthorizationHeader(bankIdServiceType);
     ...
     * Make sure to do `updateMagicKey(…)` on the result entity if needed.
     */
    private WebResource.Builder createRequest(String url, MultivaluedMap<String, String> queryParameters) {
        if (magicKey != null) {
    ...
    private String getObfuscatedMagicKey() {
        try {
            return URLEncoder.encode(MobileBankingEncryptionHelper.modifyMagicKey(magicKey), CHARSET_NAME_ASCII);
        } catch (UnsupportedEncodingException e) {
            throw new IllegalStateException(e.getMessage(), e);
        }
    }

    private MessageContainer createEncryptedVerifyRequest(BankIdServiceType bankIdServiceType, String orderReference) {
    ...
    private BankIdResponse getDecryptedBankIdResponse(BankIdServiceType bankIdServiceType,
            ClientResponse verifyBankidClientResponse) throws LoginException {
        MessageContainer verifyResponseMessage = verifyBankidClientResponse.getEntity(MessageContainer.class);
    ...
    private static Class<? extends BankIdResponse> getBankIdResponseClass(BankIdServiceType bankIdServiceType) {
        switch (bankIdServiceType) {
        case VERIFYAUTH:
    ...
    /**
     * Used both by BankID login and by password login. When using BankID login we're using two DanskeBank systems,
     * at which point we're using the NSSID cookie to create an authenticated session that allows us to be authenticated
     * in both systems. For password logins we only use the same system for login and for other authed calls, so there
     * we don't use this call with NSSID.
     */
    private CreateSessionResponse initAndCreateSession(Credentials credentials, Optional<String> nssId) {
        InitSessionResponse initSessionResponse = postWithoutMagicKeyUpdate(
                        DanskeBankUrl.INITSESSION, InitSessionResponse.class, new InitSessionRequest());

        CreateSessionRequest createSessionRequest = CreateSessionRequest
                .create(initSessionResponse, credentials, providerCountry, sessionLanguage, nssId);

        CreateSessionResponse createSessionResponse = post(
