...
public class EncapSoapUtils {
    ...
    public String buildAuthenticationSessionCreateV1Body(String username) {
        String encryptedEdbCredentials = EncapCryptoUtils.computeEncryptedEdbCredentials(
    ...
    // Encap does not provide any wsdl so that we can "easily" generate SOAP messages, which is why they are static
    public String buildActivationCreateV1Body(String username, String activationSessionId, String samlObjectB64) {
        String encryptedEdbCredentials = EncapCryptoUtils.computeEncryptedEdbCredentials(
                configuration.getCredentialsBankCodeForEdb(),
                storage.getHardwareId(),
                configuration.getClientPrivateKeyString(),
    ...
    public String buildAuthenticationV2Body(String username, String samlObjectB64) {
        ...
        return "<soap:Envelope xmlns:c=\"http://schemas.xmlsoap.org/soap/encoding/\" xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:n1=\"urn:sam.sec.fs.evry.com:ws:mobileauthentication:v1\" xmlns:n2=\"urn:sam.sec.fs.evry.com:domain:authentication:v1\" xmlns:n3=\"urn:mobile.security.fs.edb.com:domain:mobileapplication:v1\" xmlns:n4=\"http://edb.com/ws/WSCommon_v21\">"
                ...
                + String.format("<n1:hardwareId>%s</n1:hardwareId>", StringEscapeUtils.escapeXml(
                        storage.getHardwareId()))
                + String.format("<n1:samlObject>%s</n1:samlObject>", StringEscapeUtils.escapeXml(samlObjectB64))
                + "</n1:mobileAuthSessionReadRequest>"
                + "</soap:Body>"
                + "</soap:Envelope>";
    }
    ...
    public Optional<String> getActivationSessionId(String soapResponse) {
        ...
        String errorCode = element.getElementsByTagName("ns2:ErrorCode")
                .item(0)
                .getFirstChild()
                .getTextContent();
        ...
        default:
            String errorMessage = element.getElementsByTagName("ns2:Message")
                    .item(0)
                    .getFirstChild()
                    .getTextContent();
