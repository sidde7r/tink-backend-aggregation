...
public class NordnetAgent extends AbstractAgent
  ...
  /**
   * We've previously for BANKID and PASSWORD stored an access token for persistent login. Though it
   * doesn't work anymore, and it also wasn't a long-term access token so gave not much value.
   *
   ...
   * password --> originally BANKID credential that should be reset so that we can log it in again.
   */
  ...
  public FetchTransactionsResponse fetchSavingsTransactions() {
    ...
    // Though, since there are some transactions that are sent in other currencies we cannot get a
    // complete view of
    // the account transactions and we thus removed the aggregation of them since they turned up in
    // search and ID-koll.
    ...
    getAccounts()
        .forEach(
            accountEntity -> {
              ...
              // A sparkonto don't hold instruments
              if (Objects.equals(
                  "sparkonto", Strings.nullToEmpty(accountEntity.getAccountCode()).toLowerCase())) {
                return;
              }
              // accounts can be blocked, will have no further data
              if (accountEntity.isBlocked()) {
