...
public class FortisApiClient {
    ...
    private FortisApiClient(SessionStorage sessionStorage, TinkHttpClient client) {
        this.sessionStorage = sessionStorage;
        this.client = client;
    }

    public static FortisApiClient create(SessionStorage sessionStorage, TinkHttpClient client) {
        return new FortisApiClient(sessionStorage, client);
    }
    ...
    private void checkBlockedAccount(HeaderDto header) throws AuthorizationException {
        ...
        if (Objects.equals(FortisConstants.ResultCode.ZERO_TWO, resultValue)) {
    ...
    private void verifyDoubleZeroResponseCode(HeaderDto header) {
        verifyResponseCode(header, FortisConstants.ResultCode.DOUBLE_ZERO);
    }
    ...
    private void notEnoughFundsCancelTransfer(HeaderDto header, String resultValue) {
        if (resultValue.equalsIgnoreCase(FortisConstants.ResultCode.ZERO_TWO)
                && matchesErrorMessage(header.getResultMessage(),
                FortisConstants.ErrorMessage.ACCOUNT_HAS_INSUFFICIENT_FUNDS)) {
    ...
    private void generateAndStoreCipherKey() {
        final byte[] key = RandomUtils.secureRandom(FortisConstants.Encryption.AES_KEY_LENGTH);
        sessionStorage.put(FortisConstants.Encryption.AES_SESSION_KEY_KEY, EncodingUtils.encodeAsBase64String(key));
    }

    private byte[] getCipherKey() {
        return EncodingUtils.decodeBase64String(sessionStorage.get(FortisConstants.Encryption.AES_SESSION_KEY_KEY));
    }
    ...
    private byte[] getCipherIv(byte[] cipherBytes) {
        return Arrays.copyOfRange(cipherBytes, 0, FortisConstants.Encryption.IV_LENGTH);
    }

    // Extract the body of the response containing the ciphertext
    private byte[] getCipherBody(byte[] cipherBytes) {
        return Arrays.copyOfRange(cipherBytes, FortisConstants.Encryption.IV_LENGTH, cipherBytes.length);
    }

    private String encryptAndEncodePublicKey() {
        PublicKey publicKey = RSA.getPubKeyFromBytes(
                EncodingUtils.decodeBase64String(FortisConstants.Encryption.PUBLIC_KEY));
    ...
    private <T> T post(
            FortisConstants.Url url, Object request, Class<T> responseType) {
        return post(url, request, responseType, true);
    }

    private <T> T post(
            FortisConstants.Url url, Object request, Class<T> responseType, boolean encryptAndEncodeRequest) {
    ...
    public void prepareSession() throws AuthorizationException {
        ...
        keyExchange(FortisConstants.RequestInput.COMPANY_ID, FortisConstants.RequestInput.APP_FAMILY);
    }

    public void logout() {
        LogoutResponse response = post(FortisConstants.Url.LOGOUT, null, LogoutResponse.class);
    ...
    public KeyExchangeResponse keyExchange(String companyId, String appFamily) throws AuthorizationException {
        ...
        KeyExchangeResponse response = post(FortisConstants.Url.KEY_EXCHANGE,
    ...
    public String challenge() throws AuthorizationException {
        ChallengeRequest challengeRequest = ChallengeRequest.createWithStandardTypes(
                FortisConstants.RequestInput.AUTHENTICATION_TYPE, FortisConstants.LANGUAGE);

        ChallengeResponse response = post(FortisConstants.Url.CHALLENGE, challengeRequest, ChallengeResponse.class);
        checkBlockedAccount(response.getHeader());
        verifyDoubleZeroResponseCode(response.getHeader());
    ...
    public RegisterLogonResponse registerLogon(String username, String challengeResponse)
        ...
        RegisterLogonRequest registerLogonRequest = RegisterLogonRequest.builder()
                .applicationId(FortisConstants.ApplicationId.REGISTER_LOGON)
                .captcha(FortisConstants.RequestInput.EMPTY_CAPTCHA)
                .company(FortisConstants.RequestInput.COMPANY_ID)
                .language(FortisConstants.LANGUAGE)
                .response(challengeResponse)
                .saveCardNumber(FortisConstants.RequestInput.SAVE_CARD_NUMBER)
                .ucrType(FortisConstants.RequestInput.UCR_TYPE)
                .username(username).build();

        RegisterLogonResponse response =
                post(FortisConstants.Url.REGISTER_LOGON, registerLogonRequest, RegisterLogonResponse.class);
        checkBlockedAccount(response.getHeader());
        verifyDoubleZeroResponseCode(response.getHeader());
    ...
    public PersonalisationResponse personalisation() {
        PersonalisationResponse response = post(FortisConstants.Url.PERSONALISATION,
    ...
    public String enrollDevice() {
        EnrollDeviceRoundOneRequest enrollDeviceRequest = EnrollDeviceRoundOneRequest.createWithStandardTypes(
                FortisConstants.RequestInput.IS_POLITICAL_PROMINENT_PERSION_CHECKED,
                FortisConstants.ApplicationId.APPLICATION_TYPE_CODE,
                FortisConstants.RequestInput.IS_COMMON_REPOST_STANDARD_CHECKED,
                FortisConstants.RequestInput.APP_CONDITIONS_CHECKED,
                FortisConstants.RequestInput.BANK_CONDITIONS_CHECKED,
                FortisConstants.RequestInput.DOCCLE_CHECKED);

        EnrollDeviceRoundOneResponse response = post(FortisConstants.Url.ENROLL_DEVICE,
                enrollDeviceRequest, EnrollDeviceRoundOneResponse.class);
        verifyResponseCode(response.getHeader(), FortisConstants.ResultCode.ZERO_NINE);
    ...
    public String signTypeManual(String signingId) throws AuthorizationException {
        ...
        SignTypesResponse signTypesResponse =
                post(FortisConstants.Url.SIGNING_TYPES, signRequest, SignTypesResponse.class);
        checkBlockedAccount(signTypesResponse.getHeader());
        verifyDoubleZeroResponseCode(signTypesResponse.getHeader());

        return signTypesResponse.getSignTypeId(FortisConstants.Predicates.SIGN_TYPE_MANUAL);
    }
    ...
    public String signChallenge(String signTypeId, String signingId) throws AuthorizationException {
        ...
        SignChallengeResponse signChallengeResponse =
                post(FortisConstants.Url.SIGNING_CHALLENGE, signChallengeRequest, SignChallengeResponse.class);
    ...
    public String signValidation(String signingResponse, String panNr, String signingId) throws AuthorizationException {
        ...
        SignValidationResponse signValidationResponse =
                post(FortisConstants.Url.SIGNING_VALIDATION, signValidationRequest, SignValidationResponse.class);
    ...
    public EnrollDeviceRoundTwoResponse enrollDeviceWithSigningId(String signingId) throws AuthorizationException {
        ...
        EnrollDeviceRoundTwoResponse enrollDeviceRoundTwoResponse = post(FortisConstants.Url.ENROLL_DEVICE,
    ...
    public ActivationLicenseResponse activationLicence(KbcDevice device, String iv,
        ...
        ActivationLicenseRequest activationLicenseRequest = ActivationLicenseRequest.builder()
                .applicationId(FortisConstants.RequestInput.APPLICATION_ID)
                .applicationTypeCode(FortisConstants.ApplicationId.APPLICATION_TYPE_CODE)
                .applicationVersionNo(FortisConstants.RequestInput.VERSION_NUMBER)
                .clientInitialVector(iv)
                .companyNo(FortisConstants.RequestInput.COMPANY_ID)
        ...
        ActivationLicenseResponse activationLicenseResponse = post(
                FortisConstants.Url.ACTIVATION_LICENSE, activationLicenseRequest, ActivationLicenseResponse.class);
    ...
    public String activationInstance(KbcDevice device, String iv, String encryptedNonce, String challenge,
        ...
        ActivationInstanceResponse activationInstanceResponse = post(
                FortisConstants.Url.ACTIVATION_INSTANCE, activationInstanceRequest, ActivationInstanceResponse.class);
    ...
    public ActivationVerificationResponse activationVerification(KbcDevice device, String verificationMessage)
        ...
        ActivationVerificationRequest activationVerificationRequest = ActivationVerificationRequest.builder()
                ...
                .osVersionNo(FortisConstants.RequestInput.OS_VERSION_NO)
                .osType(FortisConstants.RequestInput.OS_TYPE)
                .fingerprint(device.getFingerprint())
                .build();

        ActivationVerificationResponse activationVerificationResponse = post(
                FortisConstants.Url.ACTIVATION_VERIFICATION, activationVerificationRequest,
    ...
    public String challengeSotp(KbcDevice device) throws AuthorizationException {
        ChallengeSotpRequest challengeRequest = ChallengeSotpRequest.builder()
                .setApplicationId(FortisConstants.RequestInput.APPLICATION_ID)
                .setApplicationTypeCode(FortisConstants.ApplicationId.APPLICATION_TYPE_CODE)
                .setApplicationVersionNo(FortisConstants.RequestInput.VERSION_NUMBER)
                .setCompanyNo(FortisConstants.RequestInput.COMPANY_ID)
                .setDeviceId(device.getDeviceId())
                .setDeviceName(FortisConstants.RequestInput.DEVICE_NAME)
                .setLogonId(device.getAccessNumber())
                .setOsVersionNo(FortisConstants.RequestInput.OS_VERSION_NO)
                .setOsType(FortisConstants.RequestInput.OS_TYPE)
                .setFingerprint(device.getFingerprint())
                .build();

        ChallengeSotpResponse challengeResponse = post(
                FortisConstants.Url.CHALLENGE_SOTP, challengeRequest, ChallengeSotpResponse.class);
    ...
    public LoginSotpResponse loginSotp(KbcDevice device, String otp) throws AuthorizationException {
        LoginSotpRequest loginSotpRequest = LoginSotpRequest.builder()
                .setApplicationId(FortisConstants.RequestInput.APPLICATION_ID)
                .setAppType(FortisConstants.ApplicationId.APPLICATION_TYPE_CODE)
                .setAppVersion(FortisConstants.RequestInput.VERSION_NUMBER)
                .setCompany(FortisConstants.RequestInput.COMPANY_ID)
                .setDeviceId(device.getDeviceId())
                .setDeviceType(FortisConstants.RequestInput.DEVICE_NAME)
                .setOtp(otp)
                .setUsername(device.getAccessNumber())
                .setWithTouchId(FortisConstants.RequestInput.WITH_TOUCH_ID)
                .setOsVersion(FortisConstants.RequestInput.OS_VERSION_NO)
                .setOs(FortisConstants.RequestInput.OS_TYPE)
                .setFingerprint(device.getFingerprint())
                .build();

        LoginSotpResponse response = post(FortisConstants.Url.LOGIN_SOTP, loginSotpRequest, LoginSotpResponse.class);
        checkBlockedAccount(response.getHeader());
        verifyDoubleZeroResponseCode(response.getHeader());
    ...
    public AccountsResponse fetchAccounts() {
        ...
        this.accountResponse = post(FortisConstants.Url.ACCOUNTS, accountsRequest, AccountsResponse.class);
    ...
    public TransactionsHistoryResponse fetchTransactions(String accountNo, String repositioningKey) {
        ...
        TransactionsHistoryRequest request = TransactionsHistoryRequest.builder()
                ...
                .setSearchAmount(FortisConstants.RequestInput.SEARCH_AMOUNT)
                .setSearchMessage(FortisConstants.RequestInput.SEARCH_MESSAGE)
                .setTransactionsQuantity(FortisConstants.RequestInput.TRANSACTIONS_QUANTITY)
                .build();
        TransactionsHistoryResponse response =
                post(FortisConstants.Url.TRANSACTIONS_HISTORY, request, TransactionsHistoryResponse.class);
        verifyDoubleZeroResponseCode(response.getHeader());
        return response;
    }
    ...
    public FutureTransactionsResponse fetchFutureTransactions(String accountNo, String repositioningKey) {
        ...
        FutureTransactionsRequest request = FutureTransactionsRequest.builder()
                ...
                .setTransactionsQuantity(FortisConstants.RequestInput.TRANSACTIONS_QUANTITY)
                .setCurrencyCode(FortisConstants.RequestInput.CURRENCY)
                .build();

        FutureTransactionsResponse response =
                post(FortisConstants.Url.FUTURE_TRANSACTIONS, request, FutureTransactionsResponse.class);

        String resultCode = getResultCodeOrThrow(response.getHeader());
        if (!FortisConstants.ResultCode.DOUBLE_ZERO.equals(resultCode)
                && !FortisConstants.ResultCode.ZERO_TWO.equals(resultCode)) {
    ...
    public String fetchCards() {
        return post(FortisConstants.Url.CARDS, null, String.class);
    }
    ...
    public AccountsResponse accountsForTransferToOwn() {
        ...
        AccountsResponse response =
                post(FortisConstants.Url.ACCOUNTS_FOR_TRANSFER_TO_OWN, request, AccountsResponse.class);
        verifyDoubleZeroResponseCode(response.getHeader());
    ...
    public AccountsResponse accountsForTransferToOther() {
        ...
        AccountsResponse response =
                post(FortisConstants.Url.ACCOUNTS_FOR_TRANSFER_TO_OTHER, request, AccountsResponse.class);
        verifyDoubleZeroResponseCode(response.getHeader());
    ...
    public BeneficiariesResponse beneficiariesHistory() {
        BeneficiariesResponse response =
                post(FortisConstants.Url.BENEFICIARIES_HISTORY, null, BeneficiariesResponse.class);
        verifyDoubleZeroResponseCode(response.getHeader());
    ...
    public ValidateTransferResponse validateTransfer(Transfer transfer, AgreementDto sourceAccount, boolean isTransferToOwnAccount) {
        ...
        ValidateTransferResponse response =
                post(FortisConstants.Url.TRANSFER_VALIDATE, request, ValidateTransferResponse.class);
        verifyDoubleZeroResponseCode(response.getHeader());

        return response;
    }

    public String prepareTransfer(Transfer transfer, boolean isTransferToOwnAccount, FortisConstants.Url url, AgreementDto sourceAccount) {
        TransferRequest request = TransferRequest.create(transfer, sourceAccount, isTransferToOwnAccount);
    ...
    public SignTypesResponse signingTypes(String signingId) {
        ...
        SignTypesResponse signTypesResponse =
                post(FortisConstants.Url.MOB_A031_SIGNING_TYPES, signRequest, SignTypesResponse.class);
        verifyDoubleZeroResponseCode(signTypesResponse.getHeader());
    ...
    public SigningChallengeSotpResponse signingChallengeSotp(String signTypeId, String signingId) {
        ...
        SigningChallengeSotpResponse response =
                post(FortisConstants.Url.MOB_A031_SIGNING_CHALLENGE_SOTP, request, SigningChallengeSotpResponse.class);
        verifyDoubleZeroResponseCode(response.getHeader());
    ...
    public void signingValidationSotp(String signingResponse, String panNr, String signingId) {
        ...
        SignValidationResponse signValidationResponse =
                post(FortisConstants.Url.MOB_A031_SIGNING_VALIDATION_SOTP, signValidationRequest,
    ...
    public SigningChallengeUcrResponse signingChallengeUcr(String signTypeId, String signingId) {
        ...
        SigningChallengeUcrResponse response =
                post(FortisConstants.Url.MOB_A031_SIGNING_CHALLENGE_UCR, request, SigningChallengeUcrResponse.class);
        verifyDoubleZeroResponseCode(response.getHeader());
    ...
    public void signingValidationUcr(String signingResponse, String panNr, String signingId) {
        ...
        SignValidationResponse signValidationResponse =
                post(FortisConstants.Url.MOB_A031_SIGNING_VALIDATION_UCR, signValidationRequest,
                        SignValidationResponse.class);
        verifyDoubleZeroResponseCode(signValidationResponse.getHeader());
    }

    public TransferResponse signTransfer(String signingId, FortisConstants.Url url) {
