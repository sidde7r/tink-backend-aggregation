...
public class MobileBankingEncryptionHelper {
    ...
    public static String encryptInitSessionToken(String token) {
        try {
            ...
            byte[] firstKey = BASE64_CODEC.decode(MobileBankingEncryptionHelper.FIRST_KEY_PART_ENCODED);
            String secondKeyWithFirstKeyLength = SECOND_KEY_PART_ENCODED.substring(0, firstKey.length);
            byte[] secondKey = reverse(secondKeyWithFirstKeyLength).getBytes(Charsets.US_ASCII);
            byte[] finalKey = xor(firstKey, secondKey);

            byte[] decodedKey = BASE64_CODEC.decode(new String(finalKey, Charsets.US_ASCII).getBytes("UTF-8"));
        ...
        } catch (InvalidKeySpecException | UnsupportedEncodingException | IllegalBlockSizeException |
                    NoSuchProviderException | NoSuchPaddingException | InvalidKeyException | BadPaddingException |
    ...
    public static String modifyMagicKey(String magicKey) {
        ...
        return MobileBankingEncryptionHelper.substitute(
                MobileBankingEncryptionHelper.substitute(magicKey, i2, magicKey.charAt(i1)), i1, c1);
    }
    ...
    public static byte[] xor(byte[] paramArrayOfByte1, byte[] paramArrayOfByte2) {
        if (paramArrayOfByte1.length != paramArrayOfByte2.length) {
            throw new IllegalArgumentException(paramArrayOfByte1.length + " != " + paramArrayOfByte2.length);
        }
