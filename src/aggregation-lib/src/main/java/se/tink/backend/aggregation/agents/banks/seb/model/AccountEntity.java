...
public class AccountEntity implements GeneralAccountEntity {
    ...
    /**
     * To do payments to e.g. PG or BG types we need to have the BETFL flag == "1" to be able to do these kinds of
     * transfers (according to analyze from Charles on one of our SEB credential)
     */
    ...
    public boolean isAllowedToTransferTo(AccountIdentifier.Type destinationType) {
        if (destinationType == AccountIdentifier.Type.SE) {
            return true;
        } else if (destinationType == AccountIdentifier.Type.SE_PG || destinationType == AccountIdentifier.Type.SE_BG) {
