...
public class SEBKortAgent extends AbstractAgent implements DeprecatedRefreshExecutor {
    ...
    private boolean authenticateWithMobileBankId() throws BankIdException {
        ...
        String authenticationReponse = createClientRequest(authenticationUrl, client).get(String.class);
        ...
        OrderBankIdResponse orderBankIdResponse = createClientRequest(bankIdUrl + "/order", client).type(
        ...
        for (int i = 0; i < BANKID_MAX_ATTEMPTS; i++) {
            collectBankIdResponse = createClientRequest(bankIdUrl + "/collect", client)
        ...
        String completeBankIdResponse = createClientRequest(collectBankIdResponse.getCompleteUrl(), client).type(
        ...
        String samlResponse = createClientRequest(endpoint, client)
        ...
        ClientResponse authenticateClientResponse = createClientRequest(authenticationEndpoint, client)
                .type(MediaType.APPLICATION_FORM_URLENCODED_TYPE).entity(ElementUtils.parseFormParameters(formElement))
        ...
        createClientRequest(loginRedirectUrl, client).get(String.class);
        return true;
    }
    ...
    private List<ContractEntity> fetchContracts() throws Exception {
        ContractsResponse response = createClientRequest(BASE_URL + code + "/a/contracts", client).get(
    ...
    private List<InvoiceBillingUnitEntity> fetchInvoiceBillingUnits() throws Exception {
        InvoiceBillingUnitsResponse response = createClientRequest(BASE_URL + code + "/a/invoiceBillingUnits", client)
    ...
    private InvoiceDetailsEntity fetchInvoiceDetails(InvoiceBillingUnitEntity invoiceBillingUnit,
        ...
        InvoiceDetailsResponse response = createClientRequest(
                BASE_URL + code + "/a/invoices/details/" + invoiceBillingUnit.getArrangementNumber() + "/"
                        + invoice.getInvoiceId(), client).get(InvoiceDetailsResponse.class);
    ...
    private List<InvoiceEntity> fetchInvoices(InvoiceBillingUnitEntity invoiceBillingUnit) throws Exception {
        InvoiceBillingUnitResponse response = createClientRequest(
                BASE_URL + code + "/a/invoices/" + invoiceBillingUnit.getArrangementNumber(), client).get(
    ...
    private InvoiceDetailsEntity fetchPengingTransactions(ContractEntity contractEntity) throws Exception {
        InvoiceDetailsResponse response = createClientRequest(
                BASE_URL + code + "/a/pendingTransactions/" + contractEntity.getContractId(), client).get(
