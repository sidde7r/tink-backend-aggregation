...
/**
 * BankPrefixes reverse engineered from the SEB's webapp (from "wow4502.aspx"). The `mobileAppBankCode` were extracted
 * by entering accounts into the SEB mobile app, listening to the HTTPS traffix and extracting the code from the JSON
 * blobs.
 */
public class BankPrefix {
    ...
    private static final ImmutableMap<ClearingNumber.Bank, String> lookup = ImmutableMap
            .<ClearingNumber.Bank, String> builder()
            ...
            .put(Bank.EKOBANKEN, "PGBANK") // PGBANK is partnered with Nordea. Ekobanken is partnered with Nordea.
            .put(Bank.JAKBANKEN, "PGBANK")// PGBANK is partnered with Nordea. Jakbanken is partnered with Nordea.
            .build();

    public static String fromAccountIdentifier(AccountIdentifier identifier) {
        Preconditions.checkArgument(identifier.is(Type.SE),
                "Only swedish banks are handled at the moment.");

        if (identifier.getIdentifier(formatter).matches("^(4993|336[3-7])\\d{6}$")
                || identifier.getIdentifier(formatter).matches("^6044993\\d{6}$|^604336[3-7]\\d{6}$")) {
            // Workaround for SEB not knowing the correct clearing number to Nordea/Danske Bank. We've notified them
            ...
            Optional<String> bankPrefix = BankPrefix.fromClearingNumber(sebDestinationIdentifierBank);
            if (bankPrefix.isPresent()) {
                return bankPrefix.get();
            } else {
                throw new IllegalArgumentException("Could not BankPrefix for SEB destination identifier: "
