...
public class SebRequestTest {
    ...
    public void builtWithKundNrHasServiceInput() {
        ...
        assertThat(request.request.ServiceInput.get(0)).isEqualToComparingFieldByField(expectedServiceInput);
    }
    ...
    public void addsServiceInputs() {
        ImmutableList<ServiceInput> expectedServiceInput = ImmutableList.<ServiceInput>builder()
                .add(new ServiceInput("SEB_KUND_NR", "the user id"))
                .add(new ServiceInput("TESTING_SECOND_VARIABLE", 1))
                .add(new ServiceInput("TEST_VARIABLE", "testing the variable")).build();

        SebRequest request = SebRequest
                .withSEB_KUND_NR("the user id")
                .addServiceInputEQ("TEST_VARIABLE", "testing the variable")
                .addServiceInputEQ("TESTING_SECOND_VARIABLE", 1).build();

        assertThat(request.request.ServiceInput).hasSize(3);
        assertThat(request.request.ServiceInput).usingFieldByFieldElementComparator().containsAll(expectedServiceInput);
    }
}