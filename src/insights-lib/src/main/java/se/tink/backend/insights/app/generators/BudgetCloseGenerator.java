...
public class BudgetCloseGenerator implements InsightGenerator {
    private static final LogUtils log = new LogUtils(BudgetCloseGenerator.class);
    ...
    @Inject
    public BudgetCloseGenerator(CommandGateway gateway,
    ...
    public void generateIfShould(UserId userId) {
        ...
        for (FollowItem followItem : followItems) {
            ...
            if (followItemTransaction.isCloseToOverSpending()) {
                followItemTransactions.add(followItemTransaction);
            }
        }

        if (followItemTransactions.size() <= 0) {
            log.info(userId, "No insight generated. Reason: No followItemTransactions found");
