...
public class LeftToSpendLowGenerator implements InsightGenerator {
    ...
    @Inject
    public LeftToSpendLowGenerator(CommandGateway gateway, StatisticsQueryService statisticsQueryService,
    ...
    public void generateIfShould(UserId userId) {
        ...
        if (amount > 0) {
            log.info(userId, "No insight generated. Reason: Still got left to spend");
            return;
        }

        CreateLeftToSpendLowInsightCommand command = new CreateLeftToSpendLowInsightCommand(userId, Amount.of(amount));
