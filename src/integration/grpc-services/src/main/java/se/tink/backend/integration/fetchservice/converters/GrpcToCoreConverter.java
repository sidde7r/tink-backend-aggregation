...
public class GrpcToCoreConverter {
    ...
    public static FetchCheckingAccountsCommand convert(IntegrationRequest request) {
        ...
        return FetchCheckingAccountsCommand.of(
                ...
                AgentInfo.of(
                        request.getAgentInfo().getAgentClassName(),
                        request.getAgentInfo().getState().getState()),
                Credentials.of(
                        request.getCredentials().getId(),
                        request.getCredentials().getUserId(),
                        request.getCredentials().getFieldsSerialized(),
                        EnumMapper.CREDENTIALS_TYPE.getOrDefault(
                                request.getCredentials().getType(), Credentials.Type.UNKNOWN)),
                AggregatorInfo.of(
                        request.getAggregatorInfo().getClientId(),
