...
public class TrustRootCaStrategy implements TrustStrategy {
    ...
    private TrustRootCaStrategy(
            X509TrustManager customCaTrustManager, X509TrustManager fallbackTrustManager) {
    ...
    // Create a TrustStrategy that will both trust servers with a signed certificate from the CA in
    // the keystore
    ...
    // Create a TrustStrategy that only trusts servers with a signed certificate from the CA in the
    // keystore.
    ...
    public boolean isTrusted(X509Certificate[] chain, String authType) throws CertificateException {
        return isTrusted(customCaTrustManager, chain, authType)
                || (Objects.nonNull(fallbackTrustManager)
                        && isTrusted(fallbackTrustManager, chain, authType));
    }
}
