...
public class EncapCrypto {
    ...
    static String computeMAC(byte[] rand16BytesKey, byte[] rand16BytesIv, byte[] inputData) {
        byte[] hmacKey = Bytes.concat(rand16BytesKey, rand16BytesIv);
        byte[] hash = Hash.hmacSha256(hmacKey, inputData);
        return Base64.getEncoder().encodeToString(hash);
    }

    static String computePublicKeyHash(byte[] pubKeyBytes) {
    ...
    static String computeB64ChallengeResponse(String authenticationKey, String b64OtpChallenge) {
