...
public class SupplementalInformationController {
    private static final AggregationLogger log = new AggregationLogger(SupplementalInformationController.class);
    ...
    public void setSupplementalInformation(String credentialsId, String fields) {
        ...
        DistributedBarrier lock = new DistributedBarrier(coordinationClient,
                BarrierName.build(BarrierName.Prefix.SUPPLEMENTAL_INFORMATION, credentialsId));
    ...
    public String getSupplementalInformation(String credentialsId) {
        try {
            return (String) cacheClient.get(CacheScope.SUPPLEMENT_CREDENTIALS_BY_CREDENTIALSID, credentialsId);
        } finally {
