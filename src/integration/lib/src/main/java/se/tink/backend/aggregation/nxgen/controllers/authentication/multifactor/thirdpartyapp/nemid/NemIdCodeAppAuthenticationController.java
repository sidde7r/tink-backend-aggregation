...
public abstract class NemIdCodeAppAuthenticationController  implements //AutoAuthenticator,
    ThirdPartyAppAuthenticator<String> {
  ...
  public NemIdCodeAppResponse collect(String reference)
    ...
    NemIdCodeAppPollResponse pollResponse =
        pollCodeApp(reference);

    String pollStatus = pollResponse.getStatus();
    ThirdPartyAppStatus status;
    switch (pollStatus.toLowerCase()) {
      case Status.STATUS_OK:
        status = pollResponse.isConfirmation() ? ThirdPartyAppStatus.DONE : ThirdPartyAppStatus.CANCELLED;
        break;
  ...
  public ThirdPartyAppAuthenticationPayload getAppPayload() {
    ...
    // TODO this is the app scheme, but it shouldn't be used to open the app, according to the docs
