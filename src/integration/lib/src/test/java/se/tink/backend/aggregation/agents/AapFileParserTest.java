...
public class AapFileParserTest {
    ...
    public void testParser() throws Exception {
        ...
        final Set<Pair<String, String>> expectedRequestHeaders =
                ImmutableSet.of(
                        new Pair<>("Accept", "application/json"),
                        new Pair<>("X-Aggregator", "Tink Testing"));

        final Set<Pair<String, String>> expectedResponseHeaders =
                ImmutableSet.of(
                        new Pair<>("Content-Type", "application/json;charset=utf-8"),
                        new Pair<>(
                                "Cache-Control", "private, no-store, no-cache, must-revalidate"));

        Set<Pair<HTTPRequest, HTTPResponse>> expectedResult =
                ImmutableSet.of(
                        new Pair<>(
        ...
        Set<Pair<HTTPRequest, HTTPResponse>> pairs = parser.parseRequestResponsePairs();
