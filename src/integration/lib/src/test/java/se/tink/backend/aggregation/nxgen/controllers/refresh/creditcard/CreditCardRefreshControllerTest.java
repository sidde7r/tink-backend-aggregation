...
public class CreditCardRefreshControllerTest {
    ...
    @Mock private AccountFetcher<CreditCardAccount> accountFetcher;
    @Mock private TransactionFetcher<CreditCardAccount> transactionFetcher;
    ...
    public void ensureExceptionIsThrown_whenContext_isNull() {
        new CreditCardRefreshController(
                metricRefreshController, null, accountFetcher, transactionFetcher);
    }

    @Test(expected = NullPointerException.class)
    public void ensureExceptionIsThrown_whenAccountFetcher_isNull() {
        new CreditCardRefreshController(
                metricRefreshController, updateController, null, transactionFetcher);
    }

    @Test(expected = NullPointerException.class)
    public void ensureExceptionIsThrown_whenTransactionFetcher_isNull() {
        new CreditCardRefreshController(
                metricRefreshController, updateController, accountFetcher, null);
    }
    ...
    public void ensureNullAccountsList_isConverted_toEmptyList() {
        ...
        Mockito.verify(updateController, Mockito.never())
    ...
    public void ensureNullTransactionsList_isConverted_toEmptyList() {
        List<CreditCardAccount> accounts =
                ImmutableList.of(TestAccountBuilder.from(CreditCardAccount.class).build());
        Mockito.when(accountFetcher.fetchAccounts()).thenReturn(accounts);
        Mockito.when(transactionFetcher.fetchTransactionsFor(Mockito.any(CreditCardAccount.class)))
                .thenReturn(null);

        creditCardRefresher.fetchTransactions();

        Mockito.verify(accountFetcher).fetchAccounts();
        Mockito.verify(transactionFetcher)
                .fetchTransactionsFor(Mockito.any(CreditCardAccount.class));
        Mockito.verify(updateController)
                .updateTransactions(accounts.get(0), Collections.emptyList());
    }
}
