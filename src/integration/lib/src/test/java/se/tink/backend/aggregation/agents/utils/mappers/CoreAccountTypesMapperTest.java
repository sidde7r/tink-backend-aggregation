...
public class CoreAccountTypesMapperTest {
    ...
    public void exhaustiveCorrectMapping_fromMainToAggregationTypes() {
        // CHECKING, SAVINGS, INVESTMENT, MORTGAGE, CREDIT_CARD, LOAN, DUMMY, PENSION, OTHER,
        // EXTERNAL;
        assertEquals(
                "CHECKING", CoreAccountTypesMapper.toAggregation(AccountTypes.CHECKING).name());
        assertEquals("SAVINGS", CoreAccountTypesMapper.toAggregation(AccountTypes.SAVINGS).name());
        assertEquals(
                "INVESTMENT", CoreAccountTypesMapper.toAggregation(AccountTypes.INVESTMENT).name());
        assertEquals(
                "MORTGAGE", CoreAccountTypesMapper.toAggregation(AccountTypes.MORTGAGE).name());
        assertEquals(
                "CREDIT_CARD",
                CoreAccountTypesMapper.toAggregation(AccountTypes.CREDIT_CARD).name());
        ...
        assertEquals(
                "EXTERNAL", CoreAccountTypesMapper.toAggregation(AccountTypes.EXTERNAL).name());
    }
}
