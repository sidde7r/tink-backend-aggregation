 ...
 *
 * <p>Helper class allowing you to pass custom command line arguments to tests. This has a couple of
 * advantages over modifying (typically empty) string constants: - Editing string constants makes
 * the credentials show up in git diff, polluting the total diff and increasing the risk of
 * accidentally commiting the modifiction. - Test do not need to be @Ignore'd. If a credential has
 * not been supplied, the test will automatically be skipped. To pass custom arguments to your test,
 * supply them as Bazel flags like so: --jvmopt=-Dtink.username=myusername
 * --jvmopt=-Dtink.password=mypassword In IntelliJ, that would be: Select Run/Debug configuration ->
 * Edit configurations... -> Bazel flags window Note that system properties live on beyond the
 * lifetime of one test execution.
 */
public final class ArgumentHelper {
    ...
    /**
     * Declare the names of your command line parameters, e.g. "tink.username", "tink.password". The
     * reason they have to be specified here is so the class knows what arguments to look for when
     * deciding whether the test should be skipped.
     */
    ...
    public void before() {
        ...
        for (final String arg : arguments) {
            ...
            assumeNotNull(
                    System.getProperty(
                            arg)); // Will terminate the method here if the property is missing
    ...
    /**
     * Call this method in your @AfterClass. This will cause the helper to log a warning if any
     * tests were skipped.
     */
    public static void afterClass() {
        if (skippedTestsCount > 0) {
            logger.warn(
                    String.format(
                            "Skipped %s tests because arguments were not supplied: %s",
