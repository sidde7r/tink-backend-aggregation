...
public class HttpLoggingFilterTest {
    ...
    @Rule public WireMockRule wireMockRule = new WireMockRule(MOCK_SERVER_PORT);

    @Test
    @Ignore
    public void testHttpLoggingFilterIntegration() throws IOException {
        // Setup wiremock to stub response for post, attach a custom stream to system out to get the
        // logs
    ...
    private static void assertThatlogContainsExpectedAndMaskedInfo(String string) {
        ...
        assertThat(string)
                .contains("\"agent\":\"se.tink.backend.aggregation.agents.HttpLoggableExecutor\"");
        assertThat(string).contains("\"timestamp\":\"");
        assertThat(string)
                .contains(
                        "\"maskedBody\":null,\"maskedHeader\":\"***MAAAAASK***{\\\"testone\\\":[\\\"***MASKED***\\\"],\\\"Content-Type\\\":[\\\"text/html\\\"],\\\"Accept\\\":[\\\"application/json\\\"]}\",\"maskedMethod\":\"POST\",\"maskedUri\":\"***MAAAAASK***http://localhost:");
    ...
    private static void assertThatResponseIsUntouched(ClientResponse response) {
        assertThat(response.getEntity(String.class))
                .isEqualTo(
                        "{\n\"key1\":\"test\",\n\t\"error\":\"random stuff\",\"key3\":\"name@test.se\"}");
        assertThat(response.getHeaders().getFirst("CanBeSensitive"))
                .isEqualTo("ValueOfSensitiveKey");
    }
    ...
    @Test
    public void httpLogginFilterSplitsLogLinesInMultipleLinesWhenLargeResponse()
            throws IOException {
        // Setup wiremock to stub response for post, attach a custom stream to system out to get the
        // logs
        // and create the filter
        ...
        assertThat(logLines.length)
                .isGreaterThan(10); // Some magic number just to be sure the response is split up
    }
    ...
    private static ClientResponse attachFilterAndExecuteFakeRequest(
            HttpLoggingFilter httpLoggingFilter) {
