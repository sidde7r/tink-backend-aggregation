...
public class HandelsbankenBaseLiveEnrolement {
    ...
    public ThirdPartiesResponse getThirdPartiesId() {
        return client
                .request(new URL(HandelsbankenBaseConstants.Urls.THIRD_PARTIES))
                .header(HandelsbankenBaseConstants.HeaderKeys.TPP_TRANSACTION_ID, UUID.randomUUID().toString())
                .header(HandelsbankenBaseConstants.HeaderKeys.TPP_REQUEST_ID, UUID.randomUUID().toString())
    ...
    public TokenResponse getBearerToken(String clientId) {
        return client
                .request(new URL(HandelsbankenBaseConstants.Urls.TOKEN))
                .body(HandelsbankenBaseConstants.BodyKeys.GRANT_TYPE + "=" + HandelsbankenBaseConstants.BodyValues.CLIENT_CREDENTIALS + "&" + HandelsbankenBaseConstants.BodyKeys.SCOPE + "=" + HandelsbankenBaseConstants.BodyValues.PSD2_ADMIN + "&" + HandelsbankenBaseConstants.BodyKeys.CLIENT_ID + "=" + clientId)
    ...
    public SubscriptionResponse getAdditionalSubscription(String clientId, String appId, String code, String subscription) {
    ...
    public SubscriptionResponse getSubscription(String clientId, String code, String oauthRedirectURI) {

        return client
                .request(new URL(HandelsbankenBaseConstants.Urls.SUBSCRIPTIONS))
                .header(HandelsbankenBaseConstants.HeaderKeys.AUTHORIZATION, code)
                .header(HandelsbankenBaseConstants.HeaderKeys.TPP_REQUEST_ID, UUID.randomUUID().toString())
                .header(HandelsbankenBaseConstants.HeaderKeys.TPP_TRANSACTION_ID, UUID.randomUUID().toString())
                .header(HandelsbankenBaseConstants.HeaderKeys.X_IBM_CLIENT_ID, clientId)
    ...
    public void enroleAndSubscribeNewApp() {
        ...
        SubscriptionResponse subscriptionResponse = getSubscription(thirdPartiesResponse.getClientId(), tokenResponse.getAccessToken(), configuration.getRedirectUrl());
        SubscriptionResponse consents = getAdditionalSubscription(thirdPartiesResponse.getClientId(), subscriptionResponse.getClientId(), tokenResponse.getAccessToken(), HandelsbankenBaseConstants.BodyValues.CONSENTS);
