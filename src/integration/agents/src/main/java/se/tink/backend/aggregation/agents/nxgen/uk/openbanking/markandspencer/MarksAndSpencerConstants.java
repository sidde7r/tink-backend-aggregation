package se.tink.backend.aggregation.agents.nxgen.uk.openbanking.markandspencer;

final class MarksAndSpencerConstants {

    class Urls {

        class V11 {
            public static final String AIS_AUTH_URL =
                    "https://private.api.marksandspencer.com/open-banking/v1.1";
            public static final String AIS_API_URL =
                    "https://private.api.marksandspencer.com/open-banking/v1.1";
            public static final String PIS_AUTH_URL = "";
            public static final String PIS_API_URL = "";
            public static final String WELL_KNOWN_URL =
                    "https://developer.hsbc.com/marksandspencer/.well-known/openid-configuration";
        }

        class V31 {
            public static final String AIS_AUTH_URL =
                    "https://private.api.marksandspencer.com/open-banking/v3.1/aisp";
            public static final String AIS_API_URL =
                    "https://private.api.marksandspencer.com/open-banking/v3.1/aisp";
            public static final String PIS_AUTH_URL = "";
            public static final String PIS_API_URL = "";
            public static final String WELL_KNOWN_URL =
                    "https://developer.hsbc.com/marksandspencer/.well-known/openid-configuration";
        }
    }
}
