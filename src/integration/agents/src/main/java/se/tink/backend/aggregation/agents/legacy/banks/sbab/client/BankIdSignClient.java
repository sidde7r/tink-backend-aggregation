...
public class BankIdSignClient extends SBABClient {
    ...
    private void setSignMode(SignFormRequestBody signFormRequestBody)
            throws UnsupportedSignTypeException {
        signPath = signFormRequestBody.getSignUri();
    }

    public BankIdStartResponse initiateSign(SignFormRequestBody signFormRequestBody)
            throws Exception {
    ...
    public BankIdStatus getStatus(SignFormRequestBody signFormRequestBody, String orderReference)
            throws Exception {
        if (!signFormRequestBody.containsKey("nx_orderref")) {
            signFormRequestBody.add("nx_orderref", orderReference);
        }

        BankIdPollResponse pollResponse =
                createFormEncodedHtmlRequest(getSignUrl(POLL_PATH, signFormRequestBody.getClient()))
        ...
        switch (pollResponse.getBankIdStatus()) {
            case DONE:
                ...
                WebResource.Builder signRequest =
                        createFormEncodedHtmlRequest(getSignUrl(signPath));
                ...
                ClientResponse signResponse =
                        signRequest.post(ClientResponse.class, signFormRequestBody);
