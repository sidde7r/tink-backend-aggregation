...
public final class WLSessionHandler {
    ...
    public WLSessionHandler(
            final WLApiClient client,
            final WLSessionHandlerStorage storage,
    ...
    public HttpResponse logout() {
        ...
        final HttpResponse response =
                client.getClient()
                        .request(getApiUrl(WLConstants.Url.LOGOUT))
                        .header(
                                HttpHeaders.CONTENT_TYPE,
                                MediaType.APPLICATION_FORM_URLENCODED_TYPE)
                        .header(WLConstants.Headers.X_WL_APP_VERSION, WLConstants.WL_APP_VERSION)
                        .header(
                                WLConstants.Storage.WL_INSTANCE_ID,
                                storage.getOptionalWlInstanceId().orElse(""))
                        .body(form.serialize())
    ...
    public HttpResponse heartbeat() throws SessionException {
        final String wlInstanceId =
                storage.getOptionalWlInstanceId()
                        .orElseThrow(SessionError.SESSION_EXPIRED::exception);
        return client.getClient()
                .request(getApiUrl(WLConstants.Url.HEARTBEAT))
