...
public class CreditAgricoleBaseApiClient {
  ...
  public CreditAgricoleBaseApiClient(TinkHttpClient client,
  ...
  private CreditAgricoleBaseConfiguration getConfiguration() {
    return Optional.ofNullable(configuration)
        .orElseThrow(() -> new IllegalStateException(CreditAgricoleBaseConstants.ErrorMessages.MISSING_CONFIGURATION));
  }

  public TokenResponse getToken(final String code) {
    return TokenService.getInstance()
        .get(getConfiguration(), client, getBaseUrl(), code);
  }

  public OAuth2Token refreshToken(final String refreshToken) {
    final TokenResponse tokenResponse = TokenService.getInstance()
  ...
  public void putConsents(final List<AccountIdEntity> listOfNecessaryConstents) {
    ConsentsService.getInstance()
        .put(getBaseUrl(), persistentStorage, client, listOfNecessaryConstents, configuration);
  }
  ...
  private String getBaseUrl() {
    final BankEnum bank = persistentStorage
        ...
        .orElseThrow(() -> new IllegalStateException(CreditAgricoleBaseConstants.ErrorMessages.UNABLE_RECOGNIZE_BANK));
