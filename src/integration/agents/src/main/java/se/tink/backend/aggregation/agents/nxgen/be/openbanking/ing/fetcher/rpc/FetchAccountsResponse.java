...
public class FetchAccountsResponse {
    ...
    public Collection<AccountEntity> getAccounts(String currency) {
        return accounts != null ? accounts.stream()
            .collect(Collectors.groupingBy(AccountEntity::getResourceId)).entrySet().stream()
            .flatMap(d -> d.getValue().size() > 1 ? d.getValue().stream()
                .filter(a -> a.getCurrency().equalsIgnoreCase(currency))
                : d.getValue().stream()).collect(Collectors.toList()) : Collections.emptyList();
    }
}
