...
public class MobileBankingAuthenticationPayload {
    ...
    public static ThirdPartyAppAuthenticationPayload create(Catalog catalog, String token) {
        ...
        payload.setDownloadMessage(catalog.getString("You need to download the Mobile Banking in order to continue."));

        payload.setUpgradeTitle(catalog.getString("Upgrade Mobile Banking"));
        payload.setUpgradeMessage(catalog.getString("You need to upgrade the Mobile Banking in order to continue."));
        ...
        iosPayload.setDeepLinkUrl(String.format(
                "nl.abnamro.constellation.enrollment://grip?id=%s&successurl=grip://open&failurl=grip://open", token));
        ...
        androidPayload.setIntent(String.format(
                "nl.abnamro.constellation.enrollment://grip?id=%s&successurl=grip://open&failurl=grip://open", token));
