...
public class NordeaV21InvestmentFetcher implements AccountFetcher<InvestmentAccount> {
    ...
    public Collection<InvestmentAccount> fetchAccounts() {
        List<InvestmentAccount> accounts =
                client.getAccountProductsOfTypes(NordeaV21Constants.ProductType.ACCOUNT).stream()
                        .filter(
                                pe -> {
                                    AccountTypes accountType = parser.getTinkAccountType(pe);
                                    return InvestmentAccount.ALLOWED_ACCOUNT_TYPES.contains(
                                            accountType);
                                })
