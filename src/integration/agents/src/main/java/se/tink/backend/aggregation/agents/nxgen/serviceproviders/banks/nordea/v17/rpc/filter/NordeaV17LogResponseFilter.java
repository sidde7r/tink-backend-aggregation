...
public class NordeaV17LogResponseFilter extends LogResponseFilter {
    private static final LogTag FOUND_CREDIT_CARD_CONTINUE_KEY_LOG_TAG =
            LogTag.from("#http_response_found_credit_card_continue_key_log_tag");
    private static final Pattern CONTINUE_KEY_PATTERN =
            Pattern.compile("^.*(continueKey|continuationKey).*$", Pattern.CASE_INSENSITIVE);
    ...
    public HttpResponse handle(HttpRequest httpRequest)
        ...
        log.infoExtraLong(
                String.format("Response (%s): %s", responseModel.getSimpleName(), body), logTag);

        return response;
    }

    // Temporary solution to find out if CreditCardTransactionsResponse can contain "continueKey" or
    // "continuationKey"
    ...
    private LogTag selectLogTag(String body) {
        return (Objects.equals(responseModel, CreditCardTransactionsResponse.class)
                        && CONTINUE_KEY_PATTERN.matcher(body).matches())
                ? FOUND_CREDIT_CARD_CONTINUE_KEY_LOG_TAG
                : logTag;
    }
}
