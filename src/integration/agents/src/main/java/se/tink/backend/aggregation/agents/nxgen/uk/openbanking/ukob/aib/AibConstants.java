package se.tink.backend.aggregation.agents.nxgen.uk.openbanking.ukob.aib;

public class AibConstants {

    public static final String ORGANISATION_ID = "0015800000jf9VgAAI";

    public static final String AIS_API_URL = "https://apis.aibgb.co.uk/api/open-banking/v3.1/aisp";
    public static final String PIS_API_URL = "https://apis.aibgb.co.uk/api/open-banking/v3.1/pisp";
    public static final String WELL_KNOWN_PERSONAL_URL =
            "https://openapi.aibgb.co.uk/endpoints/v2/retail/.well-known/openid-configuration";
    public static final String WELL_KNOWN_CORPORATE_URL =
            "https://openapi.aibgb.co.uk/endpoints/v2/corporate/.well-known/openid-configuration";
}
