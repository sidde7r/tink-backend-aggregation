...
public class NordeaV17CreditCardFetcher
    ...
    public Collection<CreditCardAccount> fetchAccounts() {
        return client.getAccountProductsOfTypes(ProductType.CARD).stream()
                ...
                .map(
                        pe -> {
                            CardBalancesResponse cardBalancesResponse =
                                    client.fetchCardDetails(pe.getNordeaAccountIdV2());
                            List<CardsEntity> distinctCardEntities =
                                    cardBalancesResponse
                                            .getGetCardBalancesOut()
                                            .getDistinctCardsList();

                            // We ask for one credit card, we should only get one distinct credit
                            // card in the response
                            Preconditions.checkState(
                                    distinctCardEntities.size() == 1,
    ...
    public TransactionKeyPaginatorResponse<String> getTransactionsFor(
            CreditCardAccount account, String invoicePeriod) {
        ...
        nextInvoicePeriod.ifPresent(
                next ->
                        Preconditions.checkState(
                                !next.equalsIgnoreCase(invoicePeriod),
                                String.format(
                                        "current invoice-period (%s) == next invoice-period (%s)",
                                        invoicePeriod, nextInvoicePeriod)));
