...
/**
 * The aim of this class is to encapsulate the authentication procedure that the agent needs to
 * perform in order to begin querying information from a server powered by the IBM® Worklight®
 * security framework.
 */
public final class WLPasswordAuthenticator implements PasswordAuthenticator {
    ...
    public WLPasswordAuthenticator(
            final WLApiClient client, final WLAuthenticatorStorage storage, final WLConfig config) {
        this.client = client;
        this.config = config;
    ...
    private RequestBuilder getRequest(final String resource, final MediaType mediaType) {
        return client.getClient()
                .request(config.getEndpointUrl() + resource)
    ...
    private InitResponse init1() {
        final InitResponse initResponse =
                init(
                        getRequest(
                                getApiPath(WLConstants.Url.INIT),
    ...
    /**
     * Compute authenticity realm string Submit solution to challenge by sending computed
     * authenticity realm string in Authorization header Assert allowed=true in response
     */
    private void init2(final String wlChallengeData, final String wlInstanceId) {
        final String challengeResponse =
                AuthenticityChallengeHandler.challengeToAuthenticityRealmString(
                        wlChallengeData, config.getModuleName(), config.getAppId());

        final AuthorizationEntity authorizationAuthenticity =
                new AuthorizationEntity(challengeResponse);
        final InitResponse initResponse =
                init(
                        getRequest(
                                        getApiPath(WLConstants.Url.INIT),
                                ...
                                .header(
                                        ...
                                        SerializationUtils.serializeToString(
                                                authorizationAuthenticity)));

        Preconditions.checkArgument(
                initResponse.getAllowed(), "Received allowed=false; expected allowed=true");
    }

    /**
     * Initiate Diffie-Hellman key exchange: Send token + client public key in CSR JWT (signed with
     * client private key) in Authorization header Fetch X.509 certificate (server-side public key)
     */
    private String init3(final KeyPair keyPair, final String token, final String wlInstanceId) {
        final Jwt jwt =
                new CsrJwt(token, (RSAPublicKey) keyPair.getPublic(), config.getModuleName());

        final WlDeviceAutoProvisioningRealmEntity realmEntity =
                new WlDeviceAutoProvisioningRealmEntity();
        ...
        final InitResponse initResponse =
                init(
                        getRequest(
                                ...
                                .header(
                                        HttpHeaders.AUTHORIZATION,
                                        SerializationUtils.serializeToString(authorizationJwt)));
    ...
    private void authenticateStep(
        ...
        final AuthenticateRequest body =
                new AuthenticateRequest(token, certificate, privateKey, config.getModuleName());
    ...
    private void login(final String wlInstanceId) {
        final Form form =
                Form.builder().put(WLConstants.Forms.REALM, WLConstants.Forms.REALM_VALUE).build();
        final HttpResponse response =
                getRequest(
                                getApiPath(WLConstants.Url.LOGIN),
                                MediaType.APPLICATION_FORM_URLENCODED_TYPE)
                        .header(WLConstants.Storage.WL_INSTANCE_ID, wlInstanceId)
                        .body(form.serialize())
                        .post(HttpResponse.class);
    ...
    /** Send username + password Fetch px2 string */
    private String loginSecurityCheck(String username, String password, final String wlInstanceId) {
        ...
        final JSecurityCheckResponse response =
                WLUtils.encasedJsonToEntity(httpResponse, JSecurityCheckResponse.class);
    ...
    /**
     * Send username + px2 string Assert isSuccessful, legitimation, authentication/authorization
     * error codes
     */
    private void invoke(final String username, final String px2, final String wlInstanceId)
        ...
        final Form request =
                Form.builder()
                        ...
                        .put(
                                WLConstants.Forms.PARAMETERS,
                                SerializationUtils.serializeToString(
                                        ImmutableList.of(
                                                username, px2, WLConstants.Forms.SECP_VALUE_SHORT)))
                        .build();
        final HttpResponse httpResponse =
                getRequest(WLConstants.Url.INVOKE, MediaType.APPLICATION_FORM_URLENCODED_TYPE)
                        .header(WLConstants.Storage.WL_INSTANCE_ID, wlInstanceId)
                        .body(request.serialize())
                        .post(HttpResponse.class);

        final InvokeResponse response =
                WLUtils.encasedJsonToEntity(httpResponse, InvokeResponse.class);
        ...
        if (response.isPasswordIncorrect()) {
            ...
            logger.warn(
                    "{} - Potential failure in /invoke/: expected isSuccessful to be true",
            ...
            logger.warn(
                    "{} - Potential failure in /invoke/: expected legitimation to be >= 1, found {}",
                    WLConstants.LogTags.WL_AUTHENTICATION_NOT_LEGIT.toTag(),
                    legitimation);
        } else {
