package se.tink.backend.aggregation.agents.nxgen.se.banks.seb.fetcher.investment.entities;
...
public class FundAccountEntity {
    ...
    //
    //        Account account =
    // InvestmentAccount.nxBuilder().withPortfolios(toPortfolio()).withCashBalance(getDisposableIncome()).withId();
    //
    ...
    //    }
    //
    //    private PortfolioModule toPortfolio() {
    //        PortfolioModule portfolio =
    // PortfolioModule.builder().withType(PortfolioModule.PortfolioType.DEPOT).withUniqueIdentifier(getAccountNumber()).withCashValue(getDisposableIncome()).withTotalProfit(0d).withTotalValue(0d).
    //
