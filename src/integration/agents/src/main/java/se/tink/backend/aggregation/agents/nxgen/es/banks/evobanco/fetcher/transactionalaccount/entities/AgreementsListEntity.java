...
public class AgreementsListEntity {
    ...
    private CardDataEntityGlobalPositionFirstTimeResponse cardData;
    ...
    public Optional<TransactionalAccount> toTinkAccount(String holderName) {
        ...
        Amount balance =
                new Amount(
                        EvoBancoConstants.CURRENCY_TYPE_MAPPER.translate(currencyCode).orElse("EUR"),
                        Double.parseDouble(unspentBalance));

        switch (type) {
            case CHECKING:

                tinkAccount =  Optional.of(
