...
public class IngExternalTransferExecutor {
    ...
    public IngExternalTransferExecutor(
    ...
    public void executeExternalTransfer(Transfer transfer, AccountEntity sourceAccount) {
        ...
        Optional<BeneficiaryEntity> savedBeneficiary =
                tryFindSavedBeneficiary(transfer.getDestination());
    ...
    private void executeTrustedTransfer(
            Transfer transfer, AccountEntity sourceAccount, BeneficiaryEntity destinationAccount) {
        ValidateExternalTransferResponseEntity validateExternalTransferResponseEntity =
                apiClient.validateTrustedTransfer(
                        loginResponse, transfer, sourceAccount, destinationAccount.getIbanNumber());

        ingTransferHelper.verifyTransferValidationJsonResponse(
                validateExternalTransferResponseEntity);
                        ...
                        .orElseThrow(
                                () ->
                                        ingTransferHelper.buildTranslatedTransferException(
                                                IngConstants.EndUserMessage.MISSING_DESTINATION_NAME
                                                        .getKey()
                                                        .get(),
                                                SignableOperationStatuses.CANCELLED));

        String validatedDestinationName =
                getDestinationNameWithinMaxLength(transferDestinationName);

        ValidateExternalTransferResponseEntity validateExternalTransferResponseEntity =
                apiClient.validateThirdPartyTransfer(
                        loginResponse,
                        transfer,
                        sourceAccount,
                        transfer.getDestination().getIdentifier(),
                        validatedDestinationName);
    ...
    private Optional<BeneficiaryEntity> tryFindSavedBeneficiary(
    ...
    private boolean isSavedBeneficiary(
            BeneficiaryEntity beneficiary, AccountIdentifier destinationAccountIdentifier) {
    ...
    private int calcOtp(List<ChallengeEntity> challengeList) {
        Preconditions.checkNotNull(
                challengeList, "No challenge list received from bank when signing transfer");
        ...
        byte[] otpKey =
                EncodingUtils.decodeHexString(
                        persistentStorage.get(IngConstants.Storage.OTP_KEY_HEX));

        int otp =
                IngCryptoUtils.calcOtpForSigningTransfer(
                        otpKey, otpCounter, challenge1, challenge2);
        otpCounter++;
    ...
    private String getDestinationNameWithinMaxLength(String rawDestinationName) {
        return rawDestinationName.length() > 35
                ? rawDestinationName.substring(0, 35)
                : rawDestinationName;
    }

    private void assertTransfersToExternalAccountsAllowed(AccountEntity account) {
        // If destination account isn't found among user's own accounts we must verify that the
        // source
        // account isn't restricted to only allowing transfers to user's own accounts.
        if (!IngConstants.AccountTypes.TRANSFER_TO_OWN_RULE.equalsIgnoreCase(
                account.getRulesCode())) {
