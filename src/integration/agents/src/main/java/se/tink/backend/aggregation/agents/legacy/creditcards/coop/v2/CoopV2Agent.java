...
public class CoopV2Agent extends AbstractAgent implements RefreshCreditCardAccountsExecutor {
   ...
   * transaction paging. It's only possible to give the size. Therefore we start at a small pagesize
   * and increase if needed. Usually people don't have many transactions each day. So the start size
   * of 50 should almost always be enough.
   *
  ...
  /**
   * The transactions in the app API are not bound to any specific account, but rather to an account
   * type. Though, users can have multiple accounts that map to the same account type: Type 7 and 9
   * are both named MedMera Efter, of which we think one of them are previous Visa card and the
   * other is a regular MedMera. Since V2 we now start to separate these to two accounts instead of
