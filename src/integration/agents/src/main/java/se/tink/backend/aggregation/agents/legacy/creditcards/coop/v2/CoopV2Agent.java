...
public class CoopV2Agent extends AbstractAgent implements RefreshCreditCardAccountsExecutor {
    private static ImmutableSortedSet<Integer> TRANSACTION_PAGE_SIZES =
            ImmutableSortedSet.<Integer>naturalOrder().add(200).add(1000).add(10000).build();
    ...
    /**
     * The Coop api is a bit cumbersome, since we cannot select a start date and end date of
     * transaction paging. It's only possible to give the size. Therefore we start at a small
     * pagesize and increase if needed. Usually people don't have many transactions each day. So the
     * start size of 50 should almost always be enough.
     *
     * <p>This method could be nice to have inside the ApiClient, but the helper methods for content
     * date is available only from the AbstractAgent currently.
     */
    private List<Transaction> getTransactions(Account account, AccountEntity accountEntity) {
        ...
        if (getContentWithRefreshDate(account) == null) {
            return apiClient.getTransactions(
                    accountType, Iterables.getLast(TRANSACTION_PAGE_SIZES));
        }
    ...
    private Account parseAccount(AccountEntity accountEntity) {
        ...
        if (Strings.isNullOrEmpty(account.getAccountNumber())) {
            // Before we start using accountNumber as bankId, we need to ensure that it's always
            // present
    ...
    /**
     * The transactions in the app API are not bound to any specific account, but rather to an
     * account type. Though, users can have multiple accounts that map to the same account type:
     * Type 7 and 9 are both named MedMera Efter, of which we think one of them are previous Visa
     * card and the other is a regular MedMera. Since V2 we now start to separate these to two
     * accounts instead of putting them together.
     *
     * <p>The user probably cannot have two accounts on the same type since it's not possible to
     * separate them in the API.
     */
    @Override
    public FetchAccountsResponse fetchCreditCardAccounts() {
        return new FetchAccountsResponse(
                getAccounts().stream().map(x -> parseAccount(x)).collect(Collectors.toList()));
    }
