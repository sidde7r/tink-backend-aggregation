...
public class BanquePopulaireRedirectHandler extends RedirectHandler {
        ...
        try {
            ...
            String transactionId =
                    parts.stream()
                            .filter(
                                    p ->
                                            BanquePopulaireConstants.Query.TRANSACTION_ID
                                                    .equalsIgnoreCase(p.getName()))
                            .map(NameValuePair::getValue)
                            .findFirst()
                            .orElseThrow(() -> new IllegalStateException("No transactionId found"));

            return new URL(
                            redirectUri.getScheme(),
                            redirectUri.getHost(),
                            MessageFormat.format(newLocation, transactionId))
                    .toExternalForm();
        } catch (Exception e) {
    ...
    private Optional<AppConfigEntity> getAppConfig() {
        return sessionStorage.get(
                BanquePopulaireConstants.Storage.APP_CONFIGURATION, AppConfigEntity.class);
    }
}
