...
public class NordnetAgent extends AbstractAgent
    ...
    /**
     * We've previously for BANKID and PASSWORD stored an access token for persistent login. Though
     * it doesn't work anymore, and it also wasn't a long-term access token so gave not much value.
     *
     * <p>Probably a keepAlive would work if we persisted cookies, but at this point there is just a
     * small value of it.
     *
     * <p>For legacy reason this method is needed. We used to change a credential from BANKID to
     * PASSWORD if access token was present (as a kind of hack of not needing two factor on next
     * login). Thus we need to check if we're password and double check that the credentials has no
     * password --> originally BANKID credential that should be reset so that we can log it in
     * again.
     */
            ...
            switch (status) {
                ...
                default:
                    throw new IllegalStateException(
                            "#login-refactoring - Unknown error detected while collecting BankID status: "
                                    + status);
            }
    ...
    public boolean login() throws Exception {
        ...
        switch (credentials.getType()) {
            ...
            default:
                throw new IllegalStateException(
                        String.format("Unsupported credentials type(%s)", credentials.getType()));
        }
    }
                ...
                .forEach(
                        accountEntity -> {
                            ...
                            if (accountEntity.isBlocked()) {
                                log.debug(
                                        String.format(
                                                "Account: %s blocked, reason: %s ",
                                                accountEntity.getAccountNumber(),
                                                accountEntity.getBlockedReason()));
                                return;
                            }
                            accounts.add(accountEntity.toAccount(AccountTypes.SAVINGS));
                        });
    ...
    public FetchTransactionsResponse fetchSavingsTransactions() {
        // NOTE:
        // Last commit SHA related to this agent with transaction aggregation:
        // feb1e91f9028a0a76e997b12146e2e4f1867e6d2
        // Though, since there are some transactions that are sent in other currencies we cannot get
        // a
        // complete view of
        // the account transactions and we thus removed the aggregation of them since they turned up
        // in
        // search and ID-koll.
    ...
    public FetchInvestmentAccountsResponse fetchInvestmentAccounts() {
        Map<Account, AccountFeatures> accounts = new HashMap<Account, AccountFeatures>();
        getAccounts()
                .forEach(
                        accountEntity -> {
                            ...
                            if (Objects.equals(
                                    "sparkonto",
                                    Strings.nullToEmpty(accountEntity.getAccountCode())
                                            .toLowerCase())) {
                                return;
                            }
                            // accounts can be blocked, will have no further data
                            ...
                            if (!positions.isPresent()) {
                                ...
                                // AccountFeatures.createForPortfolios(portfolio));
                                accounts.put(
                            ...
                            for (PositionEntity positionEntity : positions.get()) {
                                if (positionEntity
                                        .getAccountId()
                                        .equalsIgnoreCase(accountEntity.getAccountId())) {
