...
public class LclTransactionFetcher implements TransactionFetcher<TransactionalAccount> {
    ...
    /**
     * LCL don't support pagination in the app or on the web, right now we get around one month of
     * transactions. The user can get more history by going through pdf documents, but for now we
     * decided not to implement pdf parsing for this purpose.
     */
    @Override
    public List<AggregationTransaction> fetchTransactionsFor(TransactionalAccount account) {
        AccountDetailsEntity accountDetailsEntity =
                account.getFromTemporaryStorage(
                                LclConstants.Storage.ACCOUNT_DETAILS_ENTITY,
                                AccountDetailsEntity.class)
                        .orElseThrow(
                                () ->
                                        new IllegalStateException(
                                                "No account details entity found."));
