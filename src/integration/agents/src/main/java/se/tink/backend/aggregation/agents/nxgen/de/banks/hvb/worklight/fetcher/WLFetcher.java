...
public final class WLFetcher {
    ...
    public WLFetcher(
            final WLApiClient client, final WLFetcherStorage storage, final WLConfig config) {
    ...
    public <T> T getAccounts(final Class<T> entityClass) {
        return query(
                WLConstants.Procedure.fetch_FinancialStatusAccountDO.name(),
                entityClass,
                new JSONObject());
    }
    ...
    public <T> T getAccountHolders(final Class<T> entityClass) {
        return query(
                WLConstants.Procedure.fetch_AdvisorsListDO.name(), entityClass, new JSONObject());
    }
    ...
    private <T> T query(
            final String procedure, final Class<T> entityClass, final JSONObject payload) {
        final Form request =
                Form.builder()
                        ...
                        .put(
                                ...
                                SerializationUtils.serializeToString(
                                        getParameters(procedure, payload)))
                        .build();
        ...
        final QueryResponse responseBody =
                WLUtils.encasedJsonToEntity(httpResponse, QueryResponse.class);

        Preconditions.checkArgument(responseBody.getIsSuccessful());

        final byte[] ciphertextBytes =
                EncodingUtils.decodeBase64String(responseBody.getCipherText());
        final byte[] signatureBytes = EncodingUtils.decodeBase64String(responseBody.getSignature());

        final boolean isSignatureValid =
                verifySignature(config.getPublicKey(), signatureBytes, ciphertextBytes);
        if (!isSignatureValid) {
        ...
        // longLogger.infoExtraLong(plaintextString,
        // WLConstants.LogTags.WL_RECEIVED_PLAINTEXT.toTag());
    ...
    private List<QueryParameterEntity> getParameters(
            final String procedure, final JSONObject paylIWV) {
        ...
        final String plaintext =
                SerializationUtils.serializeToString(
                        new CFRequest(
                                paylIWV.toString(), DatatypeConverter.printHexBinary(z), "0"));
    ...
    private static boolean verifySignature(
            final PublicKey publicKey, byte[] signatureBytes, final byte[] ciphertext) {
