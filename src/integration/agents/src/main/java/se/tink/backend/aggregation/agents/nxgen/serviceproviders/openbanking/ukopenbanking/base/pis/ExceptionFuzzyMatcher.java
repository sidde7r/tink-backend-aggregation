...
public class ExceptionFuzzyMatcher {
    ...
    boolean isAuthorizationCancelledByUser(ErrorEntity error) {
        return StringUtils.containsIgnoreCase(
                error.getErrorMessage(), AUTHORIZATION_CANCELLED_BY_USER);
    }

    boolean isAuthorizationFailedByUser(ErrorEntity error) {
        return StringUtils.containsIgnoreCase(
                error.getErrorMessage(), AUTHORIZATION_FAILED_BY_USER);
    }

    boolean isAuthorizationTimeOut(ErrorEntity error) {
