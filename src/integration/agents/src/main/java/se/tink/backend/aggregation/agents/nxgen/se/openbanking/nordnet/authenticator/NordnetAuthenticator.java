...
public class NordnetAuthenticator implements PasswordAuthenticator {
    ...
    public void authenticate(String username, String password)
        ...
        String login =
                toBase64String(username.getBytes())
                        + ":"
        ...
        byte[] encryptedBytes =
                RSA.encryptEcbPkcs1(publicKey2, login.getBytes(StandardCharsets.UTF_8));
        String authParam = toBase64String(encryptedBytes);

        GetSessionKeyResponse sessionKeyResponse = apiClient.getSessionKey(authParam);
        sessionStorage.put(
                NordnetConstants.StorageKeys.SESSION_KEY, sessionKeyResponse.getSessionKey());
    }
