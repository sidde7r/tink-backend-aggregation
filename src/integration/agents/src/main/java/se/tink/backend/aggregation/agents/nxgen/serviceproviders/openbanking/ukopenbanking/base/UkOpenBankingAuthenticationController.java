...
import se.tink.backend.aggregation.agents.nxgen.serviceproviders.openbanking.ukopenbanking.base.jwt.validator.IdTokenValidator;
...
@Slf4j
public class UkOpenBankingAuthenticationController
    ...
    private final UkOpenBankingAuthenticator authenticator;
    ...
    private UkOpenBankingAuthenticationController(
            PersistentStorage persistentStorage,
            SupplementalInformationHelper supplementalInformationHelper,
            UkOpenBankingApiClient apiClient,
            UkOpenBankingAuthenticator authenticator,
    ...
    public ThirdPartyAppResponse<String> collect(String reference)
        ...
        String code =
                getCallbackElement(callbackData, UkOpenBankingV31Constants.CallbackParams.CODE)
                        .orElseGet(
                                () -> {
                                    log.error(
        ...
        String state =
                getCallbackElement(callbackData, UkOpenBankingV31Constants.Params.STATE)
                        .orElse(null);

        Optional<String> idToken =
                getCallbackElement(callbackData, UkOpenBankingV31Constants.CallbackParams.ID_TOKEN);
    ...
    private void saveAccessToken(OAuth2Token oAuth2Token) {
        ...
        if (authenticator.getClientCredentialScope().equals(ClientMode.ACCOUNTS)) {
            persistentStorage.rotateStorageValue(
                    UkOpenBankingV31Constants.PersistentStorageKeys.AIS_ACCESS_TOKEN, oAuth2Token);
        }
    }
    ...
    private void handlePossibleErrors(Map<String, String> callbackData)
        ...
        Optional<String> error =
                getCallbackElement(callbackData, UkOpenBankingV31Constants.CallbackParams.ERROR);
        Optional<String> errorDescription =
                getCallbackElement(
                        callbackData, UkOpenBankingV31Constants.CallbackParams.ERROR_DESCRIPTION);
    ...
    private void handleError(
        ...
        if (UkOpenBankingV31Constants.Errors.ACCESS_DENIED.equalsIgnoreCase(errorType)
        ...
        } else if (UkOpenBankingV31Constants.Errors.TEMPORARILY_UNAVAILABLE.equalsIgnoreCase(
                errorType)) {
