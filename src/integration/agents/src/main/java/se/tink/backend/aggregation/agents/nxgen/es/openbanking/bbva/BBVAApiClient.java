...
public final class BBVAApiClient {
    ...
    public PaginatorResponse fetchTransactions(TransactionalAccount account, int page) {
        return client.request(new URL(
            persistentStorage.get(StorageKeys.BASE_API_URL) + String
                .format(Urls.ACCOUNT_TRANSACTIONS, account.getApiIdentifier())))
            .header(HttpHeaders.ACCEPT, MediaType.APPLICATION_JSON)
            .header(BBVAConstants.HeaderKeys.AUTHORIZATION, getHeaderFormattedTokenFromSession())
    ...
    public URL getAuthorizeUrl(String state) {
        return client.request(new URL(
            ...
            .queryParam(BBVAConstants.QueryKeys.RESPONSE_TYPE,
                BBVAConstants.QueryValues.RESPONSE_TYPE)
            .queryParam(BBVAConstants.QueryKeys.CLIENT_ID,
                persistentStorage.get(BBVAConstants.StorageKeys.CLIENT_ID))
            .queryParam(BBVAConstants.QueryKeys.REDIRECT_URI,
    ...
    public OAuth2Token getToken(String code) {
        return client
            ...
            .queryParam(BBVAConstants.QueryKeys.GRANT_TYPE, BBVAConstants.QueryValues.GRANT_TYPE)
            .queryParam(BBVAConstants.QueryKeys.CODE, code)
            .queryParam(BBVAConstants.QueryKeys.REDIRECT_URI,
                persistentStorage.get(BBVAConstants.StorageKeys.REDIRECT_URI))
            .header(BBVAConstants.HeaderKeys.AUTHORIZATION, String
                .format(BBVAConstants.HeaderValues.AUTHORIZATION_RESPONSE, getBase64Credentials()))
        ...
        return client
            .request(
                new URL(
                    persistentStorage.get(BBVAConstants.StorageKeys.BASE_AUTH_URL)
                        + BBVAConstants.Urls.TOKEN))
            .queryParam(BBVAConstants.QueryKeys.GRANT_TYPE, BBVAConstants.QueryValues.REFRESH_TOKEN)
            .queryParam(BBVAConstants.QueryKeys.REDIRECT_URI,
                persistentStorage.get(BBVAConstants.StorageKeys.REDIRECT_URI))
            .header(BBVAConstants.HeaderKeys.AUTHORIZATION, String
                .format(BBVAConstants.HeaderValues.AUTHORIZATION_RESPONSE, getBase64Credentials()))
            .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_FORM_URLENCODED)
            .post(TokenResponse.class, refreshRequest.toForm()).toTinkToken();
    }

    public String getBase64Credentials() {
        return Base64.getEncoder().encodeToString(
            (persistentStorage.get(BBVAConstants.StorageKeys.CLIENT_ID) + ":" +
                persistentStorage.get(BBVAConstants.StorageKeys.CLIENT_SECRET)).getBytes());
    }
    ...
    private OAuth2Token getTokenFromSession() {
        return sessionStorage
            ...
            .orElseThrow(() -> new IllegalStateException(BBVAConstants.Exceptions.MISSING_TOKEN));
    }

    private String getHeaderFormattedTokenFromSession() {
        return String.format(BBVAConstants.HeaderValues.AUTHORIZATION,
            getTokenFromSession().getAccessToken());
    }
}
