...
public class IngCardReaderAuthenticator {
    ...
    public IngCardReaderAuthenticator(
    ...
    public ChallengeExchangeValues initEnroll(String ingId, String cardNumber, String otp)
        ...
        InitEnrollResponse initEnrollResponse =
                initEnrollHttpResponse.getBody(InitEnrollResponse.class);
        ...
        PrepareEnrollResponseEntity prepEnrollResponseEntity =
                this.apiClient.prepareEnroll(prepEnrollUrl);
    ...
    public void confirmEnroll(String ingId, String challengeResponse, String signingId)
        ...
        BaseMobileResponseEntity confirmEnrollResponseEntity =
                this.apiClient.confirmEnroll(
                        confirmEnrollUrl,
                        ingId,
                        signingId,
                        challengeResponse,
    ...
    public void authenticate(String ingId) throws LoginException {
        int otpCounter =
                Integer.parseInt(this.persistentStorage.get(IngConstants.Storage.OTP_COUNTER));
        ...
        if (!IngHelper.isLoginSuccessful(res)) {
            ...
            if (IngConstants.ErrorCodes.NO_ACCESS_TO_ONLINE_BANKING.equalsIgnoreCase(
                    errorResponse.getMobileResponse().getErrorCode().get())) {
            ...
            String errormsg =
                    String.format(
                            ...
                            errorResponse.getMobileResponse().getErrorCode().get(),
                            " Message: ",
                            errorResponse.getMobileResponse().getErrorText().get());
    ...
    private void calcAndPersistAuthValues(
            String ingId, InitEnrollResponseEntity initEnrollResponseEntity) {
        ...
        byte[] credentials =
                executeGetAppCredentials(
                        regCode,
                        this.cryptoInitValues.getSessionKey(),
        ...
        persistAuthValues(
                this.cryptoInitValues, ingId, virtualCardNumber, otpKey, pin, secret0, secret1);
    }

    private void persistAuthValues(
            CryptoInitValues cryptoInitValues,
            String ingId,
            String compositeVirtualCardNumber,
            byte[] otpKey,
            String systemPin,
            byte[] secret0,
            byte[] secret1) {
        ...
        this.persistentStorage.put(
                IngConstants.Storage.OTP_KEY_HEX, EncodingUtils.encodeHexAsString(otpKey));
        this.persistentStorage.put(IngConstants.Storage.SYSTEM_PIN, systemPin);
        this.persistentStorage.put(
                IngConstants.Storage.SECRET_0_IN_HEX, EncodingUtils.encodeHexAsString(secret0));
        this.persistentStorage.put(
                IngConstants.Storage.SECRET_1_IN_HEX, EncodingUtils.encodeHexAsString(secret1));
        this.persistentStorage.put(
                IngConstants.Storage.SESSION_KEY_IN_HEX,
                EncodingUtils.encodeHexAsString(cryptoInitValues.getSessionKey()));
        this.persistentStorage.put(
                IngConstants.Storage.SESSION_KEY_AUTH_IN_HEX,
                EncodingUtils.encodeHexAsString(cryptoInitValues.getSessionKeyAuth()));
    }

    private byte[] executeGetAppCredentials(
            String regCode, byte[] sessionKey, byte[] sessionKeyAuth) {
        byte[] encryptedQueryData =
                IngCryptoUtils.generateEncryptedQueryData(regCode, sessionKey, sessionKeyAuth);
        String getAppCredentialsUrl =
                this.ingHelper.getUrl(IngConstants.RequestNames.GET_APP_CREDENTIALS);
        ...
        return IngCryptoUtils.decryptAppCredentials(
                encryptedAppCredentials, sessionKey, sessionKeyAuth);
    }
    ...
    private void checkForUserInputError(HttpResponse httpResponse) throws LoginException {
        // We don't get a specific error msg if a user enters wrong ING ID, card ID, or response
        // code,
    ...
    private void checkForChallengeExchangeError(BaseMobileResponseEntity responseEntity)
        ...
        if (IngConstants.ReturnCodes.NOK.equalsIgnoreCase(returnCode)) {
            ...
            if (errorText.isPresent()) {
                throw LoginError.INCORRECT_CREDENTIALS.exception(
                        new LocalizableKey(errorText.get()));
            } else {
