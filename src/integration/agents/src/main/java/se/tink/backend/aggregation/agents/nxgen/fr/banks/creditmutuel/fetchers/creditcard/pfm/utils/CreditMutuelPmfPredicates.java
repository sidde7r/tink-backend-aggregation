...
public class CreditMutuelPmfPredicates {
    ...
    public static Function<CreditCardResponse, Stream<SubItemsEntity>> getSubItemsValueStreamFromResponse = c ->
            c.getView().getSections().stream()
                    .flatMap(s -> s.getItems().stream().map(x -> x.getSubItems()));

    public static Function<CreditCardResponse, Stream<ValueEntity>> getItemEntitiesFromResponse = r -> r.getView()
            .getSections().stream()
            .flatMap(s -> s.getItems().stream().flatMap(x -> x.getOutput().stream()));

    public static Function<CreditCardResponse, Stream<ValueEntity>> getValueEntitiesFromEachInput = r -> r.getView()
            .getSections().stream()
            .flatMap(s -> s.getItems().stream().flatMap(x -> x.getOutput().stream()));
