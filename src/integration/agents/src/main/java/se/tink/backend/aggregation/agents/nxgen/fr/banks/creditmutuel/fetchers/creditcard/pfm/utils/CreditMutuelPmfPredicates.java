...
import se.tink.backend.aggregation.agents.nxgen.fr.banks.creditmutuel.fetchers.creditcard.pfm.entities.ItemsEntity;
...
public class CreditMutuelPmfPredicates {
    ...
    public static final Function<CreditCardResponse, Stream<SubItemsEntity>>
                    ...
                    c ->
                            c.getView().getSections().stream()
                                    .flatMap(
                                            s ->
                                                    s.getItems().stream()
                                                            .map(ItemsEntity::getSubItems));

    public static final Function<CreditCardResponse, Stream<ValueEntity>>
            getItemEntitiesFromResponse =
                    r ->
