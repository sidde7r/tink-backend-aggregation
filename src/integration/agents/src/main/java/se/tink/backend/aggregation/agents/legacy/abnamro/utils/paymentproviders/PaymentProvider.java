...
public abstract class PaymentProvider {
    ...
    private static final ImmutableSortedSet<Integer> WHITE_SPACE_POSITIONS =
            ImmutableSortedSet.of(84, 51, 50, 18).descendingSet();

    /**
     * Matches web addresses either on for example the form "www.pay.nl", "pay.com", or
     * "http://www.erik.se"
     *
     * <p>Note, this is not a "correct pattern" in the sense that it will match all valid domain
     * names. Some cases are ignored by purpose. See test cases for examples.
     *
     * <p>Some patterns that we don't match: 123abc.com => A valid domain name but we don't allow it
     * to start with digits www.aaa.bbb.com => A valid sub domain but we don't allow
     */
    private static final Pattern DOMAIN_PATTERN =
            Pattern.compile(
                    "(^|\\s)(http(s)?(:)?\\/\\/)?(?<value>(www\\.)?[A-z]([^\\s\\.]|-){2,}\\.(nl|com|eu|se))",
                    Pattern.CASE_INSENSITIVE);

    /** Matches order patterns, used in payment providers that needs this */
    protected static final Pattern ORDER_PATTERN =
            Pattern.compile(
    ...
    private static String matchDomainNamePattern(String description) {
        ...
        // Return the longest name if we have multiple matches. This is done since we have cases
        // where we
