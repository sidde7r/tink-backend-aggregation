...
public class SabadellInvestmentFetcher implements AccountFetcher<InvestmentAccount> {
    ...
    private void logDeposits() {
        try {
            ...
            if (!depositsResponse.getAccountsPositions().isEmpty()) {
                log.infoExtraLong(SerializationUtils.serializeToString(depositsResponse),
                        SabadellConstants.Tags.DEPOSITS);
            }
        } catch (Exception e) {
            log.warn(String.format("%s could not fetch deposits",
                    SabadellConstants.Tags.DEPOSITS_ERROR), e);
        }
    }
    ...
    private void logServicingFunds() {
        try {
            ...
            log.warn(String.format("%s could not fetch servicing funds",
                    SabadellConstants.Tags.SERVICING_FUNDS_ERROR), e);
        }
    }
    ...
    private void logPensionPlans() {
        try {
            ...
            if (!pensionPlansResponse.getAccounts().isEmpty()) {
                log.infoExtraLong(SerializationUtils.serializeToString(pensionPlansResponse),
                        SabadellConstants.Tags.PENSION_PLANS);
            }
        } catch (Exception e) {
            log.warn(String.format("%s could not fetch pension plans",
                    SabadellConstants.Tags.PENSION_PLANS_ERROR), e);
        }
    }
    ...
    private void logSavings() {
        try {
            ...
            if (!savingsResponse.getSavingPlans().isEmpty()) {
                log.infoExtraLong(SerializationUtils.serializeToString(savingsResponse),
                        SabadellConstants.Tags.SAVINGS);
            }
        } catch (Exception e) {
            log.warn(String.format("%s could not fetch savings",
                    SabadellConstants.Tags.SAVINGS_ERROR), e);
        }
