...
public class WizinkAccountFetcherTest {
    private static final String TEST_DATA_PATH_AUTHENTICATOR =
    ...
    public void shouldReturnEmptyCollectionWhenNoCreditCardsAvailable() {
        // given
        when(wizinkStorage.getAccounts()).thenReturn(Collections.emptyList());

        // when
        Collection<TransactionalAccount> creditCardAccounts = wizinkAccountFetcher.fetchAccounts();
    ...
    private void prepareStorageData() {
        CustomerLoginResponse loginResponse =
                SerializationUtils.deserializeFromString(
                        Paths.get(TEST_DATA_PATH_AUTHENTICATOR, "customer_login_response.json")
                                .toFile(),
                        CustomerLoginResponse.class);
        when(wizinkStorage.getAccounts())
                .thenReturn(loginResponse.getLoginResponse().getGlobalPosition().getProducts());
