...
public class EnterCardAgentPaymentTest {
    ...
    private List<Payment> createListMockedDomesticPayment(int numberOfMockedPayments) {
        ...
        for (int i = 0; i < numberOfMockedPayments; ++i) {
            ...
            doReturn(Iban.random(CountryCode.SE).getAccountNumber().substring(0, 15))
                    .when(creditor)
                    .getAccountNumber();

            Debtor debtor = mock(Debtor.class);
            doReturn(AccountIdentifier.Type.SE).when(debtor).getAccountIdentifierType();
            doReturn("123456789").when(debtor).getAccountNumber();
