...
public class IbanIdModuleMapper
        implements Mapper<IdModule, se.tink.sa.services.fetch.account.IdModule> {

    // TODO: this should be generic and type base dispatched

    @Override
    public IdModule map(
            se.tink.sa.services.fetch.account.IdModule source, MappingContext mappingContext) {
        IdModule dest =
                IdModule.builder()
                        ...
                        .addIdentifier(
                                AccountIdentifier.create(
                                        AccountIdentifier.Type.IBAN, source.getUniqueId()))
                        .build();
        return dest;
    }
}
