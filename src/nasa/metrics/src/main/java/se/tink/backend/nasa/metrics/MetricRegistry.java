...
public class MetricRegistry {
     ...
     * Register a metric.
     *
     * <p>This method assumes that nothing else has registered with the same name. <emph>If someone
     * else has registered a metric with the same name, this method will not have any effect and
     * your metrics _will_ be lost.</emph> It is recommended to use factory methods such as {@link
     * #histogram(MetricId)}, {@link #timer(MetricId)}, {@link #lastUpdateGauge(MetricId)}, {@link
     * #minGauge(MetricId)} or {@link #maxGauge(MetricId)} instead.
     *
    ...
    public <T extends Metric> T registerSingleton(@Nonnull MetricId id, @Nonnull T metric) {
        ...
        preexistingMetric.ifPresent(
                k -> {
                    // Majority of these cases are Gauges which are reregistered. I've already
                    // discovered a few cases of these.
                    LOG.warn(
                            String.format(
    ...
    private <T extends Metric> Optional<T> registerOrReturnPreexistingMetric(
            @Nonnull MetricId id, @Nonnull T metric) {
        Preconditions.checkNotNull(id);

        // Important as we otherwise risk getting `null` values in `metrics`, which means
        // `ConcurrentMap#putIfAbsent`
        // returning `null` could mean that value was either absent _or_ present.
        Preconditions.checkNotNull(metric);

        // We assume that a metric with the exact same MetricId is of the same type. If not, a
     ...
     *
     * <p>This method is mostly only useful if you use {@link #registerSingleton(MetricId, Metric)}.
     *
