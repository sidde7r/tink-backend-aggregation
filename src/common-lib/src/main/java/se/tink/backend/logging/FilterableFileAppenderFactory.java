 ...
 * <td>
 * The filename pattern for archived files. {@code %d} is replaced with the date in {@code yyyy-MM-dd} form, and the
 * fact that it ends with {@code .gz} indicates the file will be gzipped as it's archived. Likewise, filename patterns
 * which end in {@code .zip} will be filled as they are archived.</td>
 * </tr>
 * <tr>
 * <td>{@code archivedFileCount}</td>
 * <td>{@code 5}</td>
 * <td>
 * The number of archived files to keep. Must be greater than {@code 0}.</td>
 * </tr>
 * <tr>
 * <td>{@code timeZone}</td>
 * <td>{@code UTC}</td>
 ...
 */
@JsonTypeName("filterableFile")
public class FilterableFileAppenderFactory extends AbstractAppenderFactory {
    @NotNull
    ...
    private List<String> ignoredPathPrefixes = ImmutableList.of();
    ...
    public Appender<ILoggingEvent> build(LoggerContext context, String applicationName, Layout<ILoggingEvent> layout) {
        ...
        addIgnoredPathsFilters(appender, getIgnoredPathPrefixes());
        appender.stop();
        appender.start();
    ...
    private static void addIgnoredPathsFilters(FileAppender<ILoggingEvent> appender, List<String> ignoredPathPrefixes) {
        IgnoredPathPrefixFilter filter = new IgnoredPathPrefixFilter(ignoredPathPrefixes);
