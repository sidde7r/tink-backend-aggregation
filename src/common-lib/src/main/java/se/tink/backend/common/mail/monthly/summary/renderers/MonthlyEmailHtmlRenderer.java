...
public class MonthlyEmailHtmlRenderer {
    ...
    public String renderEmail(PooledRythmProxy pooledRythmProxy, EmailContent input) {
        ...
        context.put("cdn", "https://s3-eu-west-1.amazonaws.com/tink-web/activities/monthly-summary");

        return pooledRythmProxy.render(Template.MONTHLY_SUMMARY_TEMPLATE_INLINED_HTML, context);
    }
    ...
    private void addFooter(Map<String, Object> context, EmailContent input, Catalog catalog) {
        ...
        String link = String.format("https://www.tinkapp.com/b/#subscription/%s/%s",
                input.getUnsubscribeToken(),
                input.getLocale());

        String linkFormat = catalog.getString(
                "If you no longer want to receive these emails, please click <a href=\"%s\">here</a>.");

        context.put("footer_description", String.format(linkFormat, link));
    }
