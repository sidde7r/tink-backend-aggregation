...
public class TypedThreadPoolBuilder {
    ...
    public TypedThreadPoolBuilder withMaximumPoolSize(int maximumPoolSize, long keepAliveTime, TimeUnit unit) {
        ...
        Preconditions
                .checkArgument(keepAliveTime >= 0,
                        "keepAliveTime must be zero or positive. Was: %d", keepAliveTime);
    ...
    /**
     * Build the thread pool. This method is package protected because it uses a
     * {@link BlockingRejectedExecutionHandler} which can block a calling thread and is considered <a
     * href="http://stackoverflow.com/a/3518588/2057275">bad practise</a>. The reason why we can use it here is that
     * thread pool created by this class only will be used within an {@link ListenableThreadPoolExecutor} which
     * has its own {@link java.util.concurrent.RejectedExecutionHandler}.
     *
    ...
    /*package protected*/ ThreadPoolExecutor build() {
        return new ThreadPoolExecutor(corePoolSize, maximumPoolSize, keepAliveTime, keepAliveTimeUnit,
                new SynchronousQueue<Runnable>(), threadFactory, new BlockingRejectedExecutionHandler());
    }
