...
public class Credentials implements Cloneable {
    ...
    public void addSerializedFields(String maskedFields) {
        ...
        Map<String, String> newFields = SerializationUtils
                .deserializeFromString(maskedFields, FieldsMap.class);
    ...
    private Map<String, String> separateFields(Provider provider, boolean masked) {
        ...
        Set<Map.Entry<String, String>> credentialsFields = getFields().entrySet();
        List<Field> providerFields = provider.getFields();

        for (final Map.Entry<String, String> fieldEntry : credentialsFields) {
    ...
    public se.tink.backend.aggregation.rpc.Credentials toAggregationCredentials() {
        se.tink.backend.aggregation.rpc.Credentials credentials = new se.tink.backend.aggregation.rpc.Credentials();

        credentials.setAdditionalInformation(this.getAdditionalInformation());
        credentials.setDebugUntil(this.debugUntil);
        credentials.setFieldsSerialized(this.fieldsSerialized);
        credentials.setId(this.id);
        credentials.setImages(this.images != null ? this.images.toAggregationImageUrls() : null);
        credentials.setNextUpdate(this.nextUpdate);
        credentials.setPassword(this.getPassword());
        credentials.setPayload(this.payload);
        credentials.setProviderLatency(this.providerLatency);
        credentials.setProviderName(this.providerName);
        credentials.setStatus(se.tink.backend.aggregation.rpc.CredentialsStatus.valueOf(this.status.name()));
        credentials.setSensitivePayloadSerialized(getSensitivePayloadSerialized());
        credentials.setSecretKey(this.secretKey);
        credentials.setStatusPayload(this.statusPayload);
        credentials.setStatusPrompt(this.statusPrompt);
        credentials.setStatusUpdated(this.statusUpdated);
        credentials.setSupplementalInformation(this.supplementalInformation);
        credentials.setType(se.tink.backend.aggregation.rpc.CredentialsTypes.valueOf(this.type.name()));
        credentials.setUpdated(this.updated);
        credentials.setUserId(this.userId);
