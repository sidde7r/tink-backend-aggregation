...
public class LowHeapMemoryMonitor {
    ...
    @Inject public LowHeapMemoryMonitor(@Nonnull MetricRegistry metricRegistry) {
        Counter thresholdExceededCounter = metricRegistry.meter(MetricId.newId("heap_threshold_exceeded"));
        MemoryPoolMXBean tenuredGenerationMemoryPool = findTenuredGenerationMemoryPool()
        ...
        subscribeToMemoryPoolThreshold(tenuredGenerationMemoryPool, LOW_MEMORY_THRESHOLD, thresholdExceededCounter);

        log.debug("Started heap memory monitoring successfully");
    }

    private static void subscribeToMemoryPoolThreshold(MemoryPoolMXBean memoryPoolMxBean, double threshold,
                                                       Counter counter) {
    ...
    @VisibleForTesting static void onThresholdExceeded(Notification notification, MemoryPoolMXBean memoryPoolMxBean,
                                                       double threshold, Counter counter) {
        ...
        if (MemoryNotificationInfo.MEMORY_THRESHOLD_EXCEEDED.equals(notification.getType())) {
            log.debug("Tenured generation memory pool usage {}% exceeds threshold, adjusting collection threshold",
                    String.format("%.4f", relativeMemoryUsage));
            updateMemoryPoolThresholds(memoryPoolMxBean, threshold);
        } else if (MemoryNotificationInfo.MEMORY_COLLECTION_THRESHOLD_EXCEEDED.equals(notification.getType())) {
            log.error("Tenured generation memory pool usage {}% after garbage collection exceeded threshold, " +
    ...
    @VisibleForTesting static void updateMemoryPoolThresholds(MemoryPoolMXBean memoryPoolMxBean, double threshold) {
        MemoryUsage usage = memoryPoolMxBean.getUsage();
    ...
    @VisibleForTesting static Optional<MemoryPoolMXBean> findTenuredGenerationMemoryPool() {
