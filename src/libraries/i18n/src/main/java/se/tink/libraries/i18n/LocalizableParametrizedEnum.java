 ...
 * <p>
 * Catalog usage:
 * catalog.getString( ENUM_VALUE.with( parameters... ) );
 * or
 * LocalizableParametrizedKey message = ENUM_VALUE.with( parameters... );
 ...
 *
 * Since enums are static, and the parameters are dynamic, the enum itself
 * cannot specify the parameters.
 * The parameters cannot be injected to the enum either, since that would
 * change all instances of that enum to have the same parameters between threads.
 *
 * There for the LocalizableParametrized key stored in the enum do not have any
 * parameters specified, instead the with() method will build a new instance
 * based on the injected parameters.
 *
