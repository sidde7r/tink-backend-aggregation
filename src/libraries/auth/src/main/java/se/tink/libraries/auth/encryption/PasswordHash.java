...
public class PasswordHash {
    ...
    public static boolean check(String cleartext, String userHash, Collection<HashingAlgorithm> permittedAlgorithms) {
        Optional<HashingAlgorithm> algorithm = HashingAlgorithm.fromHash(userHash);
        if (!algorithm.isPresent() || permittedAlgorithms.stream().noneMatch(a -> Objects.equals(algorithm.get(), a))) {
            return false;
        } else {
