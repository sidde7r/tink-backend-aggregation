...
public class ProviderFileModule extends AbstractModule {
    ...
    private static final String AVAILABLE_PROVIDERS_FILE_PATH =
            "data/seeding/providers/available-providers";
    private static final String PROVIDER_OVERRIDE_FILE_PATH =
            "data/seeding/providers/overriding-providers";
    private static final String AGENT_CAPABILITIES_FILE_PATH =
            "data/seeding/providers/capabilities";
    ...
    public Map<String, ProviderConfigurationStorage> providerConfigurationByProviderName()
            throws IOException {
    ...
    public Map<String, Map<String, ProviderConfigurationStorage>>
            provideClusterSpecificProviderConfiguration() throws IOException {
    ...
    public Map<String, Set<ProviderConfigurationStorage.Capability>> provideAgentCapabilities()
            throws IOException {
        return loadAgentCapabilities();
    }

    protected Map<String, ProviderConfigurationStorage> loadProviderConfigurationFromJson()
            throws IOException {
    ...
    protected Map<String, Set<String>> loadEnabledProvidersOnClusterFromJson() throws IOException {
        ...
        for (File availableProviderDirectory : availableProviderDirectories) {
            File[] availableProviderFiles =
                    availableProviderDirectory.listFiles(
                            (dir, fileName) ->
                                    fileName.matches("available-providers-[A-Z]{2}.json"));
            Preconditions.checkNotNull(
                    availableProviderFiles,
    ...
    protected Map<String, Map<String, ProviderConfigurationStorage>>
            loadProviderOverrideOnClusterFromJson() throws IOException {

        File[] overridingProviderDirectories = new File(PROVIDER_OVERRIDE_FILE_PATH).listFiles();
        Preconditions.checkNotNull(
                overridingProviderDirectories,
                "no available path found for loading overriding providers on cluster");

        Map<String, Map<String, ProviderConfigurationStorage>> overridingProvidersByCluster =
                Maps.newHashMap();

        for (File overridingProviderDirectory : overridingProviderDirectories) {
            File[] availableProviderFiles =
                    overridingProviderDirectory.listFiles(
    ...
    private void loadEmptyOverrideForLocalDevelopment(
            Map<String, Map<String, ProviderConfigurationStorage>> overridingProvidersOnCluster) {
        overridingProvidersOnCluster.put(LOCAL_DEVELOPMENT_KEY_STRING, Collections.emptyMap());
    }

    private void enableAllProvidersForLocalDevelopment(
            Map<String, Set<String>> availableProvidersByCluster) throws IOException {
        Set<String> providerNames =
                loadProviderConfigurationFromJson().values().stream()
    ...
    private void parseAvailableProvidersOnCluster(
            File[] availableProviderFiles, Map<String, Set<String>> providersAvailableOnCluster)
            throws IOException {
        ...
        for (File availableProviderFile : availableProviderFiles) {
            ...
            Preconditions.checkNotNull(
                    clusterIdInFile,
                    "no available cluster id for file {}",
                    availableProviderFile.getName());
            ...
            Preconditions.checkState(
                    Objects.equals(clusterId, clusterIdInFile),
            ...
            log.info(
                    "found {} providers for cluster {} in from file {}",
                    providerNames.size(),
                    clusterId,
                    availableProviderFile.getName());
    ...
    private void parseOverridingProvidersOnCluster(
        ...
        for (File overridingProviderFile : overridingProviderFiles) {
            ...
            Preconditions.checkNotNull(
                    clusterIdInFile,
                    "no available cluster id for file {}",
                    overridingProviderFile.getName());
            ...
            log.info(
                    "{} provider overriding for cluster {} in from file {}",
                    providersOnCluster.size(),
                    clusterId,
                    overridingProviderFile.getName());

            providersOnCluster.forEach(
                    providerConfig ->
    ...
    private void parseProviderConfigurations(
            File providerFile,
            Map<String, ProviderConfigurationStorage> providerConfigurationByProviderName)
        ...
        for (ProviderConfigurationStorage providerConfigurationStorage : providers) {
            Preconditions.checkNotNull(
                    market,
                    "no market found for provider configuration file {}",
                    providerFile.getName());
            Preconditions.checkNotNull(
                    currency,
                    "no currency found for provider configuration file {}",
                    providerFile.getName());
    ...
    private AgentCapabilitiesMapModel loadAgentCapabilities() throws IOException {
        ...
        File[] agentCapabilities =
                directory.listFiles((dir, fileName) -> fileName.matches("agent-capabilities.json"));

        Preconditions.checkNotNull(agentCapabilities, "Capabilities must be provided.");

        if (agentCapabilities.length != 1) {
            log.warn(
                    "Found more than one file with agent capabilities. Only first file in array will be processed.");
        }
