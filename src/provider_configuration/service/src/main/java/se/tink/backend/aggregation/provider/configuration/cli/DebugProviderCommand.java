...
public class DebugProviderCommand
        extends ProviderConfigurationCommand<ProviderServiceConfiguration> {
    ...
    private void printProviderInfo(ProviderConfigurationCore provider) {
        ...
        output.add(
                CliPrintUtils.keyValueEntry("capabilities", provider.getCapabilitiesSerialized()));
        output.add(CliPrintUtils.keyValueEntry("classname", provider.getClassName()));
        output.add(
                CliPrintUtils.keyValueEntry(
                        "credentialstype", String.valueOf(provider.getCredentialsType())));
        output.add(CliPrintUtils.keyValueEntry("currency", provider.getCurrency()));
        output.add(
                CliPrintUtils.keyValueEntry(
                        "displaydescription", provider.getDisplayDescription()));
        output.add(CliPrintUtils.keyValueEntry("displayname", provider.getDisplayName()));
        output.add(
                CliPrintUtils.keyValueEntry(
                        "fields", SerializationUtils.serializeToString(provider.getFields())));
        ...
        output.add(
                CliPrintUtils.keyValueEntry(
                        "multifactor", String.valueOf(provider.isMultiFactor())));
        ...
        output.add(
                CliPrintUtils.keyValueEntry(
                        "refreshfrequency", String.valueOf(provider.getRefreshFrequency())));
        output.add(
                CliPrintUtils.keyValueEntry(
                        "refreshfrequencyfactor",
                        String.valueOf(provider.getRefreshFrequencyFactor())));
        output.add(CliPrintUtils.keyValueEntry("status", provider.getStatus().name()));
        output.add(
                CliPrintUtils.keyValueEntry(
                        "transactional", String.valueOf(provider.isTransactional())));
    ...
    protected void run(
            ...
            Namespace namespace,
            ProviderServiceConfiguration configuration,
            Injector injector)
            throws Exception {
        ...
        ProviderConfigurationProvider providers =
                injector.getInstance(ProviderConfigurationProvider.class);
        ProviderConfigurationCore provider =
                providers.findByClusterIdAndProviderName(clusterId, providerName);
