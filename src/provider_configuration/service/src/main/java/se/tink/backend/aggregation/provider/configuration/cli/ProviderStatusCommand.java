...
public class ProviderStatusCommand
    ...
    public ProviderStatusCommand() {
        super("provider-status", "Change status for a providers or list all current statuses.");
    }
    ...
    public void configure(Subparser subparser) {
        ...
        Subparser listProviders =
                subparsers
                        .addParser("list")
                        .description("List provider statuses")
                        .setDefault(SHOW_FIELD, true);
        listProviders
                .addArgument("-m", "--market")
                .dest(MARKET_FIELD)
                .type(String.class)
                .required(false)
                .help("Market to list provider for");
        listProviders
                .addArgument("-c", "--cluster")
                .dest(CLUSTER_FIELD)
                .type(String.class)
                .required(true)
                .help("Cluster to list provider for");

        Subparser updateProviderstatus =
                subparsers
                        .addParser("global_status")
                        .description("Update provider status by name")
                        .setDefault(
                                SHOW_FIELD,
                                false) // need to set default otherwise it's a null pointer
        ...
        updateProviderstatus
                .addArgument("-s", "--status")
                .dest(STATUS_FIELD)
                .type(ProviderStatuses.class)
                .required(true)
                .help("Status to change provider to");

        Subparser removeProviderStatus =
                subparsers
                        .addParser("no_global_status")
                        .description("Update provider status by name")
                        .setDefault(
                                SHOW_FIELD,
                                false) // need to set default otherwise it's a null pointer
    ...
    private ProviderConfigurationProvider createConfigurationProvider(Injector injector)
            throws IOException {
    ...
    private void printProviderStatus(
            List<ProviderConfigurationCore> providerConfigurationCoreList) {
        Map<String, ProviderStatuses> providerStatusesList =
                providerConfigurationCoreList.stream()
                        .collect(
                                Collectors.toMap(
                                        ProviderConfigurationCore::getName,
                                        ProviderConfigurationCore::getStatus));
    ...
    protected void run(
        ...
        if (namespace.getBoolean(SHOW_FIELD)) {
            ...
            if (Objects.isNull(market)) {
                ...
                providerConfigurationCoreList =
                        configurationProvider.findAllByClusterIdAndMarket(clusterId, market);
            }
