...
GIT_CRYPT_COLLABORATOR_SUBJECT_FORMAT='^Add [0-9]+ git-crypt collaborators?$'
...
if ! echo "$COMMIT_MSG_SUBJECT" | grep --quiet --extended-regexp "${BORS_COMMIT_MSG_SUBJECT_FORMAT}"; then
    ...
    exitCodeConventionalCommit=$?
    echo "$COMMIT_MSG_SUBJECT" | grep --quiet --extended-regexp "${GIT_CRYPT_COLLABORATOR_SUBJECT_FORMAT}"
    exitCodeGitCrypt=$?

    if [ $exitCodeConventionalCommit -ne 0 ] && [ $exitCodeGitCrypt -ne 0 ]; then
      print_error_header "$COMMIT"
